...
<input 
  v-if="selectedMeasure === 'custom'" 
  type="number" 
  v-model.number="customMeasure" 
  min="1" max="32"
  placeholder="Mesure personnalisée"
  class="custom-measure-input"
/>
...
volumeSub: parseFloat(localStorage.getItem("volumeSub") || "0.5"),
  ...
  computed: {
    ...
    measure() {
  if (this.selectedMeasure === 'custom') {
    const value = parseInt(this.customMeasure);
    return isNaN(value) || value < 1 ? 4 : value;
  } else {
    return parseInt(this.selectedMeasure);
  }
},
    swingLabel() {
