...
async mounted() {
  ...
  // 🔄 Écoute le changement de JWT dans un autre onglet
  window.addEventListener("storage", async (event) => {
    if (event.key === "jwt") {
      console.log("🔄 JWT mis à jour, récupération de la note et des données...");
      if (this && this.fetchNote) await this.fetchNote();
      if (this && this.fetchFromAPI) await this.fetchFromAPI(true);
    }
  });

  // ✅ Utilise une variable pour stopper le watcher
  const stopWatcher = this.$watch(
    () => auth.user,
    (newUser) => {
      if (newUser) {
        console.log("✅ Utilisateur maintenant chargé :", newUser);
        this.isAuthChecked = true;
        stopWatcher(); // ✅ on peut maintenant arrêter l’écoute
      }
    },
    { immediate: true }
  );
