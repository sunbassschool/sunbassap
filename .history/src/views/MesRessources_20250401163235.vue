...
selectedFile: null,
      ...
      routeId: "AKfycbwCrvZUTP9W0dGCzgMO_wdfgQWXeke3xAWLiXIIR8TdT57IWE3V90xj_E2JZOxrtx4n2A/exec"
    };
  },
...
,
  ...
  methods: {
    getEmoji(type) {
      switch (type) {
        case "Image": return "Tableaux de cours 🖼️ ";

        case "Audio": return "Playback 🎧";
        case "PDF": return "Partitions et Documents📄";
        default: return "📁";
      }
    },
    toggleSection(type) {
...
,
  ...
  try {
    ...
    this.files.sort((a, b) => {
      const dateA = new Date(a["Date (modif)"]);
      const dateB = new Date(b["Date (modif)"]);
      return dateB - dateA;
    });
    ...
    this.files.forEach(file => {
      ...
      if (!(type in this.collapsedSections)) {
        this.collapsedSections[type] = true;
      }
    });
