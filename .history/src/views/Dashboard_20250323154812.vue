...
async mounted() {
  ...
  try {
    ...
    // ✅ Stop ici si utilisateur non connecté (pas de JWT du tout)
    const jwt = await getValidToken({ silent: true }); // 👈 ajoute `silent` pour ne pas tenter de refresh
    if (!jwt) {
      console.warn("🚫 Aucun JWT : utilisateur anonyme, affichage mode invité.");
      this.cards = [ /* contenu générique */ ];
      await this.fetchNote();
      this.isLoading = false;
      return;
    }

    // ✅ Chargement des données utilisateur uniquement si JWT dispo
    ...
    this.$router.push("/login"); // 👈 Redirection explicite seulement si nécessaire
  } finally {
    this.isLoading = false;
  }
}
