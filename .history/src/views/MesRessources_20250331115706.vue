        ...
        >
          ...
          <div class="resource-carousel">
            ...
            >
              ...
              <div v-if="file['Type de ressource'] === 'Image' && file['Lien direct']">
                <img 
  :src="file['Lien direct']"
  :alt="file['Nom du fichier']"
  @error="$event.target.src = '/images/image-not-found.png'"
...
export default {
  ...
  const urlOriginal = file["URL directe"] || file["URL"] || file["Lien direct"] || "";

  const match = urlOriginal.match(/\/d\/([^/]+)\//);
  const id = match?.[1];
  if (id) {
    file["Lien direct"] = `https://drive.google.com/uc?id=${id}`;
  } else {
    file["Lien direct"] = urlOriginal;
  }

  console.log("💡 Vérif URL:", file["Nom du fichier"], file["Lien direct"]);
  return file;
});
