...
async mounted() {
  try {
    ...
    try {
      jwt = await getToken(); // 🧨 C’est LUI qui jette
    } catch (err) {
      console.warn("⚠️ Token non valide ou absent (mode invité) :", err.message);
    }
    ...
    if (!jwt) {
      ...
      await this.fetchNote(); // invité ou pas, on affiche une note
      this.isLoading = false;
      return;
    }

    // ✅ Utilisateur identifié
// Vérifie si l'utilisateur est connecté avant d'exécuter les fonctions
if (localStorage.getItem('jwt')) {
  ...
  console.warn("Utilisateur non connecté, pas d'appel API effectué.");
}
