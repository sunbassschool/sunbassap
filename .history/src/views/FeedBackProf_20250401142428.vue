  ...
  <Layout>
    <div class="container mt-4">
      ...
      <div v-if="feedbacks.length && !feedbackLoading">
        <div v-for="fb in feedbacks" :key="fb.ID" class="dashboard-card rounded-3 p-3 mb-3">
          <!-- 🎖️ Type et date -->
          ...
          <!-- 🧾 Réponses du Professeur sous le feedback de l'élève -->
...
export default {
  ...
  try {
    ...
    if (data.feedbacks) {
      ...
      // 🔍 Identifier les feedbacks principaux : ceux dont ID_Feedback_Relé est vide
      const principaux = all.filter(fb => !fb.ID_Feedback_Relé); // Feedbacks principaux
      const reponses = all.filter(fb => fb.ID_Feedback_Relé); // Réponses

      // 💬 Associer chaque feedback à ses réponses
      this.feedbacks = principaux.reverse().map(fb => ({
        ...fb,
        reponses: reponses.filter(r => r.ID_Feedback_Relé === fb.ID) // Filtre les réponses associées
      }));
    } else {
