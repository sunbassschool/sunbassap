...
export default {
  ...
  computed: {
    imageFiles() {
      return this.files.filter(file => file["Type de ressource"] === "Image");
    },
    filteredFiles() {
      return this.files.filter(file => file["Type de ressource"] !== "Image");
    }
  ...
  methods: {
    ...
    getFileIcon(extension) {
      ...
      if (["mp3", "wav", "ogg"].includes(ext)) return "bi bi-file-music";
      if (["jpg", "jpeg", "png", "gif", "webp"].includes(ext)) return "bi bi-file-image";
    ...
    async loadRessources() {
      ...
      try {
        ...
        // 🔧 Patch des liens Google Drive ici
        this.files = data.map(file => {
          if (file["Type de ressource"] === "Image" && file.lien.includes("drive.google.com")) {
            const match = file.lien.match(/\/d\/([^/]+)\//);
            if (match && match[1]) {
              const fileId = match[1];
              file.lien = `https://drive.google.com/uc?export=view&id=${fileId}`;
            }
          }
          return file;
        });
    ...
    toggleVu(file) {
      file.vu = !file.vu;
      console.log("📝 Statut vu modifié pour :", file["Nom du fichier"], "➡️", file.vu);
      // TODO: Appel API POST ici si besoin pour sauvegarder
    }
  }
};
</script>
