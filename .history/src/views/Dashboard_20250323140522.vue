<template>
  ...
  <!-- Plus besoin de condition, Layout gère tout -->
      ...
      <div v-if="isAuthChecked" class="content">
  ...
  :class="{
    'first-card': index === 0,
    'fade-in-smooth': firstLoadFromCache
  }"
>
...
const cacheKey = `userData_${prenom}`;
  ...
  return{
    ...
    isAuthChecked: false, // ✅ Nouvelle prop pour contrôler l’état d’auth
    firstLoadFromCache: false, 
    cards: [],
    isLoading: true,
...
,
  ...
  objectif() {
    ...
    return userInfos.objectif || `Ton objectif musical n'a pas encore été défini.<br>
...
async mounted() {
  ...
  // 🔐 S'assure que l'utilisateur est chargé (parfois loadUser retourne trop tôt)
  await auth.loadUser();
  console.log("📌 Chargement du tableau de bord...");

  // ✅ Watch de secours si `auth.user` est encore vide (ex : token refresh async)
  if (!auth.user?.email) {
    const stopWatcher = this.$watch(
      () => auth.user?.email,
      (email) => {
        if (email) {
          console.log("✅ Utilisateur maintenant chargé via watcher :", auth.user);
          this.user = auth.user;
...
if (
  ...
  console.log("⚡ Mise à jour des cartes via cache immédiat post-auth");
  this.firstLoadFromCache = true; // Active le flag pour l'effet visuel
  this.updateData(cachedData);
}
}

  // 🔄 Écouteur pour recharger si JWT changé dans un autre onglet
  window.addEventListener("storage", async (event) => {
    if (event.key === "jwt") {
      console.log("🔄 JWT mis à jour, récupération de la note et des données...");
      if (this.fetchNote) await this.fetchNote();
      if (this.fetchFromAPI) await this.fetchFromAPI(true);
    }
  });

  // 🛡️ Protection : si l'utilisateur est non connecté
  if (!this.isLoggedIn) {
    console.warn("👤 Utilisateur non connecté – affichage partiel sans API");
    this.cards = [
      {
        icon: "bi bi-calendar-event",
        title: "Prochain Cours",
        text: "🎸 Connecte-toi pour voir ton planning personnalisé !"
      },
      {
        icon: "bi bi-flag",
        title: "Objectif actuel",
        text: "🎯 Connecte-toi pour voir ton objectif coaching"
      }
    ];
  // 📝 On affiche la note locale même sans API
  ...
  // ⏱️ Refresh auto toutes les 10 minutes
  setInterval(() => {
...
async fetchFromAPI(forceRefresh = false) {
  ...
  console.warn("⚠️ Problème API : données non conformes", data);
  this.displayError();
  this.isLoading = false;
  return;
}
...
,
  ...
  if (data.objectif !== undefined) {
    cachedInfos.objectif = data.objectif;
  }
  ...
  const now = new Date();
  const coursAVenir = Array.isArray(updatedData.planning)
    ? updatedData.planning
        .filter(cours => cours["Prénom"]?.toLowerCase() === this.prenom.toLowerCase())
        .filter(cours => {
          const dt = new Date(cours["Date et heure"]);
          return dt.toString() !== "Invalid Date" && dt > now;
        })
        .sort((a, b) => new Date(a["Date et heure"]) - new Date(b["Date et heure"]))
    : [];

  const prochainCours = coursAVenir.length ? coursAVenir[0] : null;

  let coursText = "Aucun cours à venir.";
  if (prochainCours && prochainCours["Lien Google Meet"]) {
    const formattedDate = new Date(prochainCours["Date et heure"]).toLocaleString("fr-FR", {
      weekday: "long",
      day: "2-digit",
      month: "long",
      year: "numeric",
    ...
    coursText = `
      <div class="meet-button" onclick="window.open('${prochainCours["Lien Google Meet"]}', '_blank')">
  ...
  this.cards = [
    ...
    },
    {
      icon: "bi bi-flag",
      title: "Objectif actuel",
      text: updatedData.objectif || `Ton objectif musical n'a pas encore été défini.<br>
         🧭 <strong><a href='https://www.sunbassschool.com/step/inscription-aux-cours-en-visio/' target='_blank' style='color: #ff8c00;'>Prends un cours</a></strong> pour qu'on le construise ensemble ! 💪`
    }
  ];

  this.isLoading = false; // ✅ Toujours débloquer l'affichage
}
