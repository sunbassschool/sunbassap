...
fetchURL(route) {
  ...
  // 🔄 Met à jour les données en arrière-plan SANS afficher le loader
  setTimeout(async () => {
...
async fetchUserData(forceUpdate = false) {
  if (forceUpdate || !this.isDataReady) {
    console.log("🔄 Récupération des données utilisateur...");

    try {
      // Récupérer le prénom depuis IndexedDB ou localStorage
      const prenom = sessionStorage.getItem("prenom") || localStorage.getItem("prenom");
      ...
      console.log("🔗 URL de la requête :", url);  // Affiche l'URL avant l'envoi

      // Envoi de la requête
      const response = await fetch(url);
      console.log("📌 Statut HTTP de la réponse :", response.status); // Affiche le statut de la réponse
      ...
      if (!data.error) {
        ...
        localStorage.setItem(this.cacheKey, JSON.stringify(this.user)); // Sauvegarder dans le cache local
      } else {
