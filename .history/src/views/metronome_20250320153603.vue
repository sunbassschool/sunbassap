...
{ value: 4, label: '', icon: './assets/icons/sixteenth-note.png' }
  ...
  while (this.nextNoteTime < now + 0.1) {
    ...
    // 🔥 Ajout du swing : décaler les subdivisions paires
    let swingFactor = this.swingAmount * (Math.max(0.2, 1 - this.tempo / 300));  
let swingOffset = (this.currentSubdivision % 2 === 0) ? swingFactor * (60.0 / this.tempo / this.subdivision) : 0;
    this.nextNoteTime += (60.0 / this.tempo) / this.subdivision + swingOffset;
  }
