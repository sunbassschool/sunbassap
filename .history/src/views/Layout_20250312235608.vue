...
import { isUserLoggedIn, isUserAdmin, logoutUser } from "@/utils/api.ts"; // On récupère l'état de connexion
...
export default {
  ...
  setup() {
    ...
    const logout = async () => {
    try {
      await logoutUser(); // 🔥 Appelle la fonction API pour déconnecter l'utilisateur
      location.reload();  // 🔄 Recharge la page après la déconnexion
    } catch (error) {
      console.error("❌ Erreur lors de la déconnexion :", error);
    }
  };
    // ✅ Gardé car utilisé dans le template
    const isLoggedIn = ref(isUserLoggedIn()); // 🔥 Appel direct à api.ts
    const isAdmin = ref(isUserAdmin()); // 🔥 Appel direct à api.ts
    ...
    onMounted(() => {
      ...
      // ✅ Mise à jour de l'état de connexion au montage (si API mise à jour entre-temps)
    ...
    return {
      isLoggedIn, // ✅ Utilisé pour afficher certains éléments
