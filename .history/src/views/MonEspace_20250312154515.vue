      ...
      <div class="row justify-content-center">
        <div class="col-lg-12 col-md-10 w-100">
          <div class="card glass-card p-4 text-center animated-card">
            <!-- Loader global (chargement initial de l'user) -->
            ...
            <!-- Contenu affiché une fois l'user chargé -->
            <div v-else class="info-section">
              <!-- Bloc Objectif -->
              <div class="info-box goal-box text-center" :class="{ 'shake': isSaving, 'pulse': savedAnimation, 'flash': savedAnimation }">
                <input v-if="isEditing" v-model="user.objectif" class="form-control form-control-sm w-100"
                  @keyup.enter="debouncedUpdateObjectif" @blur="updateObjectif" />
                ...
                <!-- Loader interne optionnel (si besoin pendant l'édition) -->
                <div v-if="loadingResource" class="loading-container">
                  <div class="spinner-border text-primary" role="status">
                    <span class="visually-hidden">Chargement...</span>
                  </div>
                  <p class="mt-2 text-muted">Chargement des ressources...</p>
                </div>
              </div>
              ...
              <div class="info-box resource-box">
                ...
                <!-- Loader pour ressources si aucune data dans le cache -->
                <div v-if="!isDataReady" class="loading-bar-container">
                ...
                <div v-else>
                  <ul class="list-group list-unstyled">
                    ...
                    <li class="resource-item">
                      ...
                      <template v-else>
                        <a href="https://www.sunbassschool.com/step/inscription-aux-cours-en-visio/#checkout"
              ...
              <div class="info-box profile-box">
                <button class="btn btn-primary w-100 d-flex align-items-center justify-content-center py-3 fw-bold"
                  ...
                  <ul class="list-unstyled">
                    ...
                    <li class="py-2">
                      ...
                      <span v-if="isEditingInfo !== 'telephone'" @click="editField('telephone')" class="editable text-primary">
                        {{ user.telephone }} <i class="bi bi-pencil ms-2 text-muted"></i>
                      </span>
                      <input v-else v-model="user.telephone" type="text"
                             class="form-control form-control-sm d-inline-block w-auto"
                             @blur="updateInfosPerso" @keyup.enter="updateInfosPerso" ref="telephoneInput">
                    </li>
                    <li class="py-2">
                      <strong>Objectif : </strong>
                      <span v-if="isEditingInfo !== 'objectif'" @click="editField('objectif')" class="editable text-primary">
                        {{ user.objectif }} <i class="bi bi-pencil ms-2 text-muted"></i>
                      </span>
                      <input v-show="isEditingInfo === 'objectif'" ref="objectifInput"
                             v-model="user.objectif" class="form-control form-control-sm input-objectif"
                             @keyup.enter="updateObjectif" @blur="updateUserData">
                    </li>
...
mounted() {
  ...
  if (!cacheWasUsed) {
    console.log("⚠️ Pas de cache, affichage du spinner...");
    this.loading = true;  // ✅ On active `loading` uniquement si pas de cache
    this.isUpdating = true; // ✅ Afficher la barre pendant la mise à jour
  } else {
    console.log("✅ Données récupérées via le cache, affichage immédiat !");
    this.loading = false; // ❌ Éviter de remettre `loading = true` après
    this.isDataReady = true; // ✅ Activer l'affichage
  }

  // 🔄 Récupération des données API pour mise à jour du cache
  setTimeout(async () => {
    await this.fetchUserData(true);
    // ❌ Supprime `this.loading = false;` ici car il doit être contrôlé uniquement en fonction du cache
  }, 500);
...
async fetchUserData(forceUpdate = false) {
    ...
    if (!navigator.onLine) {
      console.warn("⚠️ Mode hors ligne, chargement depuis le cache...");
      this.loading = false;
      return;
    }
    try {
  ...
  async initializeUser() {
    ...
    console.log("✅ Utilisateur détecté :", prenom);
    const cacheFound = await this.loadUserData();
    if (!cacheFound) {
      this.loading = true;
    }
    setTimeout(async () => {
...
async loadUserData() {
  ...
  if (cachedData) {
    ...
    this.isDataReady = true; // Cache vérifié
    return true;
  }
