...
,
  ...
  await this.requestWakeLock(); // 🆕 Lancer une première fois au démarrage
   // ✅ 🔁 Synchro avec le mini-player
   this.$watch('tempo', val => metronomeStatus.tempo.value = val)
  this.$watch('currentBeat', val => metronomeStatus.beat.value = val)
  this.$watch('isBeating', val => metronomeStatus.isBeating.value = val)
  this.$watch('isPlaying', val => metronomeStatus.isPlaying.value = val)
},
beforeUnmount() {
  this.stopMetronome();
