...
const prenom = ref(localStorage.getItem("prenom") || "");
  ...
  // 1️⃣ 🔹 Charger les données depuis le cache si elles existent
  ...
  // 2️⃣ 🔧 Correction ici : tous les paramètres passent avec `&` (car API_URL contient déjà `?url=...exec`)
  const fullURL = `${API_URL}&route=planning&email=${encodeURIComponent(email.value)}&prenom=${encodeURIComponent(prenom.value)}`;
