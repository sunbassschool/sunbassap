...
export default {
  ...
  methods: {
    ...
    detectFrequency() {
      ...
      // Cas spécial pour **55 Hz** (La) détecté comme **E** : 
      // Si la fréquence de **55 Hz** est captée pendant que la corde E est jouée, on la ramène à E (41.20 Hz).
      if (Math.abs(detectedFreq - 55.00) <= tolerance && this.currentNote === "E") {
        detectedFreq = 41.20; // Ramener cette fréquence à la fondamentale de E
        closestNote = { name: "E", frequency: 41.20 };
      }

      // Cas spécial pour **118.43 Hz** (proche de **E**)
