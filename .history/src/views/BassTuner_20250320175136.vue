    ...
    methods: {
      async startTuner() {
        try {
          ...
          // 🎛️ Ajout d'un filtre passe-bas ajusté à 150 Hz
          const biquadFilter = this.audioContext.createBiquadFilter();
          biquadFilter.type = "lowpass";
          biquadFilter.frequency.value = 150; // Réduction du seuil pour éviter les harmoniques trop élevées
      ...
      detectFrequency() {
        ...
        // 🛠️ Réduction des harmoniques
        while (detectedFreq > 200) {
          detectedFreq /= 2;
        }
  
        const tolerance = 3; // 🔧 Tolérance réduite pour éviter les confusions
        let closestNote = null;
  
        // 🎯 Vérification des fréquences détectées
        for (let note of this.notes) {
          ...
          // 🌀 Correction des harmoniques multiples (x2, x3)
        ...
        // 🎵 Cas spécifique pour éviter la confusion entre E et A
        if (Math.abs(detectedFreq - 55.00) <= tolerance && (this.currentNote === "E" || this.currentNote === "D")) {
          detectedFreq = 41.20;
          closestNote = { name: "E", frequency: 41.20 };
        }
  
        // 🔄 Appliquer une moyenne mobile pour stabiliser la fréquence
        const smoothingFactor = 0.8;
        this.frequency = this.frequency * smoothingFactor + detectedFreq * (1 - smoothingFactor);
        ...
        // ⏳ Limiter la fréquence de mise à jour pour éviter le clignotement
      ...
      checkTuning(referenceFreq) {
        const tolerance = 1.5; // 🎯 Tolérance fine pour accorder
