...
export default {
  ...
  return {
    tempo: 120,
    swingStyle: 50, // Valeur par défaut (Funk)
...
{ value: 4, label: '', icon: './assets/icons/sixteenth-note.png' }
  ...
  computed: {
    ...
    swingFactor() {
    return Number(this.swingStyle) / 100; // Convertit en facteur de 0.5 à 0.75
  },
  },
  ...
  while (this.nextNoteTime < now + 0.1) {
    ...
    let swingOffset = (this.currentSubdivision % 2 === 0) ? 
                  -Math.min(0.08, this.swingFactor * (60.0 / this.tempo / this.subdivision)) : 0;
