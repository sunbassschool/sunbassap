<template>
  ...
  // Vérification de la fréquence pour les notes de basse :
  // Mi (E) : 41.2 Hz, La (A) : 55 Hz, Ré (D) : 73.42 Hz, Sol (G) : 98 Hz

  // Tolérance autour des fréquences fondamentales
  const tolerance = 2;

  // Liste des notes et de leurs fréquences
  const notes = [
  ...
  for (let note of notes) {
    ...
    // Si la fréquence est une harmonique, on la ramène à la fondamentale
    if (Math.abs(detectedFreq - note.frequency * 2) <= tolerance) {
      detectedFreq = note.frequency; // Divise la fréquence par 2 pour revenir à la fondamentale
      closestNote = note;
      break;
    }
  }
