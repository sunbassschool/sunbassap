...
<div class="mt-2">
  ...
  <div class="d-flex justify-content-end">
   ...
   <!-- ✅ Checkbox "Marquer comme lu" si pas encore validé -->
...
sendingFeedback: false,
        ...
        feedbackError: null,
        routes: {
          GET: "AKfycbw765DXQDRzbdsiVwrGwykSyNuFo9ehKroEKkRusBA6hzBPbcqEULJDwfB2b6bE52SC3Q/exec",
          POST: "AKfycbw765DXQDRzbdsiVwrGwykSyNuFo9ehKroEKkRusBA6hzBPbcqEULJDwfB2b6bE52SC3Q/exec"
        },
...
async fetchFeedbacks() {
  ...
  try {
    ...
    // Sépare les feedbacks parents et les réponses
    const parents = all.filter(f => !f.ID_Parent || f.ID_Parent === "");
    const replies = all.filter(f => f.ID_Parent && f.ID_Parent !== "");

    // Associe les réponses à leur parent
    parents.forEach(parent => {
      parent.reponses = replies.filter(r => r.ID_Parent === parent.ID);
    });

    this.feedbacks = parents;

  } catch (err) {
    console.error("❌ Erreur fetchFeedbacks :", err);
    this.feedbackError = "Impossible de charger les feedbacks.";
  } finally {
