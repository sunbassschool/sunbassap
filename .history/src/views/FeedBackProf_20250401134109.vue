...
export default {
  ...
  // Ici, tu utilises les données de l'admin pour envoyer la réponse
  const idEleve = this.userData?.id || this.email; // Si pas d'idEleve, fallback sur email de l'admin
  const prenom = this.userData?.prenom || "Admin";  // Utilise "Admin" comme fallback pour l'admin

  // Si idEleve ou prenom sont toujours undefined, on ne peut pas envoyer la requête
  if (!idEleve || !prenom) {
    console.warn("❌ id_eleve ou prenom manquants pour l'admin");
  ...
  const payload = {
    ...
    contenu: texte,
    id_eleve: idEleve,  // L'ID de l'élève ou celui de l'admin (au besoin)
    prenom: prenom     // Le prénom de l'admin ou autre valeur par défaut
  };
