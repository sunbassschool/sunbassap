...
import { getValidToken, refreshToken, getRefreshTokenFromDB, isJwtExpired, preventIndexedDBCleanup } from "@/utils/api";
...
// Ajouter un flag pour vérifier si le composant est toujours monté
...
onMounted(async () => {
  ...
  try {
    ...
    const validToken = await withTimeout(getValidToken(), 5000);  // Timeout pour éviter les blocages

    // Mode hors ligne - Vérification du cache
    const cachedUserData = await getCache("userData_sunny");
    ...
    // Si un token valide est trouvé, on redirige vers l'espace utilisateur
    if (validToken && isMounted.value) {
      console.log("✅ JWT valide trouvé, connexion...");
      sessionStorage.setItem("jwt", validToken);  // Mettre à jour le JWT dans sessionStorage
      return router.push("/mon-espace");
    }

    // Si aucun JWT valide n'est trouvé ou si le refresh échoue, afficher le bouton Commencer
    if (isMounted.value) {
      console.warn("⚠️ JWT absent ou expiré, affichage du bouton Commencer.");
...
// Assurez-vous de nettoyer le flag lorsque le composant est démonté
onBeforeUnmount(() => {
