...
async fetchFeedbacks() {
  ...
  // 🔍 Un feedback principal : ID_Cours ne commence PAS par "ID"
  const principaux = all.filter(fb => !fb.ID_Cours?.startsWith("ID"));

  // 💬 Ensuite, rattacher toutes les réponses (directes) à chaque principal
  this.feedbacks = principaux.reverse().map(parent => {
    const directReplies = all.filter(rep => rep.ID_Cours === parent.ID);

    return {
      ...parent,
      reponses: directReplies
    };
      });
