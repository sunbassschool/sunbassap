...
stopMetronome() {
  if (!this.isPlaying) return; // Si le métronome est déjà arrêté, on ne fait rien
  
  // Mettre à jour l'état du métronome
  this.isPlaying = false;
  localStorage.setItem("isPlaying", "false");

  // Arrêter les animations de son (requestAnimationFrame) et le timer
  cancelAnimationFrame(this.interval); // Annule la boucle d'animation
  ...
  // Arrêter tous les sons en cours (on arrête toutes les sources audio en cours)
  if (this.audioContext) {
    // Assure-toi de stopper toute activité audio en cours
    this.audioContext.close().then(() => {
      console.log("🔇 AudioContext fermé et tout arrêté !");
    }).catch(err => {
      console.error("❌ Impossible de fermer AudioContext : ", err);
    });
  }
}
