...
async fetchNote(forceRefresh = false) {
  ...
  // ✅ Vérification du JWT via helper global
  if (!await checkAuth(false)) {
    console.warn("❌ JWT invalide. Annulation de la récupération de note.");
    this.saving = false;
  ...
  if (!forceRefresh && cachedData?.note !== undefined) {
  ...
  // ⏳ Message si ça dure
  const slowFetchWarning = setTimeout(() => {
    if (this.isRefreshingNote) {
      this.noteUpdatedMessage = "⏳ Chargement en cours, c’est un peu long...";
    }
  }, 5000);
  ...
  const hardTimeout = setTimeout(() => {
    ...
    this.isRefreshingNote = false;
    this.saving = false;
    this.noteUpdatedMessage = "❌ Temps dépassé. Réessaie plus tard.";
    console.warn("⏰ Timeout dépassé pour fetchNote");
  }, 15000);

  try {
    const jwt = sessionStorage.getItem("jwt") || localStorage.getItem("jwt");
    if (!jwt || timeoutReached) return;
    ...
    if (data.note !== undefined) {
      ...
      this.noteUpdatedMessage = "⚠️ Aucune note reçue.";
    }
    ...
    if (!timeoutReached) {
      this.noteUpdatedMessage = "❌ Une erreur est survenue.";
    }
  } finally {
    clearTimeout(slowFetchWarning);
...
async fetchFromAPI(forceRefresh = false) {
  // ✅ Pas de mise à jour si le cache est encore bon
  ...
  if (!isAuth) {
    console.warn("❌ Auth invalide, annulation de l'appel API.");
    return;
  }

  let cachedData = JSON.parse(localStorage.getItem(this.cacheKey)) || {};

  // 🔄 Vérifie email & prénom (post-auth)
  ...
  if (!this.email || !this.prenom) {
    console.error("❌ Impossible de récupérer email/prénom");
    return;
  }
  ...
  try {
    ...
    if (!jwt) {
      console.error("❌ Aucun JWT trouvé après checkAuth !");
      return;
    }
    ...
    if (!data || typeof data !== "object" || !data.success) {
      ...
      this.displayError(); // ➕ Ajoute cette ligne
      return;
    }
