...
import { refreshToken, getToken, getRefreshTokenFromDB, isJwtExpired, verifyIndexedDBSetup, preventIndexedDBCleanup, checkAndRestoreTokens } from "@/utils/api.ts";
...
onMounted(async () => {
  ...
  try {
    ...
    // ✅ Vérifie si IndexedDB a été vidée et restaure les tokens si nécessaire
    await checkAndRestoreTokens();

    // ✅ Stockage de secours : Si IndexedDB a été effacée, on restaure depuis LocalStorage
    let refreshTokenStored = localStorage.getItem("refreshToken");
    if (!refreshTokenStored) {
      console.warn("⚠️ Aucun refresh token dans LocalStorage, risque de déconnexion.");
    } else {
      console.log("📦 Refresh token récupéré depuis LocalStorage pour restauration !");
      await updateRefreshTokenInDB(refreshTokenStored);
    }
    ...
    // ✅ Rafraîchissement périodique du JWT toutes les 10 minutes
    setInterval(async () => {
      console.log("🔄 Vérification périodique du JWT...");
      let jwt = await getToken();
      if (!jwt || isJwtExpired(jwt)) {
