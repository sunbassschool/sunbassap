...
{ value: 4, label: '', icon: './assets/icons/sixteenth-note.png' }
  ...
  computed: {
    ...
    swingFactor() {
      swingFactor() {
    return Number(this.swingStyle) / 100; 
}
  ...
  while (this.nextNoteTime < now + 0.1) {
    ...
    let humanizeFactor = (Math.random() - 0.5) * 0.005; // Variation aléatoire légère
let swingOffset = (this.currentSubdivision % 2 === 0) ? 
                  this.swingFactor * (60.0 / this.tempo / this.subdivision) + humanizeFactor : 
                  humanizeFactor;  this.nextNoteTime += (60.0 / this.tempo) / this.subdivision + swingOffset;
  }
