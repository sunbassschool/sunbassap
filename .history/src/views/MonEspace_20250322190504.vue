...
fetchURL(route) {
  ...
  mounted() {
  console.log("🔒 Auth → Vérification de l'utilisateur via le store...");

  const auth = useAuthStore();

  auth.loadUser().then(async () => {
    ...
    // ✅ L'utilisateur est authentifié, on peut charger les données
    const cacheUsed = await this.loadUserData();

    this.loadUserInfos(); // 🔄 Récupère les infos complémentaires
    ...
    this.checkUserRole(); // 🔍 Détermine le rôle
    this.isInitialized = true;
    document.addEventListener("click", this.handleClickOutside);
  }).catch(err => {
    console.error("❌ Auth échouée :", err);
    this.$router.push("/login");
  });
}
...
async checkUserRole() {
  ...
  if (!this.user.role) {
    ...
    // 🧠 Essaye de récupérer le prénom depuis IndexedDB
    ...
    if (prenomInIndexedDB) {
      ...
      // 🔐 Sinon, tente de récupérer un JWT valide
      const jwt = await getValidToken();

      if (!jwt || typeof jwt !== 'string') {
        console.warn("❌ JWT invalide ou manquant :", jwt);
        return; // On ne peut pas continuer sans JWT valide
      }
      ...
      try {
        ...
        if (decoded?.role) {
          ...
          this.user.prenom = decoded.prenom;

          // 💾 Stockage pour usages ultérieurs
          await saveToIndexedDB('prenom', this.user.prenom);
          localStorage.setItem("prenom", this.user.prenom);
          console.log("🔐 Rôle et prénom récupérés via JWT :", decoded.role, decoded.prenom);
        } else {
          console.warn("⚠️ Le JWT ne contient pas les infos attendues :", decoded);
        }
      } catch (error) {
    ...
    // Sauvegarde du rôle localement si récupéré
    if (this.user.role) {
...
,
  ...
  // 🔐 Récupère dynamiquement le token valide
  const jwt = await getValidToken();

if (!jwt || typeof jwt !== 'string') {
  console.warn("❌ Token JWT invalide :", jwt);
  return;
}

const url = this.fetchURL("POST") + `?route=updateeleve&jwt=${encodeURIComponent(jwt)}&objectif=${encodeURIComponent(nouvelObjectif)}`;
  ...
  try {
    ...
    if (data.status !== "error") {
      console.log("✅ Objectif sauvegardé avec succès !");
      this.savedAnimation = true;
      setTimeout(() => (this.savedAnimation = false), 1000);
