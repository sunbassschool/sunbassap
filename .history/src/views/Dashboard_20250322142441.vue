...
async mounted() {
  ...
  // 🚫 Ne fais rien de plus si l'utilisateur n'est pas connecté
  if (!this.isLoggedIn) {
    console.warn("👤 Utilisateur non connecté – chargement limité");
  ...
  // 📝 Récupération de la note (uniquement si connecté – déjà checké)
  setTimeout(async () => {
    await this.fetchNote();
  }, 300);

  // 🔄 Synchronisation cross-tab JWT
