...
async mounted() {
  ...
  this.prepareFallbackCards(); // 🔥 Crée cette méthode qui affiche les composants "Connecte-toi"
...
async fetchFromAPI(forceRefresh = false) {
  ...
  try {
    ...
    if (!data || typeof data !== "object" || data.status !== "success") {
      console.warn("⚠️ Problème API :", data.message || "Réponse inattendue");
      this.displayError(); // pour montrer des cartes de fallback
this.isLoading = false;
      return;
    }
...
,
 ...
 // 📌 Mise à jour des cartes (exemple pour Dashboard)
if (!updatedData.planning || !Array.isArray(updatedData.planning)) {
  this.cards = [
    {
      icon: "bi bi-calendar-event",
      title: "Prochain Cours",
...
,
    ...
    displayError() {
      this.cards = [
        {
          icon: "bi bi-calendar-event",
          title: "Bienvenue sur l'application SunBassApp",
          text: "Tu n'as pas encore de cours de prévu."
        },
        {
          icon: "bi bi-flag",
          title: "Objectif actuel",
          text: "Nous n'avons pas encore eu l'occasion de déterminer ton objectif."
        }
      ];
    },
