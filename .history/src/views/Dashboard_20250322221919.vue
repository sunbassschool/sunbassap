...
async mounted() {
  const auth = useAuthStore();
  await auth.loadUser(); // Assure que tout est bien chargé
  console.log("📌 Chargement du tableau de bord...");

  // 🔄 Écouteur : changement de JWT dans un autre onglet
  window.addEventListener("storage", async (event) => {
  if (event.key === "jwt") {
    console.log("🔄 JWT mis à jour, récupération de la note et des données...");
    
    if (this && this.fetchNote) await this.fetchNote();
    if (this && this.fetchFromAPI) await this.fetchFromAPI(true); // ✅ Ajout ici !
  }
});


  // 🔒 Si utilisateur non connecté → affichage simplifié
  if (!this.isLoggedIn) {
    
    console.warn("👤 Utilisateur non connecté – affichage partiel sans API");

    // 💡 Affichage de cartes placeholder
    this.cards = [
      {
...
// ✅ S'assure que prénom et email sont disponibles avant les appels API
// 🔐 Force la récupération depuis JWT si localStorage est vide
if (!this.email || !this.prenom) {
  ...
  // 💾 Re-sauvegarde dans localStorage au passage
  if (this.email) localStorage.setItem("email", this.email);
  if (this.prenom) localStorage.setItem("prenom", this.prenom);
}
...
,
  ...
  const updatedData = { ...data }; // Copie simple, car `data` est déjà l'objet complet

  // 🔐 Prépare les clés cache
  const userInfosKey = `userInfos_${this.prenom}`;
  let cachedInfos = JSON.parse(localStorage.getItem(userInfosKey) || "{}");

  // ✅ Stocke les infos spéciales séparément
  ...
  console.log("💾 Infos utilisateur enregistrées :", cachedInfos);

  // 🔍 Objectif final (priorité API > infos séparées > fallback)
  const objectifFinal = 
    updatedData.objectif || 
    cachedInfos.objectif || 
    "🎯 Aucun objectif défini";

  // 🗓️ Cours : vérifie si planning existe
  const planning = Array.isArray(updatedData.planning) ? updatedData.planning : [];
  const now = new Date();
  const prochainCours = planning.find(c => new Date(c.date) > now);
  ...
  // ✅ Sinon, affichage complet avec lien Meet
