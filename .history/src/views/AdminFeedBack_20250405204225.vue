...
<div v-if="getUnreadFeedbacks().length" class="unread-topics mb-4">
  ...
  <div class="d-flex justify-content-between align-items-center mb-1">
    <div class="d-inline-block text-truncate" style="max-width: 70%;">
      <strong class="text-info">{{ fb.Prenom || 'Élève inconnu' }}</strong>
      <span class="text-light small fst-italic ms-2">
        🔎 {{ fb.Contenu.replace(/<[^>]*>/g, '').slice(0, 45) }}...
      </span>
    </div>
    <small class="text-muted text-nowrap ms-2">{{ formatDate(fb.Date_Publication) }}</small>
  </div>

        <!-- 💬 Contenu complet (toggle ON) -->
        <div
