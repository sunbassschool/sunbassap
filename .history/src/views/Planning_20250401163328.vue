...
export default {
  ...
  setup() {
    ...
    const API_URL =
      "https://cors-proxy-sbs.vercel.app/api/proxy?url=https://script.google.com/macros/s/AKfycbwCrvZUTP9W0dGCzgMO_wdfgQWXeke3xAWLiXIIR8TdT57IWE3V90xj_E2JZOxrtx4n2A/exec";
    ...
    // ✅ Filtrer les cours à venir seulement
    const upcomingCourses = computed(() => {
      const now = new Date();
      return planningData.value.filter((course) => {
        const courseDate = new Date(course.date || course.formattedDate); // selon le format que tu reçois
