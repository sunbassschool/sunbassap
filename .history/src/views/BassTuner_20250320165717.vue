<template>
  ...
  // Limite la fréquence à une plage basse pour éviter les harmoniques trop élevées
  while (detectedFreq > 200) {
    detectedFreq /= 2; // Divise jusqu'à obtenir une fréquence plausible
  }

  // Vérification pour Mi (E), on doit s'assurer de capturer la fondamentale
  if (detectedFreq > 60 && detectedFreq < 80) {
    // Si on détecte une fréquence dans la plage de la première harmonique de Mi (E)
    detectedFreq /= 2;
  }

  // Vérification pour Ré (D), on doit s'assurer de capturer la fondamentale
  if (detectedFreq > 120 && detectedFreq < 150) {
    // Réduit pour ramener à la fondamentale de Ré (D)
    detectedFreq /= 2;
  }
