...
.navbar-nav .nav-link {
  font-weight: 600 !important; /* 🔥 Garde le même poids toujours */
  font-size: 12px; /* ✅ Taille fixe pour éviter les micro-ajustements */
...
@media screen and (max-width: 1024px) {
  ...
  /* ✅ Ajuste l’espacement des éléments */
  .navbar-nav {
    gap: px;
  }
/* ✅ Fixer la largeur et la hauteur des icônes */
.navbar-nav .nav-link i {
  width: 30px; /* 🔥 Taille fixe pour éviter tout décalage */
  height: 30px;
  font-size: 28px; /* ✅ Taille standardisée */
  display: flex;
  align-items: center;
  justify-content: center;
  transition: none !important; /* 🔥 Désactive toute animation involontaire */
  position: relative; /* ✅ Évite le repositionnement dynamique */
}

/* ✅ Fixer la position et empêcher les micro-décalages */
.navbar-nav .nav-link {
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  min-width: 60px; /* 🔥 Assure un espace constant */
  min-height: 60px; /* ✅ Évite les petits déplacements verticaux */
  font-size: 12px;
  font-weight: 600 !important; /* ✅ Éviter le "saut" lié au changement de poids */
  transition: background 0.2s ease-in-out, color 0.2s ease-in-out;
}

/* ✅ Supprimer l'effet de micro-zoom */
.navbar-nav .nav-link:hover,
.navbar-nav .nav-link.router-link-exact-active {
  transform: none !important; /* 🔥 Empêche tout redimensionnement */
  text-shadow: none !important; /* 🔥 Supprime les effets de "tremblement" */
}

/* ✅ Empêcher les icônes de bouger lors des interactions */
.navbar-nav .nav-link i:before {
  position: absolute;
}

/* ✅ Assurer un espacement constant */
.navbar-nav {
  display: flex;
