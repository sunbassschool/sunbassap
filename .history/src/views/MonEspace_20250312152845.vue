...
<li class="resource-item">
  <template v-if="!isDataReady || isUpdating">
    <div class="loading-bar-container">
      <div class="loading-bar"></div> <!-- 🔄 Barre animée -->
    </div>
  </template>
...
<!-- 🎵 Playlist YouTube -->
<li class="resource-item">
  <!-- ✅ Affichage du loader si les données ne sont PAS encore prêtes OU si une mise à jour est en cours -->
  <template v-if="!isDataReady || isUpdating">
    <div class="loading-bar-container">
      <div class="loading-bar"></div> <!-- 🔄 Barre animée -->
    </div>
  ...
  <!-- ✅ Si la playlist YouTube existe, on affiche le lien -->
  ...
  <!-- ❌ Sinon, on affiche le message d'accès réservé -->
  <template v-else>
    <a 
      href="https://www.sunbassschool.com/step/inscription-aux-cours-en-visio/#checkout" 
      class="text-muted text-decoration-none"
...
async fetchUserData(forceUpdate = false) {
  ...
  this.isUpdating = true; // 🔥 Active la barre de chargement
  ...
  try {
    ...
    if (!data.error) {
      ...
      const oldCache = JSON.parse(localStorage.getItem(this.cacheKey)) || {};

      // 📌 Vérifie si quelque chose a changé
      const hasChanged = Object.keys(data).some(key => data[key] !== oldCache[key]);

      if (!hasChanged) {
        console.log("✅ Rien n'a changé, pas de mise à jour du cache !");
        this.isUpdating = false; // 🔥 On s'assure que la barre disparaisse
        return; // 🚀 Sortir immédiatement pour éviter toute mise à jour inutile
      }

      console.log("💾 🔄 Mise à jour du cache !");
      localStorage.setItem(this.cacheKey, JSON.stringify(data)); // ✅ Mettre à jour uniquement si besoin
      this.user = data; // ✅ Met à jour l'affichage

      this.isDataReady = true; // ✅ Données prêtes
    } else {
    ...
    this.isUpdating = false; // 🔥 Assure que la barre est bien désactivée après la requête
    this.loading = false; // 🚀 Désactive le spinner principal
  }
}
