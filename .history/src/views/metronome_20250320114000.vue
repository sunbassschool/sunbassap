...
async mounted() {
  ...
  // 🔥 Lire l'état du métronome depuis sessionStorage
  const wasPlaying = sessionStorage.getItem("isPlaying") === "true";

  if (wasPlaying) {
    this.isPlaying = true; // 🔥 Forcer l'affichage du bouton "Pause"
  } else {
    this.isPlaying = false; // 🔥 Forcer l'affichage du bouton "Play"
  }
