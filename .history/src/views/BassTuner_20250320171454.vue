<template>
  ...
  // Diviser systématiquement la fréquence par deux jusqu'à ce qu'elle corresponde à une fondamentale
  const notes = [
  ...
  while (detectedFreq > 10) { // On arrête de diviser si la fréquence devient trop basse
    for (let note of notes) {
      ...
      if (note.name === 'D' && Math.abs(detectedFreq - note.frequency * 2) <= 2) {
