...
<!-- 🎵 Playlist YouTube -->
<li class="resource-item">
  <!-- Affiche le loader uniquement si le cache ne contient pas de lien -->
  <template v-if="!user.playlist_youtube && isUpdating">
    <div class="loading-bar-container">
      <div class="loading-bar"></div>
    </div>
  </template>
  
  <!-- Si le lien existe, affiche la playlist -->
  ...
  <!-- Sinon, affiche l'accès réservé -->
  <template v-else>
    <a href="https://www.sunbassschool.com/step/inscription-aux-cours-en-visio/#checkout" class="text-muted text-decoration-none" target="_blank">
      🎵 Ma playlist YouTube <strong>(accès réservé)</strong>.<br>
      <span class="btn btn-primary btn-sm mt-2">🔗 S'inscrire maintenant</span>
    </a>
  </template>
</li>
