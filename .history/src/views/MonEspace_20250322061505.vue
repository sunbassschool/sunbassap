...
async fetchUserData(forceUpdate = false) {
  const { jwt, prenom } = await checkAuth(); // 🔐 Assure l'auth validée

  if (!prenom || !jwt) {
    console.warn("⚠️ Authentification invalide. Impossible de récupérer les données.");
    this.error = "Utilisateur non authentifié.";
    return;
  }

  const url = this.fetchURL("GET") + `?route=recupInfosMembres&jwt=${jwt}&prenom=${prenom}`;
  // ... suite du code comme tu l'as déjà
}
