...
async fetchFeedbacks() {
  ...
  try {
    ...
    if (result.feedbacks) {
      ...
      // Filtrer les feedbacks principaux (sans lien)
      const principaux = all.filter(fb => !fb.ID_Feedback_Relé || fb.ID_Feedback_Relé === "");

      // Rattacher les réponses aux feedbacks principaux
      principaux.forEach(principal => {
        const idNum = Number(principal.ID?.toString().replace("ID", ""));

        principal.reponses = all.filter(rep => {
          return Number(rep.ID_Feedback_Relé) === idNum;
        });
      });

      this.feedbacks = principaux.reverse(); // Afficher les plus récents en haut
    } else {
