...
stopMetronome() {
  ...
  if (this.isPlaying) {
    ...
    localStorage.setItem("isPlaying", "false"); // Sauvegarder l'état dans localStorage

    // Annuler le timing des beats
    ...
    // Si un son est en train de jouer, le couper
    if (this.audioContext) {
      const gainNode = this.audioContext.createGain();
      gainNode.gain.value = 0; // Couper le volume
      gainNode.connect(this.audioContext.destination);
    }

    // Si tu as des sources audio actives, tu dois les arrêter
    ...
    // Réinitialiser la source sonore
