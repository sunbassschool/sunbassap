<template>
  ...
  // Vérification pour Mi (E) : plage autour de 41.2 Hz (fondamentale)
  if (detectedFreq > 40 && detectedFreq < 50) {
    // Si la fréquence est proche de Mi (E), on fixe à 41.2 Hz
    if (Math.abs(detectedFreq - 41.2) < 2) {
      detectedFreq = 41.2;  // Fixe la fréquence à 41.2 Hz pour Mi
    }
    // Si la fréquence est proche de l'harmonique de Mi (E) à 82 Hz, la ramène à la fondamentale
    else if (Math.abs(detectedFreq - 82.4) < 2) {
      detectedFreq = 41.2;  // Fixe la fréquence à 41.2 Hz (deuxième harmonique de Mi)
    }
  }

  // Vérification pour Ré (D) : plage autour de 73.42 Hz (fondamentale)
  if (detectedFreq > 60 && detectedFreq < 80) {
    // Si la fréquence est proche de Ré (D), on évite de la diviser
    if (Math.abs(detectedFreq - 73.42) < 2) {
      detectedFreq = 73.42;  // Fixe la fréquence de Ré à 73.42 Hz
    }
    // Si la fréquence est plus élevée, divise pour revenir à la fondamentale
    else if (detectedFreq > 73) {
      detectedFreq /= 2;  // Divise si c'est une harmonique de Ré
    }
  }

  // Recherche de la note la plus proche parmi les notes définies
