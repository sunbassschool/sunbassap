...
async mounted() {
  try {
    const auth = useAuthStore();
    await auth.loadUser(); // ⚠️ Peut échouer sans JWT ni refresh

    const jwt = await getValidToken();
    if (!jwt) {
      console.warn("🚫 Pas de JWT valide → interface anonyme");
      
      // 🧼 On montre quand même les cartes de base
      this.cards = [
        {
      ...
      await this.fetchNote(); // Tu peux montrer la note s'il y en a
      this.isLoading = false;
      return; // ✅ Ne continue surtout pas après ça
    }
 ...
 if (!sessionStorage.getItem("apiSynced")) {
  const cacheWasUsed = await this.loadUserData();
  if (!cacheWasUsed || shouldUpdateCache(this.cacheKey, this.cacheDuration)) {
    await this.fetchFromAPI(true);
    sessionStorage.setItem("apiSynced", "true"); // ✅ Évite le double appel
  }
  this.isLoading = false; // ✅ important même après l'appel
} else {
  await this.loadUserData(); // 🔁 Recharge les données depuis cache
  this.isLoading = false; // ✅ à ne pas oublier ici aussi
}


    await this.fetchNote(); // ✅ Faut toujours la recharger
    this.isLoading = false;
