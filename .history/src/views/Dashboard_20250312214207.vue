...
,
  ...
  async mounted() {
    ...
    await this.loadDashboardData(); // ✅ Charge TOUT proprement
    ...
    // 🔥 Chargement de la note utilisateur si connecté
    setTimeout(async () => {
...
beforeUnmount() {
  ...
  insertEmoji(event) {
    ...
    await this.fetchUserData(); // 1️⃣ Récupérer email & prénom d'abord
    if (!this.email || !this.prenom) {
...
async fetchFromAPI(forceRefresh = false) {
    ...
    // 🔄 Vérification de l'email et du prénom via `fetchUserData()`
    if (!this.email || !this.prenom) {
        console.warn("⚠️ Email ou prénom manquant, récupération via fetchUserData...");
        await this.fetchUserData(); // ✅ On s'assure d'avoir les bonnes valeurs
