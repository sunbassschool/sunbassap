...
async loadUserData() {
  ...
  if (cachedData && cacheExpiration && Date.now() < parseInt(cacheExpiration)) {
    ...
    this.isDataReady = true;  // Activation des données prêtes
    this.loading = false;
    return true;  // Renvoie true si le cache est utilisé
  }

  console.warn("🚨 Cache expiré ou introuvable, affichage du loader...");
  return false;  // Si aucune donnée en cache ou si elle est expirée
}
,
