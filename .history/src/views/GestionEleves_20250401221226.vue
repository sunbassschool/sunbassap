<template>
  ...
  <button class="btn btn-sm btn-outline-light" @click="toggleExpand(eleve.email)">
    {{ expanded[eleve.email] ? '➖' : '➕' }}
  </button>
</div>
<div v-if="expanded[eleve.email]" class="mt-3">
...
champs: [
  ...
  this.error = "Format inattendu reçu depuis le serveur.";
      ...
      toggleExpand(email) {
  this.$set(this.expanded, email, !this.expanded[email]);
}
,
