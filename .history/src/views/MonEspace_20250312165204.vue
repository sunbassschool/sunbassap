...
async fetchUserData(forceUpdate = false) {
  try {
    ...
    if (!data.error) {
      ...
      this.isUpdating = false; // ✅ Désactive la mise à jour en cours
    }
  } catch (err) {
    this.error = "Impossible de récupérer les données.";
  } finally {
    this.loading = false; // ✅ Assure que le loader se désactive en cas d'erreur
    this.isUpdating = false; // ✅ Assure que la mise à jour se termine
  }
}
...
async loadUserData() {
  ...
  if (cachedData && cacheExpiration && Date.now() < parseInt(cacheExpiration)) {
    ...
    this.isDataReady = true; // ✅ Marque les données comme prêtes
    this.loading = false; // ✅ Désactive le loader
    this.isUpdating = false; // ✅ Fin de la mise à jour
    return true;
  }
