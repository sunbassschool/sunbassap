...
fetchURL(route) {
  ...
  // Vérifie le cache et met à jour l'état en conséquence
  ...
  if (cacheWasUsed) {
    ...
    console.log("⚠️ Pas de cache valide, affichage du loader...");
    this.loading = true;
    this.isUpdating = true; // Progress bar visible

    // Mise à jour des données en arrière-plan sans afficher le loader
    setTimeout(async () => {
      await this.fetchUserData(false); // Ne pas forcer la mise à jour si le cache existe
    }, 500);
  }
  ...
  // Ajout d'un écouteur global pour détecter les clics en dehors du champ objectif
