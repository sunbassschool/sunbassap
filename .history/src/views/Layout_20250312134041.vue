...
let refreshInterval; // ✅ Déclaration de l’intervalle de refresh

// 🔥 Variables réactives
const user = ref(null);
const isLoggedIn = ref(!!(sessionStorage.getItem("jwt") || localStorage.getItem("jwt")));
...
const checkLoginStatus = async () => {
  ...
  await restoreTokensToIndexedDB(); // 🔄 Restauration des tokens depuis IndexedDB

  let token = await getToken();
  let refreshToken = await getRefreshTokenFromDB();

  // ✅ Restaurer JWT & refreshToken dans sessionStorage si absent
  if (token && !sessionStorage.getItem("jwt")) {
    console.log("🔄 [FIX] Réécriture du JWT dans sessionStorage.");
    sessionStorage.setItem("jwt", token);
  }

  if (refreshToken && !sessionStorage.getItem("refresh_token")) {
    console.log("🔄 [FIX] Réécriture du Refresh Token dans sessionStorage.");
    sessionStorage.setItem("refresh_token", refreshToken);
  }

  nextTick(() => {
    if (!token && isLoggedIn.value) {
      console.log("❌ [ALERTE] Aucun token trouvé -> L'utilisateur sera déconnecté.");
    }
  });
...
// ✅ Rafraîchir le JWT immédiatement si nécessaire
...
// ✅ Vérification et restauration automatique des tokens au démarrage
onMounted(async () => {
  ...
  preventIndexedDBCleanup(); // 🔄 Empêche la suppression des tokens par le navigateur
  ...
  // 🔥 Ajout pour éviter les pertes de refreshToken sur Safari/iOS
  if (navigator.userAgent.includes("Safari") && !navigator.userAgent.includes("Chrome")) {
    console.warn("🚨 Safari détecté : sauvegarde forcée du refreshToken...");
  ...
  // ✅ Rafraîchissement automatique si le JWT est expiré
  if (token && isJwtExpired(token)) {
    console.warn("⏳ JWT expiré au chargement, tentative de refresh...");
    await refreshToken();
  }
...
// ✅ Écouteur pour les changements sur d’autres onglets
window.addEventListener("storage", async (event) => {
  console.log(`📢 Changement détecté sur ${event.key}:`, event.newValue);
  if (event.key === "jwt" || event.key === "refreshToken") {
    console.log("🔄 Changement détecté dans `localStorage` ! Vérification...");
...
// ✅ Suppression propre des événements et timers
onUnmounted(() => {
  console.log("🚨 Nettoyage au démontage : arrêt du refresh interval");
  clearTimeout(refreshInterval);
  window.removeEventListener("resize", () => (isMobile.value = window.innerWidth <= 1024));
...
// ✅ Export des variables et fonctions utilisées dans le template
defineExpose({
  user,
  isLoggedIn,
  isAdmin,
...
@media screen and (max-width: 1024px) {
  .menu-btn {
    ...
    color: rgb(172, 0, 0) !important; /* ✅ Couleur */
...
@media screen and (max-width: 1024px) {
  ...
  .menu-btn {
    ...
    color: rgb(172, 0, 0) !important;  /* ✅ Couleur forcée */
    background-color: transparent;
    border:none;
