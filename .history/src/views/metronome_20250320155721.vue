...
{ value: 4, label: '', icon: './assets/icons/sixteenth-note.png' }
  ...
  while (this.nextNoteTime < now + 0.1) {
    ...
    // 🔥 Appliquer le swing UNIQUEMENT sur les subdivisions
    let swingOffset = (this.currentSubdivision % 2 === 0) 
                      ? this.swingAmount * baseInterval 
                      : -this.swingAmount * baseInterval;

    // ✅ Ajoute le swing aux subdivisions mais PAS à la pulsation principale
    if (this.currentSubdivision === 1) {
      this.nextNoteTime += baseInterval; // Pas de swing sur la pulsation
    } else {
