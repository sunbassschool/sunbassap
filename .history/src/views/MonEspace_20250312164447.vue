...
fetchURL(route) {
  ...
  // 🔄 Mise à jour des données en arrière-plan sans loader si le cache existe
  setTimeout(async () => {
    const wasUpdated = await this.fetchUserData(false);
    if (!wasUpdated) {
      console.log("⏭️ Aucune mise à jour nécessaire, pas de loader.");
      return;
    }
    this.loading = false;
  }, 500);
...
async fetchUserData(forceUpdate = false) {
  ...
  if (cacheExpiration && Date.now() < parseInt(cacheExpiration) && !forceUpdate) {
    console.log("⏭️ Données encore valides, mise à jour API annulée.");
    return false;
  }

  this.isUpdating = true; // 🔥 Active l'indicateur de mise à jour SANS modifier `loading`
