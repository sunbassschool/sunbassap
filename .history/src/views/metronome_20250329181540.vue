        ...
        <div class="card-header">
          <div class="control-group">
            ...
            <input 
              v-else 
              type="number" 
              v-model.number="metronome.tempo" 
              @blur="isEditingTempo = false" 
              @keyup.enter="isEditingTempo = false"
              min="20" max="300"
              class="bpm-input"
            /> BPM
            <input type="range" v-model="metronome.volumeSub" min="0" max="1" step="0.01">

            <div class="tempo-control">
              <button @click="decreaseTempo" class="tempo-arrow">&#8592;</button>
              <input type="range" v-model="metronome.tempo" min="20" max="300" step="1" />
        ...
        <div class="card-body">
          <div class="control-container">
            ...
            <div class="slider-group">
              ...
              <input 
                type="range" 
                v-model="metronome.volumeStrong" 
...
const currentBeat = computed(() => metronome.currentBeat || 1)
...
const accentuateFirstBeat = computed({
  ...
  set: (val) => (metronome.disableStrongBeat = !val)
})

// ✅ Synchroniser la subdivision avec le store
const subdivision = computed({
  get: () => metronome.subdivision,
  set: (val) => metronome.setSubdivision(val), // Mettre à jour le store avec la nouvelle subdivision
})

watch(subdivision, (newValue) => {
  metronome.setSubdivision(newValue);  // Synchronisation avec le store
})
...
watch(swingAmount, (val) => {
  localStorage.setItem("swingAmount", val.toString())
  metronome.swingAmount = parseFloat(val)
})
