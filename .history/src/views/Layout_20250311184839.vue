...
const checkLoginStatus = async () => {
    ...
    // ✅ Si le JWT est trouvé ailleurs mais absent de localStorage → Le réécrire !
    if (token && !localStorage.getItem("jwt")) {
        console.log("🔄 [FIX] Réécriture du JWT dans localStorage.");
        localStorage.setItem("jwt", token);
    }

    // ✅ Si le refresh token est disponible mais absent de localStorage → Le réécrire !
    if (refreshToken && !localStorage.getItem("refresh_token")) {
        console.log("🔄 [FIX] Réécriture du Refresh Token dans localStorage.");
