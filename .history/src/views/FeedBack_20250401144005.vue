...
async fetchFeedbacks() {
  ...
  try {
    ...
    if (result.feedbacks) {
      ...
      // 🔍 Identifier les feedbacks principaux (ceux dont ID_Cours est vide ou null)
      const principaux = all.filter(fb => !fb.ID_Cours); // Les feedbacks principaux, sans ID_Cours

      // 💬 Associer chaque feedback principal à ses réponses directes (les réponses ont un ID_Cours qui correspond à l'ID du feedback)
      this.feedbacks = principaux.reverse().map(fb => ({
        ...fb,
        reponses: all.filter(rep => rep.ID_Cours === fb.ID) // Associer les réponses liées au feedback via ID_Cours
      }));

      console.log("Feedbacks avec leurs réponses associées :", this.feedbacks); // Vérification des données
    } else {
      this.feedbackError = "Aucun feedback trouvé.";
    }
  } catch (err) {
    console.error("❌ Erreur récupération feedbacks :", err);
    this.feedbackError = "Erreur de chargement des feedbacks.";
  } finally {
    this.feedbackLoading = false;
  }
}
