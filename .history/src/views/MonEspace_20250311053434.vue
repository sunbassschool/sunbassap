...
markVideoAsWatched() {
  sessionStorage.setItem("videoShown", "true"); // ✅ Dès que la vidéo commence, on empêche qu'elle se rejoue
    ...
    if (this.isEditing && this.$refs.objectifInput && !this.$refs.objectifInput.contains(event.target)) {
      console.log("📌 Clic en dehors du champ détecté, sauvegarde...");
      this.saveObjectif();
    }
  },
...
,
  ...
  // 🔍 Vérifie si l'utilisateur est offline
  if (!navigator.onLine) {
  ...
  try {
    ...
    if (!data.error) {
      if (JSON.stringify(this.user) !== JSON.stringify(data)) { // ✅ Vérifie si les données ont changé
        this.user = data;
        this.updateLocalCache(data);
      } else {
        console.log("⚡ Cache inchangé, pas de mise à jour nécessaire.");
      }
    }
  } catch (err) {
...
updateLocalCache(data) {
  ...
  return;
    ...
    const updatedData = { ...cachedData, ...data }; // Garde les anciennes données si elles ne sont pas dans `data`

    if (cachedData.note && !data.note) {
        updatedData.note = cachedData.note; // ✅ Garde l'ancienne note si elle n'est pas présente dans `data`
    }

    console.log("💾 Mise à jour du cache en conservant la note !");
