<template>
  ...
  <img :src="filefile['Lien direct']" :alt="file['Nom du fichier']" style="width: 100%; border-radius: 8px;" />
</div>
<a :href="file['URL directe']" target="_blank" class="btn btn-sm btn-outline-light mt-2">🔗 Voir</a>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  try {
    ...
    this.files = data.map(file => {
      const lien = file['Lien'] || file['URL'] || file['url'] || "";
      if (typeof lien === 'string' && lien.includes("drive.google.com")) {
        const match = lien.match(/\/d\/([^/]+)\//);
        if (match && match[1]) {
          file['URL directe'] = `https://drive.google.com/uc?export=view&id=${match[1]}`;
        } else {
          file['URL directe'] = lien; // fallback
        }
      } else {
        file['URL directe'] = lien; // pour tous les autres cas
      }
      return file;
    });
