...
const prenom = computed(() => userData.nom || ""); // ⚠️ "nom" au lieu de "prenom" si l'API stocke comme ça
...
console.log("📩 Prénom extrait :", prenom.value);
  ...
  if (!email.value || !prenom.value) {
    console.error("❌ Erreur : email ou prénom non défini !");
    return;
  }
  ...
  try {
    ...
    if (response.data.success && response.data.planning) {
      ...
      console.log("✅ planningData mis à jour :", planningData.value); // 🔹 Vérification
    } else {
      console.warn("⚠️ Réponse API invalide :", response.data);
    }
  } catch (error) {
