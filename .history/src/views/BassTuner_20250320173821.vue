...
export default {
  data() {
    return {
      ...
      notes: [
        { name: "E", frequency: 41.20, tolerance: 2 },
        { name: "A", frequency: 55.00, tolerance: 2 },
        { name: "D", frequency: 73.42, tolerance: 3 },
        { name: "G", frequency: 98.00, tolerance: 5 },
      ],
    };
  },
  ...
  methods: {
    ...
    detectFrequency() {
      ...
      for (let note of this.notes) {
        ...
        if (Math.abs(detectedFreq - note.frequency) <= note.tolerance) {
          closestNote = note;
          break;
        }

        // Si la fréquence détectée est proche de l'harmonique (2x la fondamentale), la ramener à la fondamentale
        if (Math.abs(detectedFreq - note.frequency * 2) <= note.tolerance) {
      ...
      if (closestNote) {
        ...
        if (Math.abs(detectedFreq - this.lastFrequency) > closestNote.tolerance) {
