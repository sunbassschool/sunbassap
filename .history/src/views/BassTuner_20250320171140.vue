<template>
  ...
  // Comparaison avec les notes fondamentales (tolérance ±2 Hz)
  ...
  for (let note of notes) {
    ...
    // Si la fréquence détectée est proche d'une harmonique (2x la fréquence fondamentale)
    if (Math.abs(detectedFreq - note.frequency * 2) <= tolerance) {
      detectedFreq = note.frequency; // Divise par 2 pour revenir à la fondamentale
      closestNote = note;
      break;
    }
  }
