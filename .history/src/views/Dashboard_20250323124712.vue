...
mounted() {
  ...
  // ✅ Ne continue que quand auth.user est bien défini
  const stopWatcher = this.$watch(
    () => auth.user?.email, // ou juste `auth.user` si t'es sûr qu’il est nul au départ
    (email) => {
      if (email) {
        console.log("✅ Utilisateur maintenant chargé :", auth.user);
        this.isAuthChecked = true;
        this.user = auth.user; // synchronise ton local si besoin
        stopWatcher();
      }
    },
    { immediate: true }
  ...
  // 🔄 Réécoute les changements de JWT dans d'autres onglets
