...
export default {
  ...
  setup() {
    ...
    const targetBase = "https://script.google.com/macros/s/AKfycbykYySA5D4taWYaICxCHt6l3WmOM5RNg4P_rCoJowfxusXpSy5D-ZuGFao9UjGWHD86AA/exec";
const route = "planning";
const fullTargetUrl = `${targetBase}?route=${route}&email=${encodeURIComponent(email.value)}&prenom=${encodeURIComponent(prenom.value)}&jwt=${encodeURIComponent(jwt)}`;
...
const response = await axios.get(proxyUrl, { cache: "no-store" });
  ...
  if (courseElement) {
    ...
    const API_URL = "https://cors-proxy-sbs.vercel.app/api/proxy?url=https://script.google.com/macros/s/AKfycbyBrhgMrzK14HMgiYI5SgkiJElfTmdX70RSnNJ85nw_6mVCIY4vrt2vL-DTMCEtu6mB4A/exec";
...
const prenom = ref(localStorage.getItem("prenom") || "");
  ...
  const url = fetchURL("GET") + 
    `?route=planning&email=${encodeURIComponent(email.value)}&prenom=${encodeURIComponent(prenom.value)}&jwt=${encodeURIComponent(jwt)}`;

  console.log("📡 URL API construite :", url);

  try {
    const response = await axios.get(url, { cache: "no-store" });
