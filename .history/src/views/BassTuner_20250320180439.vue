  ...
  export default {
    data() {
      return {
        ...
        lastNoteTime: 0, // Timestamp de la dernière note stable
    ...
    methods: {
      ...
      detectFrequency() {
        ...
        // ✅ Correction spéciale : empêcher 55 Hz d’apparaître après D
        ...
        if (closestNote) {
          if (Math.abs(detectedFreq - this.lastFrequency) > tolerance) {
            ...
            this.lastNoteTime = Date.now(); // Enregistrer le moment de la détection
            this.note = closestNote.name;
            this.frequency = detectedFreq;
            this.checkTuning(closestNote.frequency);
          }
        } else {
          // ✅ Empêcher un affichage erroné si la note précédente était D
