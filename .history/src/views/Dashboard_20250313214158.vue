...
async mounted() {
  ...
  if (cachedInfos) {
    ...
    // ✅ Priorité au cache pour éviter un chargement inutile
    this.user.objectif = parsedInfos.objectif || "🎯 Aucun objectif défini pour le moment. Fixe-toi un challenge !";
    console.log("✅ Objectif récupéré depuis le cache :", this.user.objectif);

    // Si l'objectif est manquant, on lance l'appel API en arrière-plan
    if (!parsedInfos.objectif) {
      console.log("⚠️ Objectif manquant, récupération depuis l'API...");
      await this.fetchFromAPI(true);
    }
  } else {
    console.warn("🚨 Aucun cache trouvé pour `userInfos_{prenom}`, récupération via API...");
    await this.fetchFromAPI(true);
  }
  ...
  // ✅ Chargement du cache principal (`userData_{prenom}`)
  const cacheWasUsed = await this.loadUserData();
  if (cacheWasUsed) {
    this.isLoading = false;
  }

  // 🔄 Mise à jour en arrière-plan si nécessaire (sans bloquer le chargement)
