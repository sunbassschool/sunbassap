<template>
  ...
  // Limite la fréquence à une plage raisonnable (éviter les harmoniques trop élevées)
  while (detectedFreq > 200) {
    detectedFreq /= 2; // Divise jusqu'à obtenir une fréquence plausible
  }

  // Vérifie si la fréquence est proche de Mi (E), qui devrait être autour de 41.2 Hz
  if (detectedFreq > 60 && detectedFreq < 80) {
    detectedFreq /= 2; // Divise pour obtenir la fondamentale
  }

  // Vérifie si la fréquence est proche de Ré (D), qui devrait être autour de 73.42 Hz
  if (detectedFreq > 100 && detectedFreq < 150) {
    detectedFreq /= 2;  // Divise pour ramener à la fondamentale de Ré
  }
