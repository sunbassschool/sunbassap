...
import { getValidToken, isUserLoggedIn, isUserAdmin, clearUserData, restoreTokensToIndexedDB, setTokenCookies, logoutUser, preventIndexedDBCleanup, checkAndRestoreTokens, refreshToken } from "@/utils/api.ts";
...
export default {
  ...
  setup() {
    ...
    const checkLoginStatus = async () => {
      ...
      let token = await getValidToken(); // Remplacer la logique par getValidToken
      nextTick(() => {
    ...
    onMounted(async () => {
      ...
      preventIndexedDBCleanup(); // ✅ Empêche IndexedDB d’être supprimé
      ...
      // Vérifie l'état de la connexion de l'utilisateur
      await checkLoginStatus();

      // Si un token est trouvé, on met en place le refresh automatique
