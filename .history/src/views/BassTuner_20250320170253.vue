<template>
  ...
  if (detectedFreq > 60 && detectedFreq < 75) {
    ...
    if (Math.abs(detectedFreq - 73.42) < 2) {
      detectedFreq = 73.42;  // Fixe la fréquence à 73.42 Hz pour Ré
  ...
  if (detectedFreq > 40 && detectedFreq < 50) {
    // Si la fréquence est proche de Mi (E), on évite de la diviser
    if (Math.abs(detectedFreq - 41.2) < 2) {
      detectedFreq = 41.2;  // Fixe la fréquence à 41.2 Hz pour Mi
    } else if (detectedFreq > 41.2) {
      detectedFreq /= 2;  // Divise encore pour obtenir la fondamentale de Mi (E)
    }
  }
