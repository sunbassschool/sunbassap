...
startMetronome() {
  ...
  this.initAudioContext(); // Assurer que l'AudioContext est initialisé
  this.resumeAudioContext(); // Vérifie et reprend l'AudioContext si suspendu
...
stopMetronome() {
  ...
  this.nextNoteTime = 0;
  cancelAnimationFrame(this.interval); // Arrêter les battements
}
...
updateTimer() {
  ...
  requestAnimationFrame(this.scheduleNextBeat);
}
