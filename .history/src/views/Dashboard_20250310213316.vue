<template>
  ...
  <i v-if="note.length" @click="clearNote" class="bi bi-x-circle clear-note-btn"></i>
</div>
...
cacheExpirationKey: `userData_${prenom}_expiration`,
    ...
    routes: {
      GET: "AKfycbwyVuqrGgzPZVTc9YsGF6-UVzX_5Adyrgs1OsSXW6h_ONbNL2Zi0_joOXowRNK3-YHZyQ/exec",
       POST: "AKfycbwyVuqrGgzPZVTc9YsGF6-UVzX_5Adyrgs1OsSXW6h_ONbNL2Zi0_joOXowRNK3-YHZyQ/exec"
    }
  };
}
,
...
async fetchFromAPI(forceRefresh = false) {
    ...
    console.error("❌ Erreur API : Réponse vide ou invalide");
    return;
}
...
,
    ...
    console.log("📅 Data.planning :", data.planning);

    if (!data.planning || !Array.isArray(data.planning) || data.planning.length === 0) {
        console.warn("⚠️ Aucun cours trouvé, affichage des cartes par défaut.");
        this.cards = [
            {
    ...
    const now = new Date();
    const prochainCours = data.planning.find(cours => new Date(cours.date) > now);

    if (!prochainCours) {
        console.warn("⚠️ Aucun cours futur trouvé.");
        this.cards = [
            {
                icon: "bi bi-calendar-event",
                title: "Prochain Cours",
    ...
    console.log("✅ `this.cards` mis à jour :", this.cards);
}
