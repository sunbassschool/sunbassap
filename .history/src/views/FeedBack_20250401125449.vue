...
async fetchFeedbacks() {
  ...
  try {
    ...
    if (result.feedbacks) {
      ...
      // Convertir les ID et ID_Feedback_Relé en nombres pour une comparaison fiable
      ...
      // Forcer les ID en number pour comparaison
      const feedbacksFormatted = principaux.reverse().map(fb => {
        const id = Number(String(fb.ID).replace("ID", ""));
        const associatedResponses = reponses.filter(rep => Number(rep.ID_Feedback_Relé) === id);
        return {
          ...fb,
          reponses: associatedResponses
        };
      });

      this.feedbacks = feedbacksFormatted;
    } else {
