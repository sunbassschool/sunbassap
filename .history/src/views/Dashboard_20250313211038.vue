...
async mounted() {
  ...
  if (cachedInfos) {
    try {
      this.userInfos = JSON.parse(cachedInfos) || {};
    } catch (e) {
      console.error("❌ Erreur lors du parsing de userInfos :", e);
      this.userInfos = {}; // ⚠️ Sécurisation : si parsing échoue, initialise un objet vide
    }
  } else {
    this.userInfos = {}; // ⚠️ Sécurisation : si aucun cache, initialise un objet vide
  }

  if (!this.userInfos.objectif) {
