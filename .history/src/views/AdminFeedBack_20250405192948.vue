...
<div v-if="getUnreadFeedbacks().length" class="unread-topics mb-4">
  <h4 class="text-warning mb-3">🔔 Feedbacks non lus ({{ getUnreadFeedbacks().length }})</h4>

  <div v-for="fb in getUnreadFeedbacks()" :key="fb.ID" class="topic-line bg-dark text-light border-bottom border-secondary py-3 px-3 d-flex justify-content-between align-items-start">
    <div class="flex-grow-1 me-3">
      <div class="d-flex justify-content-between">
        <strong class="text-info">{{ fb.Prenom || 'Élève inconnu' }}</strong>
        <small class="text-muted">{{ formatDate(fb.Date_Publication) }}</small>
      </div>
      <p class="mb-1 small text-light" style="line-height: 1.4;">
        {{ fb.Contenu.replace(/<[^>]*>/g, '').slice(0, 160) }}...
      </p>
    </div>
    <div>
      <button
        class="btn btn-sm btn-outline-info"
        @click="selectEleve({ email: fb.ID_Eleve, prenom: fb.Prenom })"
      >
        📄 Ouvrir
      </button>
    </div>
