...
async fetchFromAPI(forceRefresh = false) {
  ...
  try {
    const fixedPrenom = decodeURIComponent(escape(this.prenom)); // 🔧 Corrige UTF-8 s'il était déjà buggé
const url = this.getProxyURL(this.routes.GET, {
  route: "planning",
  jwt,
  email: this.email,
  prenom: encodeURIComponent(fixedPrenom) // ✅ Encoder seulement une fois, proprement
});
