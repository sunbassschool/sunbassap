...
async mounted() {
  try {
    ...
    if (!auth.jwt && !auth.user) {
      ...
      await this.fetchNote(); // ✅ On montre la note si dispo
      this.isLoading = false;
      
    }

    // 🔐 Tentative de récupération du JWT valide
    const jwt = await getValidToken();
    if (!jwt) {
      console.warn("🚫 Impossible de récupérer un JWT valide, arrêt du chargement.");
      this.isLoading = false;
      return;
    }
