...
<li class="resource-item">
  <template v-if="!isDataReady">  <!-- ✅ Affiche un loader en attendant -->
    <div class="loading-bar-container">
      <div class="loading-bar"></div> 
    </div>
  </template>

  <template v-else-if="user.espace_google_drive">  <!-- ✅ Affiche le lien si dispo -->
    <a :href="user.espace_google_drive" target="_blank">
      📂 Mon espace Google Drive
    </a>
  </template>

  <template v-else>  <!-- ❌ Affiche inscription si pas d'accès -->
    <a 
...
<li class="resource-item">
  ...
  <template v-else-if="user.playlist_youtube">  <!-- ✅ Affiche le lien si dispo -->
...
fetchURL(route) {
  ...
  if (cacheWasUsed) {
    ...
    this.isDataReady = true;  // ✅ Évite d'afficher le loader !
  } else {
    console.log("⚠️ Pas de cache, affichage du loader...");
    this.loading = true;
    this.isDataReady = false; // ❌ Assure que la barre s'affiche uniquement si besoin
  }
  ...
  setTimeout(async () => {
    ...
    this.isDataReady = true; // ✅ Les blocs peuvent s'afficher normalement
  }, 500);
}
