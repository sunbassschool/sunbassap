...
import { setJwtEverywhere, setRefreshTokenEverywhere } from "@/utils/setJwtEverywhere";
...
,
  ...
  try {
    // 🧹 Parallélisable avec le début de fetch si besoin
    const clearPromise = api.clearUserData();
    ...
    // 🧠 Pas besoin d’attendre chaque stockage ⇒ on les parallélise
    try {
  await Promise.all([
    clearPromise,
  ...
  this.setMessage("Erreur lors de la sauvegarde de session", "alert-danger");
}
...
,
  ...
  this.setMessage("Token manquant, veuillez réessayer.", "alert-danger");
  return;
}
  ...
  try {
    ...
    setJwtEverywhere(data.jwt);
setRefreshTokenEverywhere(data.refreshToken);
