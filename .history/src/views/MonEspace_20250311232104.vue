...
<li class="resource-item">
  <template v-if="loading">
    <div class="skeleton skeleton-text w-50"></div> <!-- 🔄 Loader temporaire -->
  </template>
...
fetchURL(route) {
  ...
  if (!cacheWasUsed) {
    ...
    this.loading = true; // Afficher le spinner uniquement si pas de cache
  } else {
    console.log("✅ Données récupérées via le cache, pas de spinner !");
    this.loading = false; // Pas de spinner si le cache est utilisé
  }

  // 🔄 Récupération des données API (le spinner ne s'affiche que si le cache était vide)
  setTimeout(async () => {
    await this.fetchUserData(true);
    this.loading = false; // Désactiver le spinner après récupération des données
  }, 500);
...
async fetchUserData(forceUpdate = false) {
    ...
    try {
        ...
        if (!data.error) {
            ...
            // 📌 Mise à jour intelligente : Ne met à jour que les valeurs reçues
            Object.keys(data).forEach(key => {
                if (data[key] !== null && data[key] !== undefined) {
                    cachedData[key] = data[key]; // ✅ Mise à jour des nouvelles valeurs
                }
            });
            ...
            this.user = cachedData; // ✅ Met à jour l'affichage
        } else {
