...
async mounted() {
  ...
  window.addEventListener("storage", (event) => {
    if (event.key === "jwt") {
      ...
      this.fetchNote(); // 🔥 Recharge la note immédiatement avec le nouveau JWT
    }
  });
...
/* 🎯 Style spécifique pour la date du prochain cours */
.dashboard-card.prochain-cours strong {
  font-size: 1rem; /* 📏 Taille plus grande */
  font-weight: bold;
  color: #ff8c00; /* 🟠 Orange dynamique */
  background: rgba(255, 140, 0, 0.1); /* 🔲 Fond léger */
  padding: 5px 10px;
  border-radius: 5px;
...
/* 🔥 Effet au survol */
.dashboard-card.prochain-cours strong:hover {
  background: rgba(255, 215, 0, 0.2); /* 🟡 Fond doré */
