...
,
 ...
 async updateNote() {
  ...
  // ✅ Enregistre toujours dans le cache local, connecté ou non
  let userData = getCache(`userData_${this.prenom}`) || {};
  userData.note = this.note.trim();
  setCache(`userData_${this.prenom}`, userData);

  // ⛔ Ne pas tenter d'envoyer si non connecté
  let jwt = await getValidToken();
  if (!jwt) {
    console.warn("🔒 Pas connecté, note enregistrée localement uniquement.");
    return;
  }

  // ⏳ Début du debounce pour l'API
