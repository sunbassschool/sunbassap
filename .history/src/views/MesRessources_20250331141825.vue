<template>
  ...
  v-for="file in group" 
  :key="file['ID Unique']" 
  class="list-group-item bg-dark text-light d-flex flex-column align-items-start p-2"
>
  <!-- Ligne cliquable pour fichiers non-audio -->
  <div 
    class="w-100 d-flex justify-content-between align-items-center"
    @click="file.type !== 'Audio' && openPreview(file)"
    style="cursor: pointer;"
  >
  ...
  <!-- 🎧 Lecteur audio pour fichiers Audio -->
  <audio 
    v-if="file.type === 'Audio'" 
    :src="`https://drive.google.com/uc?export=download&id=${file['ID Drive']}`" 
    controls 
  ...
  <div class="modal-content-custom">
    ...
    <iframe 
  :src="selectedFile['Lien direct'] || `https://drive.google.com/file/d/${selectedFile['ID Drive']}/preview`" 
  width="100%" 
  height="500"
  frameborder="0"
/>
...
,
openPreview(file) {
  if (file["Type de ressource"] === "Image" || file.type === "PDF" || file.type === "Autres") {
    this.selectedFile = file;
    this.showModal = true;
  } else if (file.type !== 'Audio') {
    window.open(file["Lien direct"] || `https://drive.google.com/file/d/${file['ID Drive']}/view`, '_blank');
  }
}
