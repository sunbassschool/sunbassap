      ...
      <div class="row justify-content-center">
        <div class="col-lg-12 col-md-10 w-100">
          <div class="card glass-card p-4 text-center animated-card">
            ...
            <div v-if="loading && !isDataReady" class="loading-content">
...
<!-- Loader principal UNIQUEMENT si c'est le premier chargement (sans cache) -->
<div v-if="loading && !isDataReady" class="loading-content">
  <div class="spinner-border text-primary" role="status">
    <span class="visually-hidden">Chargement...</span>
  </div>
...
<li class="resource-item">
  ...
  <template v-else>
    <a href="https://www.sunbassschool.com/step/inscription-aux-cours-en-visio/#checkout" class="text-muted text-decoration-none" target="_blank">
...
fetchURL(route) {
  ...
  if (!cacheWasUsed) {
    console.log("⚠️ Pas de cache, affichage du loader...");
    this.loading = true;  // ✅ Active le gros loader uniquement si pas de cache
    setTimeout(() => {
      this.isUpdating = true; // ✅ Active la barre de chargement immédiatement
    }, 50); // 🔥 Petit délai pour éviter un problème d'affichage
  } else {
    console.log("✅ Cache détecté, affichage immédiat !");
    this.loading = false; // ✅ Évite un rechargement inutile
  ...
  setTimeout(async () => {
    ...
    this.loading = false; // ✅ Désactive le loader principal après mise en cache
  }, 500);
...
async fetchUserData(forceUpdate = false) {
  ...
  this.isUpdating = true; // ✅ Forcer l'affichage de la barre
  ...
  try {
    ...
    if (!data.error) {
      ...
      if (!hasChanged) {
        console.log("✅ Données inchangées, pas besoin de mettre à jour !");
        this.isUpdating = false; // ✅ Désactiver la barre immédiatement
        return;
      }
    ...
    setTimeout(() => {
      this.isUpdating = false; // ✅ Retarde la désactivation de la barre pour éviter un effet de "clignotement"
    }, 300);
  }
}
