...
resetTimer() {
    this.elapsedTime = 0;
    this.formattedTime = "0"; // Réinitialise le format du temps
    if (this.isPlaying) {
      this.startTime = Date.now(); // Repartir à zéro si le timer est en cours
    }
  },
    scheduleNextBeat() {
...
/* Effet de survol */
.reset-icon:hover {
  background-color: rgba(0, 0, 0, 0.1); /* Légère couleur de fond pour le survol */
  transform: scale(1.1); /* Légère animation de zoom */
}

.reset-icon:hover svg {
  fill: #28a745; /* Changer la couleur de l'icône au survol (ex: vert) */
...
.timer p {
  font-size: 1.5rem;  /* Augmenter la taille pour la rendre plus visible */
