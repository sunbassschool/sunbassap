...
{ value: 4, label: '', icon: './assets/icons/sixteenth-note.png' }
  ...
  methods: {
    ...
    async loadSounds() {
      ...
      this.soundBuffers.strong = await this.loadSound('./assets/audio/strong-beat.wav');
this.soundBuffers.weak = await this.loadSound('./assets/audio/weak-beat.wav');
this.soundBuffers.sub = await this.loadSound('./assets/audio/subdivision.wav');
  ...
  this.resumeAudioContext(); // Vérifie et reprend l'AudioContext
  this.isPlaying = true;
...
async mounted() {
  ...
  // 🔥 Vérifier si le métronome était en cours au retour sur la page
  const wasPlaying = sessionStorage.getItem("isPlaying") === "true";
  this.isPlaying = wasPlaying;  // 🔥 Met à jour la variable
  ...
  // 🔵 Activer un faux son pour éviter l’arrêt du Web Audio (iOS)
