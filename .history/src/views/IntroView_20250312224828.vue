...
onMounted(async () => {
  ...
  try {
    ...
    // 🔹 Vérifie d'abord si le refreshToken est encore valide avant toute action
    ...
    showModal.value = true; // 🔥 Montre le bouton au lieu de bloquer
    return;
}
    console.log("🔍 Refresh token récupéré depuis IndexedDB :", refreshTokenAvailable);

    if (refreshTokenAvailable) { // ✅ On ne vérifie pas l'expiration du refresh token
    ...
    return router.replace("/login"); // 🔥 Redirection immédiate
    }

    // Si le JWT est valide, on redirige vers l'espace utilisateur
    if (jwt && jwt.includes(".") && jwt.split(".").length === 3 && !isJwtExpired(jwt)) {
    ...
    console.warn("⚠️ Aucun JWT valide ou refresh échoué, affichage du bouton Commencer.");
