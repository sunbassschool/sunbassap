...
<div class="info-box resource-box">
  ...
  <!-- 🔄 Loader affiché UNIQUEMENT si les liens ne sont pas dans le cache -->
...
fetchURL(route) {
  ...
  // Si le cache est utilisé, afficher immédiatement
  if (cacheUsed) {
    ...
    if (!this.user.espace_google_drive || !this.user.playlist_youtube) {
      ...
      // Lancer l'appel API en force (en arrière-plan) pour compléter les infos
      this.fetchUserData(true).finally(() => {
        this.isUpdating = false; // On cache la barre de progression dès que l'update est terminé
      });
    } else {
      this.isUpdating = false; // Tout est présent, on masque la barre
    }
  } else {
  ...
  // Ajout de l'écouteur global pour gérer les clics hors du champ
...
,
  ...
  if (cachedInfos) {
    ...
    // ✅ Utiliser immédiatement les données du cache si elles existent
    ...
    if (this.user.espace_google_drive && this.user.playlist_youtube) {
      console.log("✅ Ressources récupérées depuis le cache !");
      return; // 🔥 Pas besoin de loader ni d’appel API
    }
  }

  console.warn("⚠️ Ressources manquantes dans le cache, affichage du loader...");
  this.isLoadingResources = true; // 🔄 Active la barre de chargement

  // 🔥 Appel API pour récupérer les liens manquants
  await this.fetchUserInfos();

  this.isLoadingResources = false; // ✅ Désactive la barre après la récupération
}
...
async fetchUserInfos() {
  console.log("🔄 Récupération des infos utilisateur (Google Drive, YouTube, Objectif)...");
  ...
  try {
    ...
    if (!data.error) {
      const userInfos = {
        ...
        objectif: data.objectif || null, // 🔥 Ajout de l'objectif
      };

      // ✅ Mise à jour du cache `userInfos_{prenom}`
      localStorage.setItem(`userInfos_${this.prenom}`, JSON.stringify(userInfos));

      // ✅ Mise à jour des données de l'interface
      this.user.espace_google_drive = userInfos.espace_google_drive;
      this.user.playlist_youtube = userInfos.playlist_youtube;
      this.user.objectif = userInfos.objectif;

      console.log("✅ Objectif mis à jour :", this.user.objectif);
    } else {
...
async fetchUserData(forceUpdate = false) {
  ...
  try {
    ...
    if (!data.error) {
     ...
     // ⚡ Correction : force Vue à détecter la modification
this.user = Object.assign({}, this.user, data);
this.$forceUpdate(); // 🔥 Force Vue à re-render
console.log("🎯 Objectif après mise à jour forcée :", this.user.objectif);
