...
export default {
  ...
  methods: {
    ...
    startMetronome() {
  this.metronome.isPlaying = true; // ne démarre PAS localement, on laisse Global le faire
},
stopMetronome() {
...
,
  ...
  this.interval = setTimeout(() => this.scheduleNextBeat(), 25);
    ...
    increaseTempo() {
  if (this.metronome.tempo < 300) {
    this.metronome.tempo++;
  }
},
decreaseTempo() {
  if (this.metronome.tempo > 20) {
    this.metronome.tempo--;
  }
},
  ...
  watch: {
    ...
    'metronome.isPlaying'(val) {
    if (val) {
      this.startTimer(); // UI only
    } else {
      this.resetTimer(); // UI only
    }
  },
    swingAmount(val) {
