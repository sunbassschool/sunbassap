...
async fetchUserData(forceUpdate = false) {
  ...
  try {
    ...
    if (!data.error) {
      // 🔄 Fusionner avec les anciennes données (évite l'écrasement des liens)
      this.user = { 
        ...this.user,  // ✅ Garde les anciennes valeurs
        ...data,       // ✅ Applique les nouvelles valeurs de l'API
      };

      // 🛠️ Si l'API ne renvoie pas ces valeurs, on garde celles du cache
      if (!data.espace_google_drive) this.user.espace_google_drive = localStorage.getItem(this.cacheKey)?.espace_google_drive || "";
      if (!data.playlist_youtube) this.user.playlist_youtube = localStorage.getItem(this.cacheKey)?.playlist_youtube || "";

      console.log("✅ Données mises à jour avec fusion :", this.user);
