...
fetchURL(route) {
  ...
  if (!cacheWasUsed) {
    ...
    this.loading = true;
    this.isUpdating = true; // ✅ Afficher la barre dès le départ si pas de cache
  } else {
    console.log("✅ Données récupérées via le cache, pas de spinner !");
    this.loading = false;
    this.isDataReady = true; // ✅ Active l'affichage car les données sont prêtes
  }

  // 🔄 Récupération des données API (mise à jour du cache)
  setTimeout(async () => {
...
async fetchUserData(forceUpdate = false) {
  ...
  this.isUpdating = true; // 🔥 Active la barre de chargement pendant la mise à jour
  ...
  try {
    ...
    if (!data.error) {
      ...
      // 📌 Vérifier si les nouvelles données sont différentes de l'ancien cache
      const hasChanged = Object.keys(data).some(key => data[key] !== oldCache[key]);

      if (hasChanged) {
        console.log("💾 🔄 Mise à jour du cache (des changements détectés) !");
      ...
      this.isDataReady = true; // ✅ Marque que les données sont prêtes
    } else {
    ...
    this.isUpdating = false; // 🔥 Désactive la barre de chargement après la mise à jour
    this.loading = false; // 🚀 Assure que le loader principal est désactivé
  }
}
