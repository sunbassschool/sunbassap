...
export default {
  ...
  data() {
    return {
      loading: false,        // Chargement global de la page
      loadingResource: false, // Chargement spécifique aux ressources si nécessaire
      cacheUsed: false,
      isInitialized: false,
      isDataReady: false,     // Les données ressources sont prêtes (cache ou API)
      showModal: false,
      isUpdating: false,
      isSaving: false,
  ...
  mounted() {
    ...
    // Charge le cache
    this.loadUserData().then(cacheFound => {
      if (!cacheFound) {
        console.log("⚠️ Pas de cache, afficher loader ressources...");
        this.loading = true;
        this.loadingResource = true;
      } else {
        console.log("✅ Données en cache, affichage immédiat.");
  ...
  watch: {
    jwt(newVal, oldVal) {
      if (newVal && newVal !== oldVal && !this.isInitialized) {
