...
{ value: 4, label: '', icon: './assets/icons/sixteenth-note.png' }
  ...
  while (this.nextNoteTime < now + 0.1) {
    ...
    let baseInterval = (60.0 / this.tempo); // Temps entre chaque pulsation
    let subdivisionInterval = baseInterval / this.subdivision; // Temps entre subdivisions

    // 🔥 Swing appliqué UNIQUEMENT aux subdivisions impaires
    let swingOffset = (this.currentSubdivision % 2 === 0) 
                      ? this.swingAmount * subdivisionInterval 
                      : -this.swingAmount * subdivisionInterval;

    // ✅ La pulsation reste inchangée (pas de swing dessus)
    if (this.currentSubdivision === 1) {
      this.nextNoteTime += subdivisionInterval;
    } else {
