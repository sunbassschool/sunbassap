...
<li class="resource-item">
  ...
  <template v-else-if="user.espace_google_drive">
...
<li class="resource-item">
  ...
  <template v-else-if="user.playlist_youtube">
...
export default {
  ...
  data() {
    return {
      isInitialized: false,
      isReady: false,
      isLoadingResources: true, // 🔄 S'active uniquement si les ressources sont absentes du cache
      isDataReady: false,
      showModal: false,
...
async fetchUserData(forceUpdate = false) {
  ...
  try {
    ...
    // 🔄 Récupération du dernier JWT
    let jwt = sessionStorage.getItem("jwt") || localStorage.getItem("jwt");
    if (!jwt || jwtDecode(jwt).exp * 1000 < Date.now()) {
      console.warn("⚠️ JWT expiré, tentative de renouvellement...");
      jwt = await getToken();
      if (!jwt) {
        console.error("🚨 Impossible de renouveler le JWT !");
        return;
      }
    }

    const url = this.fetchURL("GET") + `?route=recupInfosMembres&jwt=${jwt}&prenom=${prenom}`;
