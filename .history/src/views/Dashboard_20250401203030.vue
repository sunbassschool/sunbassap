...
async fetchFromAPI(forceRefresh = false) {
  ...
  try {
// 🔧 Étape 1 : Correction UTF-8 au cas où le prénom serait déjà buggé
let fixedPrenom = this.prenom;
try {
  fixedPrenom = decodeURIComponent(escape(fixedPrenom)); // corrige "FranÃ§ois" → "François"
} catch (e) {
  console.warn("⚠️ Pas besoin de corriger UTF-8, prénom lisible.");
}

// 🔐 Étape 2 : Encodage safe pour passer en URL
...
console.log("👤 prénom API final (encodé) :", encodedPrenom);
