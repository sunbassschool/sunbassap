<template>
  ...
  // Diviser par 2 systématiquement jusqu'à ce que la fréquence soit proche d'une note fondamentale
  const notes = [
  ...
  // Diviser la fréquence par deux jusqu'à ce qu'elle soit dans la plage de fréquences fondamentales
  while (detectedFreq > 10) { // On arrête de diviser si la fréquence devient trop basse
    for (let note of notes) {
      if (Math.abs(detectedFreq - note.frequency) <= 2) {
        closestNote = note;
        break;
      }
    }
