...
this.soundBuffers.sub = await this.loadSound('/assets/audio/subdivision.wav');
    ...
    startMetronome() {
      if (!this.isPlaying) {
        ...
        this.timerInterval = setInterval(this.updateTimer, 1000); // Mise à jour du timer toutes les 1000ms (1s)
      }
    },
    stopMetronome() {
      this.isPlaying = false;
      clearInterval(this.timerInterval);  // Arrête le timer
      this.elapsedTime = Math.floor((Date.now() - this.startTime) / 1000);  // Enregistrer le temps écoulé en secondes
    },
updateTimer() {
      if (this.isPlaying) {
