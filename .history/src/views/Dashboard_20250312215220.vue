...
import { getToken,getJWTFromIndexedDB } from "@/utils/api.ts";
import { getUserInfoFromJWT } from "@/utils/auth.js";
...
async mounted() {
    ...
    // 1️⃣ Récupérer email & prénom
    await this.fetchUserData(); 

    if (!this.email || !this.prenom) {
        console.error("❌ Impossible de continuer : email/prénom manquants !");
        return this.$router.push("/login");
    }

    // 2️⃣ Charger les données du dashboard
    ...
    // 3️⃣ Charger la note après l'authentification réussie
...
beforeUnmount() {
  ...
  methods: {
    ...
    async fetchUserData() {
        try {
            ...
            console.log("✅ Infos utilisateur récupérées :", { email: this.email, prenom: this.prenom });
        } catch (err) {
  ...
  async loadUserData() {
    console.log("📦 Vérification du cache utilisateur...");
    ...
    console.warn("🚨 Pas de cache valide, récupération depuis l'API...");
    return false; // ❌ Le cache n'a pas été utilisé
}
,
    syncCache(event) {
  ...
  insertEmoji(event) {
    ...
    console.log("🔄 Chargement des infos utilisateur + Dashboard...");
    
    await this.loadDashboardData(); // 1️⃣ Récupérer email & prénom d'abord
    if (!this.email || !this.prenom) {
        console.error("❌ Impossible de charger le Dashboard : email/prénom manquant !");
        return;
    }
    
    await this.fetchFromAPI(); // 2️⃣ Maintenant qu'on a email & prénom, on peut récupérer le Dashboard
}
...
async loadDashboardData() {
    ...
    if (!jwt) {
        console.error("❌ Aucun JWT trouvé, redirection vers /login...");
        return this.$router.push("/login");
    }
    ...
    try {
        ...
        this.userRole = decoded.role || "user"; // 🔍 Vérifie le rôle utilisateur

        this.cacheKey = `userData_${this.prenom}`;
        this.cacheExpirationKey = `${this.cacheKey}_expiration`;
    } catch (error) {
        console.error("❌ Erreur lors du décodage du JWT :", error);
        return this.$router.push("/login");
    }
...
async fetchFromAPI(forceRefresh = false) {
    ...
    if (!this.email || !this.prenom) {
        console.error("❌ Impossible de récupérer les données : email/prénom toujours manquants !");
        return;
    }
    ...
    try {
        ...
        let url = `${this.apiBaseURL}${this.routes.GET}?route=planning&jwt=${jwt}&email=${this.email}&prenom=${this.prenom}`;
