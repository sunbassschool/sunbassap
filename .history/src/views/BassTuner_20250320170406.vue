<template>
  ...
  // Gestion de la corde Mi (E) à 41.2 Hz
  if (detectedFreq > 40 && detectedFreq < 50) {
    // Tolérance de +- 2 Hz autour de la fréquence fondamentale de Mi (E)
    if (Math.abs(detectedFreq - 41.2) < 2) {
      detectedFreq = 41.2;  // Fixe la fréquence de Mi à 41.2 Hz
    }
    // Si la fréquence est proche d'une harmonique, ignore la division
    else if (Math.abs(detectedFreq - 82.4) < 2) {
      detectedFreq = 41.2;  // Fixe la fréquence à 41.2 Hz si c'est une harmonique
    }
  }

  // Gestion de la corde Ré (D) à 73.42 Hz
  if (detectedFreq > 60 && detectedFreq < 80) {
    if (Math.abs(detectedFreq - 73.42) < 2) {
      detectedFreq = 73.42;  // Fixe la fréquence de Ré à 73.42 Hz
    }
    // Si la fréquence est plus proche d'une harmonique, divise pour revenir à la fondamentale
