<template>
  ...
  <label>Swing :</label>
  <input type="range" v-model="swingAmount" min="0" max="1" step="0.05">
</div>
<div class="control-group">
  <label>Style :</label>
  <select v-model="swingStyle">
    <option value="50">Funk</option>
    <option value="55">Hip-Hop</option>
    <option value="60">Blues</option>
    <option value="66">Jazz Standard</option>
    <option value="75">Swing Extrême</option>
  </select>
</div>
...
{ value: 4, label: '', icon: './assets/icons/sixteenth-note.png' }
  ...
  while (this.nextNoteTime < now + 0.1) {
    ...
    // 🔥 Calcul du swing SANS décaler le BPM global
    let baseInterval = (60.0 / this.tempo) / this.subdivision;
    let swingOffset = (this.currentSubdivision % 2 === 0) 
                      ? this.swingAmount * baseInterval 
                      : -this.swingAmount * baseInterval;

    // ✅ Rééquilibrer en ajustant l'offset inversement au suivant
    this.nextNoteTime += baseInterval;
    this.nextNoteTime += swingOffset / 2;  // Appliquer à cette subdivision
    this.nextNoteTime -= swingOffset / 2;  // Compense au suivant
  }
...
playClick() {
  ...
  // 🎵 Effet visuel du swing
  let swingRotation = this.swingAmount * 10 * (Math.random() - 0.5);
  document.getElementById("beatCircle").style.setProperty("--swing-rotation", swingRotation);
...
#beatCircle.beat {
  transform: scale(1.3) rotate(calc(var(--swing-rotation) * 1deg));
}
