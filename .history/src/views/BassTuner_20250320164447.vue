<template>
  ...
  // Vérifie et élimine les harmoniques
  ...
  // Si la fréquence détectée est proche du double d'une note, on la divise par 2
  let tolerance = 1.5; // Tolérance en Hz
  if (Math.abs(detectedFreq - closestNote.frequency * 2) < tolerance) {
    detectedFreq /= 2;
  }

  // Assure que la fréquence détectée soit proche de la note fondamentale
  while (Math.abs(detectedFreq - closestNote.frequency * 2) < tolerance) {
    detectedFreq /= 2;
  }

  // Mémorise la fréquence corrigée
