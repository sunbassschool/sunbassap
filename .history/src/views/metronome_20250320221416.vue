...
export default {
  ...
  if (!this.audioContext) {
    this.audioContext = new (window.AudioContext || window.webkitAudioContext)({
      ...
      sampleRate: 44100, // 🔥 Augmenter la qualité
    });
    console.log("🔊 AudioContext initialisé en mode high-performance !");
  }
},
...
,
  ...
  gainNode.gain.value = volumeLevel * 1.5; // 🔥 Augmentation du volume
...
,
  ...
  async mounted() {
  // 1️⃣ Initialisation de l'AudioContext
  this.initAudioContext();

  // 2️⃣ Chargement des sons
  await this.loadSounds();

  // 3️⃣ Activation du Wake Lock (empêche l'écran de s'éteindre)
  document.addEventListener("visibilitychange", this.requestWakeLock);
  this.requestWakeLock();

  // 4️⃣ Vérifier si on est sur mobile
  if (/Mobi|Android/i.test(navigator.userAgent)) {
    alert("🔊 Augmentez le volume média dans les réglages du téléphone !");

    // 5️⃣ Forcer le son sur haut-parleur
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      navigator.mediaDevices.getUserMedia({ audio: true })
        .then((stream) => {
          ...
          audio.play();
          console.log("🔊 Forçage du son sur haut-parleur !");
        })
        .catch((err) => console.error("⚠️ Erreur accès audio:", err));
    }
  }

  // 6️⃣ Vérifier si l’AudioContext est suspendu et le reprendre
