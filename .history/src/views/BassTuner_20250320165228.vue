<template>
  ...
  // Divise davantage pour les fréquences basses comme Mi et La
  while (detectedFreq > 100) {
    detectedFreq /= 2;
  }

  // Vérifie et ajuste la fréquence uniquement si elle est proche d'un harmonique
  // Réduit les harmoniques si la fréquence est trop haute (par exemple, 146.8 Hz pour Ré)
  ...
  // Si la fréquence est proche de la note fondamentale
  if (Math.abs(detectedFreq - closestNote.frequency) < 2) {
    this.frequency = detectedFreq;
  } else {
    this.frequency = closestNote.frequency; // Si trop loin, prendre la note la plus proche
  }
