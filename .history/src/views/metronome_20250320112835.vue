...
this.soundBuffers.sub = await this.loadSound('/app/assets/audio/subdivision.wav');
  ...
  clearTimeout(this.interval); // 🔴 Supprime toute boucle précédente
...
stopMetronome() {
  ...
  clearTimeout(this.interval);  // Empêche la superposition des beats
...
,
  ...
  // 🟢 Empêcher la mise en veille de l'écran (si supporté)
  if ('wakeLock' in navigator) {
    try {
      this.wakeLock = await navigator.wakeLock.request('screen'); // Garde l'écran allumé
    } catch (err) {
      console.error("Impossible d'activer le wake lock :", err);
    }
  }

  // 🔵 Astuce iOS : Activer un faux son pour éviter l'arrêt du Web Audio
  this.backgroundAudio = new Audio();
  this.backgroundAudio.loop = true;
  this.backgroundAudio.src = '/app/assets/audio/silence.mp3'; // Fichier audio silencieux
