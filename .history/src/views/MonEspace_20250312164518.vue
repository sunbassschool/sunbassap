...
fetchURL(route) {
  ...
  // 🔄 Récupération des nouvelles données en arrière-plan SANS affecter l'affichage si le cache était bon
  setTimeout(async () => {
    const wasUpdated = await this.fetchUserData(false); // ⏳ Ne force pas la mise à jour visuelle si ce n'est pas nécessaire
    if (!wasUpdated) {
      console.log("⏭️ Aucune mise à jour nécessaire, pas d'affichage du loader.");
      return;
    }
    this.loading = false;
  }, 500);
...
async fetchUserData(forceUpdate = false) {
  ...
  if (this.isDataReady && !forceUpdate) {
    console.log("⏭️ Données déjà prêtes, mise à jour annulée.");
    return false; // 🔥 Indique qu'il n'y a pas eu de mise à jour
  }
  ...
  return true; // 🔥 Indique qu'une mise à jour a bien eu lieu
}
