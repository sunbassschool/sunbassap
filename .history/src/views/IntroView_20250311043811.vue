...
onMounted(async () => {
  ...
  try {
    ...
    const refreshTokenAvailable = await withTimeout(getRefreshTokenFromDB(), 3000); // Timeout 3s
    ...
    if (!refreshTokenAvailable) {
      ...
      showModal.value = true; // Affichage du bouton Commencer
      return;
    }
    ...
    // Si le refresh échoue, on affiche le bouton Commencer
    console.warn("⚠️ Refresh échoué, affichage du bouton Commencer.");
    showModal.value = true;

  } catch (error) {
    console.error("❌ Erreur lors de l'initialisation :", error);
    showModal.value = true; // Assurer l'affichage du bouton en cas d'erreur
  }

  // 🔄 Petit délai pour éviter un blocage potentiel
  setTimeout(() => {
