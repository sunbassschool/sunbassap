...
async mounted() {
  try {
    ...
    if (!jwt) {
      console.warn("🛑 Aucun JWT valide. Interface en mode invité.");

      // ✅ Affichage minimal même pour non-connecté
      this.cards = [
        {
      ...
      await this.fetchNote(); // Tu peux afficher la note locale
      this.isLoading = false;
      return; // 🛑 STOP ici pour éviter les appels inutiles
    }

    window.addEventListener("storage", async (event) => {
      if (event.key === "jwt") {
        console.log("🔁 JWT modifié, resynchronisation");
    ...
    this.isLoading = false; // ✅ Sécurité absolue
  }
}
