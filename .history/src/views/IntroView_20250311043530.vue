...
onMounted(async () => {
  console.log("Début du script de vérification...");
  console.log("🔍 Vérification du JWT en cours...");

  let jwt = await getToken();

  // ✅ Vérifie si des données utilisateur sont en cache (offline mode)
  ...
  if (jwt && !isJwtExpired(jwt)) {
    ...
    console.log("JWT valide ?", !isJwtExpired(jwt));
  ...
  if (!refreshTokenAvailable) {
    console.warn("❌ Aucun refresh token, affichage du bouton Commencer.");
