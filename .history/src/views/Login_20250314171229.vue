...
import {verifyIndexedDBSetup, checkAndRestoreTokens, storeUserInfo, restoreUserInfo, restoreUserInfo    } from '@/utils/api.ts';
...
export default {
  ...
  // Retirer la vérification du statut de connexion
  // this.isLoggedIn = await this.checkLoginStatus();

  // Si tu veux que la page de login soit toujours affichée, tu peux supprimer la redirection
...
try {
  ...
  try {
    ...
    this.progress = 100;
    setTimeout(() => {
      this.loading = false; // ⏳ Attendre 500ms avant d'enlever la barre
