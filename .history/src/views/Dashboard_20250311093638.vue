...
clearNote() {
    ...
    this.note = ""; // Vide la note
    this.updateNote(); // Sauvegarde immédiatement
  },
...
,
    ...
    updateData(data) {
  console.log("📌 Mise à jour des données avec :", data);

  // 🔍 Récupérer les anciennes données du cache
  let cachedData = localStorage.getItem(this.cacheKey);
  cachedData = cachedData ? JSON.parse(cachedData) : {};

  // 🔄 Fusion des données API avec les anciennes données (on conserve ce que l'API ne renvoie pas)
  const updatedData = { 
    ...cachedData, // ✅ Garde les anciennes valeurs en priorité
    ...data,       // ✅ Ajoute les nouvelles données de l'API
  };

  // 🔥 Empêcher la suppression des liens si l’API ne les fournit pas
