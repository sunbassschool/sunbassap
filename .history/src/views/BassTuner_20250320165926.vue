<template>
  ...
  // Limite la fréquence à une plage raisonnable pour éviter des valeurs trop élevées
  while (detectedFreq > 200) {
    detectedFreq /= 2;
  }

  // Vérification pour Mi (E) - Plage entre 40 Hz et 50 Hz
  if (detectedFreq > 40 && detectedFreq < 50) {
    detectedFreq = 41.2; // Fixe à la fréquence de Mi (E) si dans la plage
  }

  // Vérification pour Ré (D) - Plage entre 70 Hz et 80 Hz
  if (detectedFreq > 70 && detectedFreq < 80) {
    // Si la fréquence est proche de la fondamentale de Ré (D), on ne la divise pas
    if (Math.abs(detectedFreq - 73.42) < 2) {
      detectedFreq = 73.42; // Fixe à la fréquence de Ré (D) si dans la tolérance
    } else if (detectedFreq > 73) {
      detectedFreq /= 2; // Divise si la fréquence est trop élevée
    }
  }
