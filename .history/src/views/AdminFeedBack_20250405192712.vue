<template>
  ...
  <!-- 🔔 Feedbacks non lus -->
<div v-if="getUnreadFeedbacks().length" class="mb-4">
  <h4 class="text-warning">🔔 Feedbacks non lus ({{ getUnreadFeedbacks().length }})</h4>
  <div
    v-for="fb in getUnreadFeedbacks()"
    :key="fb.ID"
    class="bg-dark text-light border border-warning rounded p-3 mb-2"
  >
    <strong>{{ fb.Prenom }} ({{ fb.ID_Eleve }})</strong>
    <small class="text-muted float-end">{{ formatDate(fb.Date_Publication) }}</small>
    <p class="mt-2">{{ fb.Contenu.replace(/<[^>]*>/g, '').slice(0, 100) }}...</p>
    <button class="btn btn-sm btn-outline-info mt-2" @click="selectEleve({ email: fb.ID_Eleve, prenom: fb.Prenom })">
      Voir l'élève
    </button>
  </div>
</div>
...
await this.fetchAllFeedbacks();
  ...
  methods: {
    getUnreadFeedbacks() {
  const unread = this.feedbacksAll.filter(fb => {
    console.log("📤 Feedback examiné :", fb); // 👀 inspecte en live
    return this.feedbacksAll.filter(fb =>
  fb.Statut?.toLowerCase() === "non lu" && fb.Type === "Eleve"
);
  });
  console.log("📊 Total non lus :", unread.length);
  return unread;
}
,
