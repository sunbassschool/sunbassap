...
fetchURL(route) {
  ...
  // 🔄 Mise à jour des données (mais sans impacter le cache si elles n’ont pas changé)
  setTimeout(async () => {
    await this.fetchUserData(true);
    if (!cacheWasUsed) this.loading = false; // ✅ Désactive le loader principal après mise en cache
  }, 500);
