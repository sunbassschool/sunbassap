...
async fetchUserData(forceUpdate = false) {
  ...
  try {
    ...
    if (!data.error) {
      ...
      if (!hasChanged) {
        console.log("✅ Aucune mise à jour nécessaire, données inchangées !");
        this.isUpdating = false; // 🔥 Désactive l’animation immédiatement
        return;
      }

      console.log("💾 🔄 Mise à jour du cache !");
      localStorage.setItem(this.cacheKey, JSON.stringify(data)); // ✅ Mettre à jour uniquement si nécessaire
    ...
    this.isUpdating = false; // 🔥 Désactive la barre de mise à jour
  }
}
