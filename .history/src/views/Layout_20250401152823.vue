...
<router-link v-if="isAdmin" to="/cours" class="nav-link mon-espace">📚 Cours</router-link>
 ...
 <main class="page-content fullwidth">
  ...
  <div v-if="authStore.isRefreshingToken" class="loading">
...
export default {
  ...
  setup() {
    const authStore = useAuthStore();  // ⚡️ Initialisation correcte du store avant son utilisation

    // Accède à `isRefreshingToken` après avoir initialisé `authStore`
    const isRefreshingToken = computed(() => authStore.isRefreshingToken); 
    ...
    // Déclaration des autres variables réactives
    ...
    const handleLogout = async () => {
      ...
      authStore.$reset(); // 🔥 Réinitialiser l'état global de Pinia
