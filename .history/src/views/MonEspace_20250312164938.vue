...
fetchURL(route) {
  ...
  if (cacheWasUsed) {
    ...
    this.loading = false; // NE PAS mettre de loading si cache présent
  } else {
    console.log("⚠️ Pas de cache, affichage du loader...");
    this.loading = true;  // On ne met le loader que si aucune donnée n'est dispo
  }
  ...
  setTimeout(async () => {
    await this.fetchUserData(false); // 🔥 Mise à jour discrète en arrière-plan
  }, 500);
