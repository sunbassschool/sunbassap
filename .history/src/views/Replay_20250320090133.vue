...
<div class="fixed-menu">
  ...
  <select id="courseSelect" v-model="selectedCourse" class="form-select">
    ...
    <div class="container-xxl mt-4">
      <!-- ✅ Message si aucun replay n'est disponible -->
<div v-if="!loading && !hasReplay" class="alert alert-warning text-center mt-3">
  <p class="mb-2">🎥 Aucun replay disponible pour le moment.</p>
  <p>Reviens plus tard ou contacte ton coach pour en savoir plus !</p>
</div>
      <!-- ✅ Tableau des cours -->
...
<div v-if="windowWidth < 768">
  <!-- ✅ Message si aucun replay disponible -->
<div v-if="!loading && sortedPlanningData.length === 0" class="alert alert-warning text-center mt-3">
  <p class="mb-2">🎥 Aucun replay disponible pour le moment.</p>
  <p>Reviens plus tard ou contacte ton coach pour en savoir plus !</p>
</div>
...
export default {
  ...
  return sortedPlanningData.value.some(course => course.lienReplay && course.lienReplay !== "Pas de replay disponible");
});
  ...
  try {
    const response = await axios.get(`${API_URL}?route=planning&email=${encodeURIComponent(email.value)}&prenom=${encodeURIComponent(prenom.value)}`, {
      cache: "no-store" // 🔥 Force un appel API sans cache
    });

    if (response.data.success && response.data.planning) {
      // 🔹 3. Comparer les nouvelles données avec le cache
      const newData = JSON.stringify(response.data.planning);
      if (newData !== localStorage.getItem("planningData")) {
        // 🔥 4. Mettre à jour uniquement si les données ont changé
