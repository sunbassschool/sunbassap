...
export default {
  ...
  data() {
    return {
      ...
      routes: {
        GET: "AKfycbx0GXYt_ruigUpLyMBF-btgf_xNW1BZNbwkHnzkKcHvkkIUEvbZu7F2HmhG4XXOW0shPw/exec",
        POST: "AKfycbx0GXYt_ruigUpLyMBF-btgf_xNW1BZNbwkHnzkKcHvkkIUEvbZu7F2HmhG4XXOW0shPw/exec"
      },
    };
  },
  async mounted() {
...
async toggleReadStatus(feedback) {
  ...
  // Préparer l'URL et le payload
  const url = this.getProxyPostURL(this.routes.POST);
  const payload = {
    route: "validatefeedback",  // L'endpoint backend
    jwt,
    feedback_id: feedback.ID,  // ID du feedback
