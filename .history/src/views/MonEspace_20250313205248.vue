...
async loadUserData() {
  ...
  if (cachedData && cacheExpiration && Date.now() < parseInt(cacheExpiration)) {
    ...
    if (!this.user.espace_google_drive || !this.user.playlist_youtube) {
      console.log("⚠️ Données incomplètes dans le cache, forçage de la mise à jour via l'API.");
      await this.fetchUserData(true);
    }
    
    this.isDataReady = true;  // Données prêtes
    this.loading = false;     // Cache chargé, plus de loader
...
async loadUserInfos() {
  ...
  if (cachedInfos) {
    ...
    if (!this.user.espace_google_drive || !this.user.playlist_youtube) {
      console.log("⚠️ Données incomplètes, on appelle l'API...");
      await this.fetchUserInfos();
    }
    return;
  }
  
  console.warn("🚨 Aucun cache trouvé pour `userInfos_{prenom}`, appel API obligatoire.");
  await this.fetchUserInfos();
}
,
beforeUnmount() {
