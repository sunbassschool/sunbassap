...
</textarea><br /> <div class="d-flex justify-content-end mt-1">
  <button 
    class="btn btn-sm btn-outline-light refresh-note-btn d-flex align-items-center gap-2"
    @click="fetchNote(true)"
    :disabled="isRefreshingNote"
  >
    <span v-if="!isRefreshingNote">🔍 Voir si le prof a écrit</span>
    <span v-else class="spinner-border spinner-border-sm text-light" role="status" aria-hidden="true"></span>
  </button>
</div>
...
async fetchNote(forceRefresh = false) {
  ...
  // ✅ On saute le cache si on force le rafraîchissement
  ...
  this.isRefreshingNote = forceRefresh; // ✅ Ne montrer le spinner que si on a cliqué le bouton
...
.custom-spinner {
  width: 35px;  /* 📏 Augmente la taille */
  height: 35px; /* 📏 Augmente la taille */
  border-width: 3px; /* 🔥 Rend les bordures plus épaisses */
  color: rgb(255, 72, 0);  /* 🎨 Change la couleur (par défaut, il prend la couleur du texte) */
  border-color: red; /* 🟥 Change la couleur de la bordure */
  border-right-color: transparent; /* 🌀 Effet d'animation */
  animation: spin 1s linear infinite; /* 🔄 Animation fluide */
}
