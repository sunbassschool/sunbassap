...
fetchURL(route) {
  ...
  const cacheWasUsed = this.loadUserData(); // Charge les données en cache

  if (cacheWasUsed) {
    console.log("✅ Données récupérées via le cache !");
    this.loading = false;  // ⛔ S'assurer que le loader ne s'affiche PAS si le cache est là
    this.isUpdating = false; // 🔄 Évite de relancer l'animation de mise à jour
  } else {
    console.log("⚠️ Pas de cache, affichage du loader...");
    this.loading = true;  // 🚨 On met le loader que si aucune donnée n'est dispo
  }
  ...
  setTimeout(async () => {
    await this.fetchUserData(false); // 🔥 Mise à jour discrète en arrière-plan, SANS impacter `loading`
  }, 500);
...
async fetchUserData(forceUpdate = false) {
  console.log("🔄 Vérification des données en cache avant requête...");

  const cacheExpiration = localStorage.getItem(this.cacheExpirationKey);
  if (cacheExpiration && Date.now() < parseInt(cacheExpiration) && !forceUpdate) {
    console.log("⏭️ Cache toujours valide, pas de mise à jour nécessaire.");
    return;
  }
  ...
  try {
    ...
    if (!data.error) {
      ...
      if (currentData !== newData) {
        console.log("🔄 Mise à jour des données affichées !");
        this.user = { ...this.user, ...data };
        this.isDataReady = true; // 🔥 S'assurer que les données sont prêtes
      } else {
        console.log("⏭️ Pas de changement détecté, pas de mise à jour de l'affichage.");
      }
