...
export default {
    ...
    data() {
      return {
        ...
        // Nouvelle donnée pour stocker le temps écoulé en secondes
        elapsedTime: 0,
        startTime: 0
      };
    },
    computed: {
      // Propriété calculée qui retourne le temps formaté
      formattedTime() {
      ...
      timerColor() {
        if (this.elapsedTime < 300) return 'blue'; // Moins de 5 minutes
        if (this.elapsedTime >= 300 && this.elapsedTime < 600) return 'orange'; // Entre 5 et 10 minutes
        return 'red'; // Plus de 10 minutes
      }
    },
    methods: {
      async loadSounds() {
      ...
      startMetronome() {
        if (!this.isPlaying) {
          this.isPlaying = true;
          this.startTime = Date.now() - this.elapsedTime * 1000; // Reprendre le temps correctement
...
.timer p {
  ...
  font-weight: bold;
  text-shadow: 0 0 10px rgba(255, 255, 255, 0.5), 0 0 20px rgba(0, 255, 0, 0.5);  /* Ombres lumineuses pour donner un effet dynamique */
