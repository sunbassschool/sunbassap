...
const cacheKey = `userData_${prenom}`;
  ...
  return{
    ...
    routes: {
      GET: "AKfycbyBrhgMrzK14HMgiYI5SgkiJElfTmdX70RSnNJ85nw_6mVCIY4vrt2vL-DTMCEtu6mB4A/exec",
       POST: "AKfycbyBrhgMrzK14HMgiYI5SgkiJElfTmdX70RSnNJ85nw_6mVCIY4vrt2vL-DTMCEtu6mB4A/exec"
    }
  };
}
,
...
async fetchNote(forceRefresh = false) {
  ...
  const base = "https://script.google.com/macros/s/AKfycbyBrhgMrzK14HMgiYI5SgkiJElfTmdX70RSnNJ85nw_6mVCIY4vrt2vL-DTMCEtu6mB4A/exec";
const query = `?route=getnote&jwt=${jwt}`;
const fullURL = encodeURIComponent(`${base}${query}`);

const proxyURL = `https://cors-proxy-sbs.vercel.app/api/proxy?url=${fullURL}`;
