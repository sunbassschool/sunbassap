...
,
startMetronome() {
    // Vérifie si le métronome est déjà en cours, si oui, ne rien faire
...
,
  ...
  async mounted() {
  // Vérifie si le métronome est déjà en cours lorsque la page est chargée
  const isMetronomeActive = sessionStorage.getItem("isPlaying") === "true";
  
  if (isMetronomeActive) {
    this.isPlaying = true;  // Si le métronome est actif, on le maintient actif
    this.startTimer();
    this.scheduleNextBeat();
  };
