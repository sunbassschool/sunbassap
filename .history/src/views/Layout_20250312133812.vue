...
import {  getToken, isUserLoggedIn, isUserAdmin, clearUserData, restoreTokensToIndexedDB, setTokenCookies, logoutUser, preventIndexedDBCleanup, checkAndRestoreTokens, refreshToken, getRefreshTokenFromDB     } from "@/utils/api.ts";
...
const checkLoginStatus = async () => {
    ...
    await restoreTokensToIndexedDB(); // 🔄 Restauration des tokens depuis IndexedDB si nécessaire

    let refreshToken = await getRefreshTokenFromDB(); // Récupération du refresh token

    // ✅ Restaurer le JWT dans `sessionStorage` s'il est encore disponible
    if (token && !sessionStorage.getItem("jwt")) {
        console.log("🔄 [FIX] Réécriture du JWT dans sessionStorage.");
        sessionStorage.setItem("jwt", token);
    }

    // ✅ Restaurer le refresh token dans `sessionStorage`
...
onMounted(async () => {
  ...
  // ✅ Vérifie et restaure les tokens (JWT & refreshToken)
