...
async mounted() {
  ...
  this.authCheckInProgress = true; // 🔄 Démarrer la vérification
  try {
    await verifyIndexedDBSetup();
    await checkAndRestoreTokens();
    await restoreUserInfo(); // 🔥 Restaure les infos utilisateur perdues

    // 🔄 Vérifier si un JWT valide est disponible avant d'afficher le formulaire
    let jwt = await getToken();

    // Si on est déjà connecté, on redirige sans vérifier les autres tokens
    if (jwt && !isJwtExpired(jwt)) {
      console.log("✅ JWT valide trouvé, redirection vers la page adéquate !");
      return this.redirectUser(); // 🔥 Redirection vers la page adéquate
    }

    // ✅ Vérifier si un refresh token est disponible avant de tenter un refresh
    const refreshTokenExists = localStorage.getItem("refreshToken") || sessionStorage.getItem("refreshToken");

    if (!jwt || isJwtExpired(jwt)) {
      if (!refreshTokenExists) {
        console.warn("⚠️ Aucun JWT et aucun refresh token disponible. Affichage du formulaire.");
        return;
      }

      console.warn("⏳ Aucun JWT valide, tentative de refresh...");
      try {
        jwt = await refreshToken();
        if (jwt) {
          console.log("✅ JWT rafraîchi avec succès, redirection vers /dashboard !");
          return this.redirectUser(); // 🔥 Redirection automatique après refresh
        }
      } catch (error) {
        console.error("❌ Échec du refresh :", error);
      }
    } else {
      console.log("✅ JWT valide trouvé ! Pas de refresh nécessaire.");
      return this.redirectUser(); // 🔥 Si un JWT valide est trouvé, rediriger immédiatement
    }
  } catch (error) {
...
,
  ...
  methods: {
    ...
    async login() {
      ...
      this.startLoading(); // Démarre l'animation réaliste
      try {
        ...
        setTimeout(() => {
          this.loading = false;
          this.redirectUser(); // 🔥 Redirection après connexion
        }, 500);
      } catch (error) {
    ...
    async storeSession(data) {
      ...
      try {
        ...
        const userData = { prenom: decoded.prenom || "Utilisateur", email: decoded.email || "" };
