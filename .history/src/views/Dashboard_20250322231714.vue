    ...
    <div class="container d-flex flex-column align-items-center justify-content-center">
      ...
      <div v-if="!isLoading" class="content">
        ...
        >
          ...
          <!-- Exemple de carte où le contenu change en fonction de la connexion -->
          <div v-if="index === 1" class="dashboard-card rounded-3 p-4 d-flex flex-column">
            <h3 class="h5 mb-2">🎶 🎯 Coaching & Feedback</h3>

            <!-- Si l'utilisateur est connecté, afficher un champ de texte pour les notes -->
            <textarea
              v-if="isLoggedIn"
              v-model="note"
            ...
            <p v-else class="text-muted mt-2">🎸 Connecte-toi pour laisser une note et recevoir un feedback !</p>

            <!-- Bouton pour charger la note -->
            ...
            <!-- Message de mise à jour de la note -->
            <div v-if="noteUpdatedMessage" class="note-update-msg mt-1">
              {{ noteUpdatedMessage }}
            </div>

            <!-- Option pour effacer la note -->
      ...
      <!-- 🔒 Si l'élève n'est pas connecté, afficher un message ou un lien pour se connecter -->
      <div v-if="!isLoading && !isLoggedIn" class="guest-content">
        <p>🎸 Connecte-toi pour voir ton planning et ton objectif personnalisé !</p>
        <button class="btn btn-primary" @click="redirectToLogin">Se connecter</button>
      </div>
...
async mounted() {
  try {
    const auth = useAuthStore();
    await auth.loadUser(); // Charge les données de l'utilisateur depuis le store

    // Vérifie si l'utilisateur est connecté
    let jwt = await getToken(); // Tente de récupérer le JWT

    if (!jwt) {
      console.warn("⚠️ Utilisateur non connecté, mode invité activé");
      this.cards = [
        {
      ...
      return; // Si non connecté, s'arrête ici
    }

    // Si l'utilisateur est connecté, on continue avec les appels API et autres actions
    await this.fetchNote(); // Charge les notes
    const cacheOk = await this.loadUserData();

    if (!cacheOk || shouldUpdateCache(this.cacheKey, this.cacheDuration)) {
...
beforeUnmount() {
  ...
  if (!auth.jwt || !auth.user) {
    console.warn("❌ Aucun utilisateur connecté, annulation du chargement.");
    return false;
  }
...
,
    ...
    const auth = useAuthStore();
if (!auth.jwt || !auth.user) {
  console.warn("🔓 Utilisateur non connecté, affichage limité des cartes.");
  this.cards = [
    {
      icon: "bi bi-calendar-event",
      title: "Prochain Cours",
