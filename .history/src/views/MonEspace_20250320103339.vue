...
<div class="info-box resource-box">
  ...
  <!-- 🔄 Loader affiché UNIQUEMENT si les liens sont en cours de chargement -->
  <div v-if="isLoadingResources" class="loading-container">
    <div class="spinner-border text-danger" role="status">
      <span class="visually-hidden">Chargement des ressources...</span>
    </div>
    <p class="mt-2 text-muted">📡 Récupération de tes ressources en cours...</p>
  </div>

  <!-- 📂 Affichage des liens UNIQUEMENT si le chargement est terminé -->
  <ul v-if="!isLoadingResources" class="list-group list-unstyled">
    ...
    <li class="resource-item">
      ...
      <template v-else>
        <a 
          ...
          target="_blank"
        >
          🎵 Ma playlist YouTube <strong>(accès réservé)</strong>.
          <br>
          <span class="btn btn-primary btn-sm mt-2">🔗 S'inscrire maintenant</span>
        </a>
      </template>
...
import { getToken,getFromIndexedDB, saveToIndexedDB } from "@/utils/api";
...
fetchURL(route) {
  ...
  const cacheUsed = await this.loadUserData(); // ✅ On attend le chargement du cache
  this.loadUserInfos(); // 🔥 Récupération des infos complémentaires (si besoin)

  if (cacheUsed) {
    console.log("✅ Données récupérées via le cache.");
    this.loading = false;
    if (!this.user.espace_google_drive || !this.user.playlist_youtube) {
      console.log("⚠️ Données incomplètes, mise à jour en arrière-plan...");
    ...
    console.log("⚠️ Cache expiré, chargement via API...");
    this.loading = true;
    this.isUpdating = true;
    await this.fetchUserData(true); // ✅ On attend la récupération des données
    this.loading = false;
    this.isUpdating = false;
  }

  // ✅ Écoute les mises à jour du JWT
  window.addEventListener("storage", (event) => {
    if (event.key === "jwt") {
      console.log("🔄 JWT mis à jour, récupération des données...");
      this.fetchUserData(true); // 🔥 Recharge immédiatement Google Drive & YouTube
    }
  });
...
async loadUserData() {
  console.log("🔍 Début de `loadUserData()`");

  console.log("📌 Cache Key générée :", this.cacheKey);
  console.log("📦 Données brutes du cache récupérées :", localStorage.getItem(this.cacheKey));
  console.log("🕒 Timestamp d'expiration enregistré :", localStorage.getItem(this.cacheExpirationKey));
  ...
  if (cachedData && cacheExpiration) {
    const expirationTimestamp = parseInt(cacheExpiration, 10);
    console.log("⏳ Expiration du cache dans :", expirationTimestamp - Date.now(), "ms");

    if (Date.now() < expirationTimestamp) {
      console.log("✅ Cache valide ? OUI");
      
      // 🛑 Empêche d'appeler l'API si le cache est encore bon
      ...
      console.warn("⏳ Le cache est expiré !");
    }
  } else {
    console.warn("🚨 Le cache est introuvable ou incomplet !");
  }
...
async fetchUserInfos() {
  console.log("🔄 Récupération des infos utilisateur (Google Drive, YouTube)...");
  this.isLoadingResources = true;

  try {
    const url = this.fetchURL("GET") + `?route=recupInfosMembres&jwt=${await getToken()}&prenom=${this.prenom}`;
    ...
    if (!data.error) {
      this.user.espace_google_drive = data.espace_google_drive || this.user.espace_google_drive;
      this.user.playlist_youtube = data.playlist_youtube || this.user.playlist_youtube;
      
      // ✅ Mettre à jour le cache sans forcer un rechargement complet
...
async fetchUserData(forceUpdate = false) {
  ...
  const cachedData = localStorage.getItem(this.cacheKey);
  const cacheExpiration = localStorage.getItem(this.cacheExpirationKey);
  const expirationTimestamp = cacheExpiration ? parseInt(cacheExpiration, 10) : null;

  console.log("📦 Cache actuel :", cachedData ? "✔️ Présent" : "❌ Absent");
  console.log("🕒 Expiration enregistrée :", expirationTimestamp);
  console.log("⏳ Temps restant :", expirationTimestamp ? expirationTimestamp - Date.now() : "N/A");

  // ✅ Si les données en cache sont valides et pas expirées, on évite l'appel API
  if (cachedData && expirationTimestamp && Date.now() < expirationTimestamp && !forceUpdate) {
    console.log("✅ Données valides dans le cache, chargement immédiat !");
    this.user = JSON.parse(cachedData);
    this.isDataReady = true;
    this.loading = false;
    this.isUpdating = false;
  ...
  console.warn("🚨 Cache expiré ou absent, appel API nécessaire...");
  this.loading = true;
  this.isUpdating = true;
  ...
  const prenom = sessionStorage.getItem("prenom") || localStorage.getItem("prenom");
  if (!prenom) {
    console.error("❌ Prénom manquant, impossible d'appeler l'API.");
    this.loading = false;
    return;
  }

  console.log("📡 Récupération des données utilisateur pour :", prenom);

  try {
    // 🔗 Construction de l'URL API
    const url = this.fetchURL("GET") + `?route=recupInfosMembres&jwt=${await getToken()}&prenom=${prenom}`;
    ...
    if (!data || data.error) {
      console.error("❌ L'API a renvoyé une erreur :", data.message || "Aucune donnée reçue.");
      this.loading = false;
      return;
    }
    ...
    // ✅ Sauvegarde des données et expiration dans localStorage
    try {
      localStorage.setItem("prenom", this.user.prenom);
      localStorage.setItem("email", this.user.email);
      localStorage.setItem(this.cacheKey, JSON.stringify(this.user));

      // 🕒 Stocker la date d'expiration du cache
      const newExpirationTimestamp = Date.now() + this.cacheDuration;
      localStorage.setItem(this.cacheExpirationKey, newExpirationTimestamp.toString());

      console.log("✅ Données stockées dans localStorage avec succès !");
      console.log("🕒 Nouvelle expiration du cache :", new Date(newExpirationTimestamp));
    } catch (e) {
...
async loadUserData() {
  ...
  if (cachedData && cacheExpiration) {
    ...
    if (Date.now() < expirationTimestamp) {
      ...
      console.log("🎯 Données utilisateur chargées depuis le cache :", this.user);

      // 🔥 Vérifions que prénom et email sont bien récupérés
      if (!this.user.prenom || !this.user.email) {
        console.warn("⚠️ Prénom ou email manquant dans le cache, appel API...");
        await this.fetchUserData(true);
      } else {
        console.log("✅ Cache complet, aucun appel API nécessaire.");
      }
      ...
      console.warn("⏳ Le cache est expiré ! Il faut recharger les données.");
    }
  } else {
    console.warn("🚨 Le cache est introuvable ou incomplet !");
  }
...
async fetchUserData(forceUpdate = false) {
  ...
  const expirationTimestamp = cacheExpiration ? parseInt(cacheExpiration, 10) : null;
  const isCacheValid = cachedData && expirationTimestamp && Date.now() < expirationTimestamp;

  console.log("📦 Cache Key :", this.cacheKey);
  console.log("📦 Données brutes du cache récupérées :", cachedData);
  console.log("🕒 Timestamp d'expiration enregistré :", expirationTimestamp);
  console.log("⏳ Temps restant avant expiration :", expirationTimestamp ? expirationTimestamp - Date.now() : "N/A");
  console.log("✅ Cache valide ?", isCacheValid ? "OUI" : "NON");

  // ✅ Utilise le cache si valide et qu'on ne force pas l'update
  ...
  try {
    ...
    // ✅ Mise en cache avec gestion correcte du timestamp d'expiration
    try {
      localStorage.setItem("prenom", this.user.prenom);
      localStorage.setItem("email", this.user.email);
      localStorage.setItem(this.cacheKey, JSON.stringify(this.user));

      // 🕒 Si `cacheExpiration` est null, on le définit correctement
      const newExpirationTimestamp = Date.now() + this.cacheDuration;
      localStorage.setItem(this.cacheExpirationKey, newExpirationTimestamp.toString());
