...
import {verifyIndexedDBSetup, checkAndRestoreTokens, isJwtExpired    } from '@/utils/api.ts';
...
export default {
  ...
  try {
    // 🔍 Vérifier et restaurer IndexedDB au lieu de la recréer
    await verifyIndexedDBSetup();

    // 🔄 Vérifier et restaurer les tokens (JWT et refreshToken)
...
async storeSession(data) {
    ...
    try {
        ...
        if (isJwtExpired(data.refreshToken)) {
            console.warn("🚨 RefreshToken expiré, rejeté.");
            return;
        }
        ...
        // 🔥 Force la synchronisation des tokens après login
        await api.syncRefreshToken();

        // 🔄 Programme automatiquement le rafraîchissement du JWT
        api.scheduleJwtRefresh();
    } catch (error) {
