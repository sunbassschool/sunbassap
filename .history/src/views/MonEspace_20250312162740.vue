...
apiBaseURL: "https://cors-proxy-37yu.onrender.com/https://script.google.com/macros/s/", // Suppression du proxy côté base URL
routes: {
...
fetchURL(route) {
  ...
  if (cacheWasUsed) {
    ...
    this.apiLoaded = true; // ⚡ Évite d'afficher la loading bar
    this.isDataReady = true;
  } else {
    console.log("⚠️ Pas de cache disponible, affichage du loader...");
    this.loading = true;
  }
  
  // 🔄 Lance un fetch en arrière-plan pour mettre à jour les données
  this.fetchUserData(!cacheWasUsed).finally(() => {
    this.loading = false;
    this.apiLoaded = true; // ✅ Assure que la loading bar disparaît après l'appel API
  });
...
async fetchUserData(forceUpdate = false) {
  ...
  try {
    ...
    if (!data.error) {
      ...
      this.user = data;
      this.isDataReady = true;
      this.apiLoaded = true; // 🚀 Ici, on marque l'API comme chargée
    } else {
