    ...
    📅 Réserver un cours maintenant
      ...
      <div v-if="!loading && planningData.length > 0" class="table-responsive mt-3">
        <table class="table table-hover shadow-sm" style="font-size: 1rem;">
          ...
          </thead><tbody v-if="sortedPlanningData.length > 0 && sortedPlanningData.some(row => row.lienReplay && row.lienReplay !== 'Pas de replay disponible' && row.lienReplay !== 'pb de connection')">
