...
async mounted() {
  ...
  // 🔄 Chargement des infos utilisateur (et tentative de refresh si expiré)
  await auth.loadUser();
  console.log("📌 Chargement du tableau de bord...");

  // ✅ Cas 1 : Aucun token (nouvel utilisateur ou jamais connecté)
  if (!auth.jwt && !auth.user) {
    console.warn("🟢 Aucun token présent, utilisateur jamais connecté.");
    this.cards = [
      {
    ...
    await this.fetchNote(); // ✅ Affiche la note locale si dispo
    this.isLoading = false;
    return;
  }

  // 🔄 Écouteur JWT mis à jour dans un autre onglet
  ...
  // 🗂️ Charge le cache (si dispo)
  const cacheWasUsed = await this.loadUserData();
  if (cacheWasUsed) this.isLoading = false;

  // 🚀 Si nécessaire → met à jour les données en arrière-plan
  ...
  // 📝 Récupération de la note (même si cache utilisé)
  setTimeout(async () => {
    await this.fetchNote();
  }, 300);

  // ⏱️ Rafraîchissement automatique des notes toutes les 10 minutes
  setInterval(() => {
