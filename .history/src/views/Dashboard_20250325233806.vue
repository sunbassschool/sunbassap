...
async fetchNote(forceRefresh = false) {
  ...
  const base = this.routes.GET;
  const query = `?route=getnote&jwt=${jwt}`;
  const fullTargetURL = encodeURIComponent(`https://script.google.com/macros/s/${base}${query}`);
  const proxyURL = `${this.apiBaseURL}${fullTargetURL}`;
