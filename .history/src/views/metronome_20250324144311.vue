...
stopMetronome() {
  ...
  // Annule la boucle d'animation et le timer
  cancelAnimationFrame(this.interval); // Annule la boucle requestAnimationFrame
  ...
  console.log("🔇 Métronome et tous les sons ont été arrêtés.");
},

stopAudio() {
  // Si l'AudioContext existe, arrête tous les sons en cours
  if (this.audioContext) {
    // On doit annuler toutes les sources audio en cours dans le contexte
    // Il peut être nécessaire d'arrêter chaque source si elles sont encore en train de jouer.
    this.audioContext.close().then(() => {
      console.log("🔇 AudioContext fermé et tous les sons arrêtés !");
    }).catch(err => {
      console.error("❌ Impossible de fermer l'AudioContext : ", err);
    });
  }
}
...
checkIfPlaying() {
  const isMetronomeActive = localStorage.getItem("isPlaying") === "true";
  this.isPlaying = isMetronomeActive; // Si le métronome était actif avant, on le reprend

  if (isMetronomeActive) {
    // Redémarre le métronome et la lecture du tempo
    this.startTimer();
    this.scheduleNextBeat();
  }
}
