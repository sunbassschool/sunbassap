...
export default {
  ...
  // Utilisation de l'ID et du prénom de l'admin comme fallback
  const idEleve = this.userData?.id || "admin-id";  // Utiliser un ID unique pour l'admin
  const prenom = this.userData?.prenom || "Admin";  // Utiliser le prénom "Admin" comme fallback
  ...
  const payload = {
    ...
    contenu: texte,
    id_eleve: idEleve,  // ID de l'admin ou de l'élève, à voir côté serveur
    prenom: prenom     // Le prénom de l'admin, à voir côté serveur
  };
