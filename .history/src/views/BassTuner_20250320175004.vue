  ...
  methods: {
    async startTuner() {
      try {
        ...
        biquadFilter.frequency.value = 150; // On garde que les basses fréquences
    ...
    detectFrequency() {
      ...
      // Tolérance globale pour toutes les notes : on peut ajuster ici
      const tolerance = 3; // Tolérance de 5 Hz pour toutes les notes

      let closestNote = null;

      // Comparer la fréquence détectée avec les notes fondamentales
      for (let note of this.notes) {
        // Si la fréquence détectée est dans la plage de tolérance spécifique à la note
        ...
        // Si la fréquence détectée est proche de l'harmonique (3x la fondamentale), la ramener à la fondamentale
        if (Math.abs(detectedFreq - note.frequency * 3) <= tolerance) {
          detectedFreq = note.frequency; // Divise la fréquence par 3 pour revenir à la fondamentale de la note
          closestNote = note;
          break;
        }
      }

      // Cas spécial pour la fréquence de 118.43 Hz (proche de E) :
      if (Math.abs(detectedFreq - 118.43) <= tolerance) {
        detectedFreq = 41.20; // Ramener cette fréquence à la fondamentale de E
        closestNote = { name: "E", frequency: 41.20 };
      }

      // Si la fréquence détectée est proche de 55.00 Hz (et on est sur E ou D)
      if (Math.abs(detectedFreq - 55.00) <= tolerance) {
        // Cas où la fréquence est confondue avec La (A), mais est liée à la corde E ou D
        if (this.currentNote === "E" || this.currentNote === "D") {
          detectedFreq = 41.20; // Ramener cette fréquence à la fondamentale de E
          closestNote = { name: "E", frequency: 41.20 };
        }
      }
