      ...
      <div class="control-card">
        <div class="card-header">
          <div class="control-group">
            <label><span v-if="!isEditingTempo" @click="startEditingTempo" class="editable-bpm">
...
<input 
  ...
  @blur="stopEditingTempo" 
  @keyup.enter="stopEditingTempo"
  min="20" max="300"
  class="bpm-input"
/> BPM</label>
...
<div class="slider-group">
  ...
  <label>
    <input type="checkbox" v-model="disableStrongBeat" />
    Temps fort
...
export default {
  ...
  return {
    tempo: parseInt(localStorage.getItem("userBpm") || "120"), // Récupère le BPM ou met 120 si non trouvé
    measure: parseInt(localStorage.getItem("userMeasure") || "4"), // Récupère la mesure ou met 4 si non trouvé
    subdivision: parseInt(localStorage.getItem("userSubdivision") || "1"), // Récupère la subdivision ou met 1 si non trouvé
...
,
  ...
  methods: {
    ...
    if (document.visibilityState === 'visible') {
      // Lorsque la page devient visible, on réactive le métronome
      if (this.isPlaying) {
        this.resumeAudioContext();
        this.scheduleNextBeat();
      }
    } else {
      // Lorsque la page devient invisible, on arrête le métronome
      this.stopMetronome();
    }
  },
    startEditingTempo() {
  ...
  stopEditingTempo() {
    ...
    // Enregistrer la nouvelle valeur du BPM dans localStorage
    localStorage.setItem('userBpm', this.tempo);
  },
  initAudioContext() {
  if (!this.audioContext) {
    // Initialiser l'AudioContext si ce n'est pas encore fait
...
,
startMetronome() {
  // Vérifie si le métronome est déjà en cours
  const isMetronomeActive = localStorage.getItem("isPlaying") === "true";
  
  if (isMetronomeActive) return; // Si le métronome est déjà actif, on ne le redémarre pas

  this.initAudioContext();
  this.resumeAudioContext();
  this.isPlaying = true;
  localStorage.setItem("isPlaying", "true"); // Sauvegarde de l'état dans localStorage
...
stopMetronome() {
  // Vérifie si le métronome est déjà arrêté
  if (this.isPlaying) {
    this.isPlaying = false;
    localStorage.setItem("isPlaying", "false"); // Sauvegarde de l'état dans localStorage

    // Annuler l'intervalle de beats
    cancelAnimationFrame(this.interval); // Annule le requestAnimationFrame
    clearInterval(this.timerInterval); // Annule l'intervalle du timer

    // Réinitialiser les paramètres
    this.nextNoteTime = 0;
    this.elapsedTime = 0;

    // Arrêter tous les sons en cours si un son est joué
    if (this.audioContext) {
      // On arrête les buffers audio en cours
      Object.values(this.soundBuffers).forEach(buffer => {
        if (buffer && buffer.stop) {
          buffer.stop(); // Arrêter le son en cours de lecture
        }
      });
    }

    // Réinitialiser les sources de sons
    this.soundBuffers = { strong: null, weak: null, sub: null };
  }
}
...
,
checkIfPlaying() {
  // Vérifie si le métronome est déjà en cours (via localStorage)
  const isMetronomeActive = localStorage.getItem("isPlaying") === "true";
  this.isPlaying = isMetronomeActive;

  // Si le métronome est actif, initialise l'AudioContext si ce n'est pas déjà fait
  if (isMetronomeActive) {
    // Initialiser l'AudioContext si ce n'est pas déjà fait
    if (!this.audioContext) {
      this.initAudioContext();  // Initialise l'AudioContext
    }

    // Lancer le timer et la séquence des beats
    this.startTimer();
    this.scheduleNextBeat();
  }
}
...
,
scheduleNextBeat() {
  if (!this.isPlaying || !this.audioContext) return; // Ajout d'une vérification ici pour éviter toute exécution lorsque le métronome est arrêté

  const now = this.audioContext.currentTime;
  while (this.nextNoteTime < now + 0.1) {
    this.playClick(); // Joue le clic
  ...
  // On s'assure que l'intervalle n'est pas redémarré après l'arrêt
  if (this.isPlaying) {
...
,
  ...
  measure(newVal) {
    localStorage.setItem('userMeasure', newVal);
  },
  subdivision(newVal) {
    localStorage.setItem('userSubdivision', newVal);
  },
    swingAmount(val) {
  ...
  async mounted() {
  // Vérifie si le métronome est déjà en cours (localStorage)
  this.checkIfPlaying();  // Vérifie si le métronome doit continuer ou non

  // Initialisation audio et autres composants
  document.body.style.overflow = "hidden";
  this.initAudioContext();
  this.loadSounds();

  // Ajouter un écouteur pour l'événement visibilitychange
...
beforeUnmount() {
  ...
  // Retirer l'écouteur d'événement visibilitychange
