...
import { getCache } from "@/utils/cacheManager"; // 📌 Vérification des données locales
...
onMounted(async () => {
  ...
  try {
    preventIndexedDBCleanup(); // ✅ Empêche le navigateur de supprimer IndexedDB

    // 🔄 Récupération du JWT via getToken()
    let jwt = await withTimeout(getToken(), 3000);
