...
export default {
  ...
  try {
    ...
    console.error("❌ Problème avec IndexedDB :", error);
  }
...
try {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = (event) => {
      ...
      reject("IndexedDB inaccessible");
    };

    request.onsuccess = (event) => {
      console.log("✅ IndexedDB déjà initialisée.");
      const db = event.target.result;

      // 📌 Vérifier l'intégrité en tentant une lecture
      let transaction = db.transaction(["authStore"], "readonly");
      let store = transaction.objectStore("authStore");
      let getRequest = store.get("test_key");
      ...
      getRequest.onerror = () => {
        console.warn("⚠️ IndexedDB semble corrompue, suppression...");
        db.close();
        indexedDB.deleteDatabase(dbName);
        this.createIndexedDB().then(resolve).catch(reject);
      };
    };

    request.onupgradeneeded = (event) => {
      console.log("🛠️ Mise à jour / création de IndexedDB.");
...
createIndexedDB() {
  return new Promise((resolve, reject) => {
    const dbName = "SunBassSchoolDB";
    const dbVersion = 1;
    let request = indexedDB.open(dbName, dbVersion);

    request.onerror = (event) => {
      console.error("🚨 Impossible d'initialiser IndexedDB :", event.target.errorCode);
      reject("Échec de la création de IndexedDB");
    };

    request.onsuccess = (event) => {
      console.log("✅ IndexedDB réinitialisée !");
      const db = event.target.result;
      let transaction = db.transaction(["authStore"], "readwrite");
      let store = transaction.objectStore("authStore");

      // 🔐 Stocker une clé test pour validation
      store.put({ id: "test_key", status: "ok" });

      resolve(true);
    };

    request.onupgradeneeded = (event) => {
      console.log("🛠️ Création de IndexedDB !");
      let db = event.target.result;
...
async storeSession(data) {
    ...
    try {
        const decoded = jwtDecode(data.jwt); // 🔥 Décodage du JWT
        ...
        const userRole = decoded.role || decoded.userRole || decoded["https://yourdomain.com/role"]; // Ajuste selon ton JWT
        console.log("🎯 Rôle détecté :", userRole);

        // ✅ Stocker le rôle récupéré
