  ...
  <button @click="resetTimer" class="reset-icon">
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
      <path d="M12 4V1L8 5l4 4V7c4 0 7 3 7 7s-3 7-7 7-7-3-7-7h2a5 5 0 1 0 5-5z"/>
          ...
          <div class="wake-lock-status" :class="{ active: isWakeLockActive }" :title="isWakeLockActive ? 'Écran maintenu actif' : 'Veille possible'">
  🔋
</div>
  ...
  try {
    if ('wakeLock' in navigator) {
      ...
      this.wakeLock.addEventListener('release', () => {
        console.log('🔓 Wake Lock relâché (releasing)');
      });
    }
  } catch (err) {
