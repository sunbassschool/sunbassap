...
async fetchFeedbacks() {
  ...
  try {
    ...
    if (result.feedbacks) {
      ...
      // 💬 Associer chaque feedback principal à ses réponses directes en utilisant ID_Cours
      this.feedbacks = principaux.reverse().map(fb => ({
        ...fb,
        reponses: all.filter(rep => rep.ID_Cours === fb.ID) // Filtrer les réponses liées au feedback via ID_Cours
      }));

      console.log("Feedbacks récupérés et associés avec leurs réponses :", this.feedbacks); // Log pour vérifier la structure
    } else {
