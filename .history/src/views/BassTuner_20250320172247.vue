    ...
    detectFrequency() {
      ...
      while (detectedFreq > 10) { // On arrête de diviser si la fréquence devient trop basse
        for (let note of notes) {
          // Vérification directe des notes fondamentales (dans un intervalle de 2 Hz)
      ...
      if (closestNote) {
        // Si la note détectée est stable par rapport à la précédente, on ne la change pas
        if (this.currentNote === closestNote.name || Math.abs(detectedFreq - this.lastFrequency) <= tolerance) {
          this.stableFrequencyCounter++;
        } else {
          this.stableFrequencyCounter = 0;
        }

        // Si la fréquence est stable pendant un certain nombre d'itérations, on "verrouille" la note
        if (this.stableFrequencyCounter > 5) {
