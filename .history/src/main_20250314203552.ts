...
async function initApp() {
  ...
  // ✅ Démarrer Vue immédiatement pour éviter un écran noir
  const app = createApp(App);
  app.use(createPinia());
  app.use(router);
  app.mount("#app");

  // ✅ Vérifier le logo sans bloquer
  checkAndFixLogo();

  // ✅ Attendre que IndexedDB soit prête AVANT de lancer `initAuth()`
  await initIndexedDB(); 
  await initAuth();

  // ✅ Masquer l'écran de chargement après l'init complète
  hideLoadingScreen();
}
