...
let indexedDBReady = false; // ✅ Empêche plusieurs accès simultanés
let indexedDBResetInProgress = false; // ✅ Évite suppression multiple

async function initIndexedDB(): Promise<void> {
  if (indexedDBReady) return console.log("✅ IndexedDB déjà initialisée.");
  if (indexedDBResetInProgress) return console.warn("⚠️ Réinitialisation en cours, attente...");

  console.log("🔄 Vérification d'IndexedDB...");

  let retryCount = 0;

  while (retryCount < 2) { // ✅ Réduction à 2 essais max
    try {
      const db = await openIndexedDB("AuthDB", 1);

      if (!db.objectStoreNames.contains("authStore")) {
        console.warn(`⚠️ Tentative ${retryCount + 1}/2 : 'authStore' est manquant, attente...`);
        await new Promise(resolve => setTimeout(resolve, 300)); // Petite pause réduite
        retryCount++;
        continue;
      }
  ...
  console.warn("⚠️ IndexedDB toujours corrompue après essais, suppression...");
