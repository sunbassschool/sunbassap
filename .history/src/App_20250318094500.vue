...
const appKey = ref(0); // 🔄 Clé pour forcer le rechargement UI
const authStore = useAuthStore();

onMounted(async () => {
  console.log("🔄 Restauration des tokens en cours...");

  // ✅ Timeout pour afficher un message si le chargement prend trop de temps
  ...
  try {
    ...
    if (!jwt) {
      ...
      // ✅ Forcer Vue à recharger l’UI après le refresh
      nextTick(() => {
        console.log("🔄 UI rechargée après refresh");
        appKey.value++; // 🔥 Change la clé pour forcer un re-render
      });
    }
  } catch (error) {
...
<template>
  ...
  <div v-else>
    <router-view :key="appKey" /> <!-- ✅ Clé dynamique pour forcer le re-render -->
  </div>
</template>
