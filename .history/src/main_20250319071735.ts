...
async function initApp() {
  ...
  // 3️⃣ Si localStorage est vide ou invalide, essayer IndexedDB
  if (!jwt) {
    console.log("🔄 Essai de récupération depuis IndexedDB...");
    
    const isDBReady = await verifyIndexedDBSetup();
    if (isDBReady) {
      await restoreTokensIfNeeded(); // Récupérer les tokens depuis IndexedDB
      jwt = await getValidToken(); // Vérifier à nouveau après récupération
    } else {
      console.warn("⚠️ IndexedDB non accessible !");
    }
  }

  // 4️⃣ Si JWT toujours invalide, tenter un refresh avec refreshToken
  ...
  // 5️⃣ Si JWT valide après refresh ou récupération, on met à jour IndexedDB
  if (jwt) {
    console.log("✅ JWT valide, l'utilisateur est connecté !");
    
    // Si IndexedDB est disponible, on synchronise pour éviter sa suppression future
    const isDBReady = await verifyIndexedDBSetup();
    if (isDBReady) {
