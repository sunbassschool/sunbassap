...
window.addEventListener("storage", async (event) => {
  if (event.key === "jwt" && !event.newValue) {
      console.warn("🚨 JWT manquant dans localStorage, tentative de récupération...");

      let restoredJwt = sessionStorage.getItem("jwt") || await getJWTFromIndexedDB();
      
      if (!restoredJwt) {
          restoredJwt = document.cookie.split("; ").find(row => row.startsWith("jwt="))?.split("=")[1];
      }

      if (restoredJwt) {
          console.log("🔄 [FIX] Réécriture du JWT dans localStorage.");
          localStorage.setItem("jwt", restoredJwt);
      } else {
          console.error("❌ Impossible de restaurer le JWT, risque de déconnexion !");
      }
  }
});
