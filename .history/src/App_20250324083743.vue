...
async function checkAuth() {
  ...
  try {
    ...
    if (dbReady) {
      await restoreTokensIfNeeded(); // IndexedDB est disponible, on peut tenter une restauration
    } else {
      console.warn("⚠️ IndexedDB non dispo, tentative de fallback sur localStorage.");
    }

    await authStore.loadUser(); // charge les infos avec ou sans restoreTokens

    if (!authStore.jwt && !jwt) {
      console.warn("🚨 Aucun JWT valide trouvé, redirection !");
      router.push("/login");
    } else {
      ...
      if (!prenom) {
        console.warn("⚠️ Aucun prénom trouvé, valeur par défaut.");
        prenom = "Utilisateur";
      } else {
