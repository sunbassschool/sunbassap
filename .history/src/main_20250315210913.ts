...
async function initApp() {
  ...
  const canRestoreTokens = await restoreTokensIfNeeded();

  if (!canRestoreTokens) {
    console.warn("⚠️ Aucun token valide, redirection forcée vers /login.");
    router.replace("/login");
    return;
  }

  console.log("🔄 Restauration des tokens en cours...");
  await restoreTokensToIndexedDB(); // ✅ Attendre la fin de la restauration avant de continuer
  ...
  if (jwt) {
    ...
    console.warn("⚠️ Aucun JWT trouvé après restauration, redirection vers login.");
    router.replace("/login");
  }
