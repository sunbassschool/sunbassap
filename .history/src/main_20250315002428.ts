...
async function initApp() {
  ...
  // ✅ Masquer l'application au début
  const appContainer = document.getElementById("app");
  if (appContainer) {
    appContainer.style.opacity = "0";
  }

  app.mount("#app"); // ✅ MONTER L'APP IMMÉDIATEMENT MAIS INVISIBLE
  ...
  // 🔄 Restaurer les tokens avant d'afficher l'application
  ...
  // ✅ Afficher l'application après la restauration des tokens
  setTimeout(() => {
    ...
    if (appContainer) {
      ...
      appContainer.style.opacity = "1"; // ✅ Réafficher proprement l'app
    }
  }, 500);
}
