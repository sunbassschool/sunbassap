...
async function initApp() {
  ...
  if (!isDBReady) {
    ...
    if (!jwt && !storedRefreshToken) {
      ...
      if (loadingScreen) loadingScreen.style.display = "none"; // Cache l'écran de chargement
      showLoginScreen(); // Affiche la page de connexion
      return;
    }
  ...
  if (jwt) {
    ...
    if (appContainer) {
      appContainer.style.transition = "opacity 0.5s ease-out"; // Animation de l'apparition de l'interface
      appContainer.style.opacity = "1"; // Afficher l'interface
    }
    return;
  }
