...
async function attemptTokenRefresh(storedRefreshToken: string, attempt = 1): Promise<any> {
  ...
  // 🚨 Vérifier si le token stocké est un JWT par erreur
  if (storedRefreshToken && storedRefreshToken.split(".").length === 3) {
    console.warn("⚠️ [ALERTE] Le token récupéré semble être un JWT au lieu d'un Refresh Token !");

    // 👉 On force la récupération d’un vrai refresh token
