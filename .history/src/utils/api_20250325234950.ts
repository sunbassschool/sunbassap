...
export async function refreshToken(): Promise<string | null> {
  ...
  try {
    ...
    // 🌐 URL
   const url = `https://cors-proxy-sbs.vercel.app/api/proxy?url=https://script.google.com/macros/s/AKfycbwEHhLoAVs9Lsn5HKfPnhB2gRAU2OBCeCTgTSK7CgV0ZMdqW_EQD2iyWZhCqc_UHLykog/exec?route=refresh&refreshtoken=${encodeURIComponent(storedRefreshToken)}`;
  ...
  data = await Promise.race([
    fetch(url, {
      ...
      headers: {
        ...
        "X-Requested-With": "XMLHttpRequest", // Ajoute ce header pour gérer CORS
      }
    }).then((response) => {
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      return response.json();
    }),
    new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), 35000)), // Timeout de 35 secondes
  ]);
