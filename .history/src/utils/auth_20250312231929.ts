...
async function refreshToken(): Promise<string | null> {
    if (isRefreshing) {
        console.warn("⚠️ Un rafraîchissement est déjà en cours.");
        return null;
    }

    isRefreshing = true; // 🔒 Empêcher d'autres appels en parallèle
    ...
    try {
        ...
        isRefreshing = false; // 🔓 Débloquer après la fin
        return data.jwt;
    } catch (error) {
        console.error("❌ Échec du refresh :", error);
        isRefreshing = false; // 🔓 Débloquer en cas d'erreur
        return forceLogout();
    }
}
