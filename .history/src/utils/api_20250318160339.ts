...
export async function getValidToken(): Promise<string | null> {
  ...
  let jwt = await getStoredJWT(); // ou une autre méthode pour récupérer le JWT

  // Vérification si le JWT est expiré ou invalide
  if (!jwt || isJwtExpired(jwt)) {
    console.warn("🚨 JWT invalide ou expiré, tentative de rafraîchissement...");

    // S'assurer que le refresh token est disponible
    const refreshToken = await getRefreshToken();
    if (!refreshToken) {
      console.error("❌ Aucun refresh token disponible");
      return null; // Si aucun refresh token n'est disponible, l'utilisateur doit se reconnecter
    }

    // Rafraîchir le JWT avec le refresh token
    try {
      const newJwt = await refreshToken(); // Appelle ta fonction de rafraîchissement ici
      if (!newJwt) {
        console.error("❌ Impossible de rafraîchir le JWT");
        return null;
      }

      console.log("✅ Nouveau JWT récupéré avec succès !");
      return newJwt; // Retourne le nouveau JWT valide
    } catch (error) {
      console.error("❌ Erreur lors du rafraîchissement du JWT :", error);
      return null; // Si le rafraîchissement échoue, l'utilisateur doit se reconnecter
    }
  }

  console.log("✅ JWT valide trouvé !");
  return jwt; // Si le JWT est encore valide, on le retourne
}
