...
export async function getValidToken(): Promise<string | null> {
  ...
  // 🛠️ Vérifier d'abord si un JWT et un refreshToken existent
  const jwtStored = await getToken();
  const refreshTokenStored = localStorage.getItem("refreshToken") || sessionStorage.getItem("refreshToken");

  console.log("📌 [DEBUG] JWT récupéré :", jwtStored);
  console.log("📌 [DEBUG] Refresh Token récupéré :", refreshTokenStored);

  // 🚨 Si aucun token n'existe, inutile d'aller plus loin
  if (!jwtStored && !refreshTokenStored) {
    console.warn("❌ Aucun JWT ni Refresh Token disponible !");
    return null;
  }

  let jwt: string | null = null;

  // 🔄 Vérifier et extraire le JWT
  if (jwtStored && typeof jwtStored === "object" && "value" in jwtStored) {
    jwt = (jwtStored as { value: string }).value;
  } else if (typeof jwtStored === "string") {
    jwt = jwtStored;
  }
  ...
  // 🚨 Si le JWT est expiré mais qu'on a un refreshToken, on tente un refresh
  if (refreshTokenStored) {
    ...
    if (refreshedToken) {
      ...
      const authStore = useAuthStore();
      authStore.setUserToken(refreshedToken);
      authStore.user = getUserInfoFromJWT(refreshedToken);

      return refreshedToken;
    }
  }

  console.error("❌ Aucun JWT valide disponible !");
  return null;
}
  ...
  if (!refreshToken) {
    ...
    await logoutUser(); // 🔥 Forcer la déconnexion propre
    return null;
  }
  ...
  try {
    ...
    await logoutUser(); // 🔥 Déconnexion forcée
    return null;
  }
}
...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    // ✅ Vérifie si un timer existe déjà avant de le démarrer
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      ...
      db.close();
      await new Promise((resolve) => setTimeout(resolve, 500)); // ⏳ Attente avant suppression
      await deleteDB("AuthDB");

      await new Promise((resolve) => setTimeout(resolve, 1000)); // ⏳ Attente avant recréation
      
      // ✅ Terminer le timer existant avant d'en démarrer un nouveau
      console.timeEnd("⏳ Temps total de création IndexedDB");
      delete (globalThis as any).__indexedDBTimerStarted; // Supprimer le flag
    ...
    console.timeEnd("⏳ Temps total de création IndexedDB"); // ✅ Assurer que le timer est bien terminé
    delete (globalThis as any).__indexedDBTimerStarted; // Nettoyer le flag
    return true;
  } catch (error) {
...
export async function getItemFromStore(storeName: string, key: string): Promise<string | null> {
  try {
    ...
    const db = await openDB("AuthDB", 1, {
      upgrade(db) {
        if (!db.objectStoreNames.contains(storeName)) {
          console.warn(`⚠️ Création du store ${storeName}...`);
    ...
    if (!db.objectStoreNames.contains(storeName)) {
      console.warn(`⚠️ Store ${storeName} absent après vérification.`);
      return null;
    }
...
export async function preventIndexedDBCleanup() {
  ...
  try {
    ...
    console.error("❌ Impossible de protéger IndexedDB :", error);

    // 🔄 Si IndexedDB est corrompu, la supprimer et forcer une nouvelle création
