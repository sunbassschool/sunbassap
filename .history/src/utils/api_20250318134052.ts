...
export async function getValidToken(): Promise<string | null> {
  ...
  try {
    ...
    if (newJwt) {
      ...
      // 🔄 Mise à jour du store avec le nouveau token et utilisateur
      authStore.setUserToken(newJwt);
      authStore.user = getUserInfoFromJWT(newJwt);

      // ✅ 🔄 Forcer le re-render de Vue sans recharger la page
      authStore.triggerRefresh(); 
      router.replace({ path: router.currentRoute.value.path, force: true });
