  ...
  return new Promise((resolve) => {
    ...
    request.onsuccess = () => {
      ...
      if (!db.objectStoreNames.contains("authStore")) {
        ...
        db.close();
        indexedDB.deleteDatabase("AuthDB").onsuccess = () => {
          console.log("🔄 IndexedDB supprimée, recréation en cours...");
          setTimeout(() => resolve(false), 500); // Attendre avant de réessayer
        };
      } else {
...
export async function refreshToken(): Promise<string | null> {
  ...
  try {
    ...
    if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
      ...
      // 🔄 Vérification de l'accessibilité de IndexedDB
      const isDBReady = await verifyIndexedDBSetup();
      if (isDBReady) {
        console.log("🔄 Mise à jour IndexedDB après refresh...");
        await storeTokenInIndexedDB(data.data.jwt, data.data.refreshToken);
      } else {
        console.warn("⚠️ IndexedDB non disponible, stockage impossible.");
      }
