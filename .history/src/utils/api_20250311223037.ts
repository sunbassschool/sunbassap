...
async function getRefreshTokenFromIndexedDB(): Promise<string | null> {
  return new Promise((resolve) => {
    const request = indexedDB.open("AuthDB"); // ✅ Bonne base de données

    request.onerror = () => {
      console.error("❌ [DEBUG] Erreur lors de l'ouverture d'IndexedDB.");
      resolve(null);
    };
    ...
    request.onsuccess = () => {
      ...
      // 🚨 Vérifier si l'object store "authStore" existe avant d'y accéder
      if (!db.objectStoreNames.contains("authStore")) {
        console.error("❌ [DEBUG] L'object store 'authStore' n'existe pas !");
        resolve(null);
        return;
      }
      ...
      getRequest.onsuccess = () => {
        if (getRequest.result) {
          console.log("✅ [DEBUG] Refresh Token récupéré depuis IndexedDB :", getRequest.result.value);
          resolve(getRequest.result.value);
        } else {
          console.warn("⚠️ [DEBUG] Aucun refresh token trouvé dans IndexedDB.");
          resolve(null);
        }
      };

      getRequest.onerror = () => {
        console.error("❌ [DEBUG] Erreur lors de la récupération du refreshToken depuis IndexedDB.");
        resolve(null);
      };
    };
  });
...
export async function getRefreshTokenFromDB(): Promise<string | null> {
  console.log("🔄 [DEBUG] Vérification du refresh token dans IndexedDB, LocalStorage, SessionStorage et Cookies...");

  // 1️⃣ 🔍 Vérification dans IndexedDB
  let refreshToken = await getRefreshTokenFromIndexedDB();
  if (refreshToken) {
      console.log("📌 [DEBUG] Refresh token trouvé dans IndexedDB :", refreshToken);
      return refreshToken;
  }

  // 2️⃣ 🔍 Vérification dans LocalStorage
  refreshToken = localStorage.getItem("refreshToken");
  if (refreshToken) {
      console.log("📌 [DEBUG] Refresh token trouvé dans LocalStorage :", refreshToken);
      return refreshToken;
  }

  // 3️⃣ 🔍 Vérification dans SessionStorage
  refreshToken = sessionStorage.getItem("refreshToken");
  if (refreshToken) {
      console.log("📌 [DEBUG] Refresh token trouvé dans SessionStorage :", refreshToken);
      return refreshToken;
  }

  // 4️⃣ 🔍 Vérification dans les Cookies
  const cookieToken = document.cookie.split("; ").find(row => row.startsWith("refreshToken="))?.split("=")[1];
  if (cookieToken) {
      console.log("📌 [DEBUG] Refresh token trouvé dans les Cookies :", cookieToken);
      return cookieToken;
  }

  console.warn("❌ [DEBUG] Aucun refresh token trouvé !");
  return null;
}
...
export async function getRefreshTokenDirect(): Promise<string | null> {
  try {
    console.log("🔍 [DEBUG] Accès direct à IndexedDB...");
    const db = await openDB("AuthDB", 1);
    const result = await db.get("authStore", "refreshToken");

    if (result) {
      console.log("📌 [DEBUG] Refresh token trouvé dans IndexedDB :", result);
      return result; // Directement la valeur, sans `.value`
    } else {
      console.warn("🚨 [DEBUG] Aucun refresh token trouvé en IndexedDB.");
      return null;
    }
  } catch (error) {
    console.error("❌ [DEBUG] Erreur lors de la lecture IndexedDB :", error);
    return null;
  }
}
...
export async function refreshToken(): Promise<string | null> {
  ...
  // 🛠 1️⃣ Vérification du Refresh Token dans tous les stockages
  ...
  if (!storedRefreshToken) {
    ...
    storedRefreshToken = await getRefreshTokenFromIndexedDB(); // ✅ On appelle la version corrigée

    if (!storedRefreshToken) {
      console.error("❌ Impossible de récupérer le refresh token. Déconnexion nécessaire.");
      return null;
    }
  ...
  if (storedRefreshToken.includes(".")) {
    console.warn("⚠️ [ALERTE] Le refresh token ressemble à un JWT. Vérifie s'il est bien stocké !");
  }

  console.log("📡 [DEBUG] Envoi du Refresh Token à l'API :", storedRefreshToken);

  try {
    // ✅ 2️⃣ Appel à l'API pour rafraîchir le JWT
    const response = await attemptTokenRefresh(storedRefreshToken);

    console.log("📡 [DEBUG] Réponse API reçue :", response);

    if (!response || !response.accessToken) {
      console.error("❌ [DEBUG] Impossible de rafraîchir le JWT, réponse API invalide :", response);
      await handleRefreshFailure();
      return null;
    }

    // ✅ 3️⃣ Stocker correctement le JWT après le refresh
    console.log("✅ Nouveau JWT reçu, stockage en cours...");
    localStorage.setItem("jwt", response.accessToken);
    sessionStorage.setItem("jwt", response.accessToken);
    await saveJWTToIndexedDB(response.accessToken);
...
async function attemptTokenRefresh(storedRefreshToken: string, attempt = 1): Promise<any> {
  if (attempt > MAX_REFRESH_ATTEMPTS) {
    console.error("🚨 [DEBUG] Échec du refresh après plusieurs tentatives !");
    throw new Error("Échec du rafraîchissement du token après plusieurs tentatives.");
  }

  let encodedToken = encodeURIComponent(storedRefreshToken);
  console.log(`📡 [DEBUG] Tentative ${attempt} - Requête envoyée avec Refresh Token : ${storedRefreshToken} (Encodé: ${encodedToken})`);

  let url = `https://cors-proxy-37yu.onrender.com/https://script.google.com/macros/s/AKfycbxc69Alhr0V0-DqrHblDJonvPKFy8PbjhEA6nA4sC4aFCTJOCKO9t0Q-uaApC_vcXgyHA/exec?route=refresh&refreshtoken=${encodedToken}`;
  ...
  try {
    ...
    console.log("📡 [DEBUG] Réponse JSON API :", data);
    ...
    console.error(`❌ [DEBUG] Tentative ${attempt} - Erreur lors du rafraîchissement du token :`, error.message);
    
    console.log("🔄 [DEBUG] Recherche d'un refreshToken alternatif dans IndexedDB...");
    const indexedDbRefreshToken = await getRefreshTokenFromDB();

    if (indexedDbRefreshToken) {
      console.log(`✅ [DEBUG] Tentative ${attempt + 1} - Refresh Token trouvé, nouvelle tentative...`);
      return attemptTokenRefresh(indexedDbRefreshToken, attempt + 1); // ✅ On passe `attempt + 1`
    } else {
      console.error("❌ [DEBUG] Aucun refreshToken valide trouvé après plusieurs tentatives !");
      throw new Error("Impossible de rafraîchir le token après plusieurs tentatives.");
    }
  }
}
...
export async function updateRefreshTokenInDB(newRefreshToken: string | null) {
  ...
  try {
    const db = await openDB("AuthDB", 1); // ✅ Assure-toi que `openDB` est bien défini
    const tx = db.transaction("authStore", "readwrite");
    const store = tx.objectStore("authStore");
    ...
    console.warn("⚠️ Erreur lors de l'enregistrement du Refresh Token dans IndexedDB :", err);
  }
}
