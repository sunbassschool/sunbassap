...
async function hasUserEverLoggedIn(): Promise<boolean> {
  ...
  // ⚠️ Empêcher une valeur corrompue ou non initialisée
  if (!userStatus || userStatus !== "true") {
    return false; // 🛑 Aucune connexion jamais enregistrée.
  }

  return true; // ✅ Un utilisateur a bien été connecté avant.
}
