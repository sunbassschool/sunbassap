...
export async function checkAndRestoreTokens(): Promise<"valid" | "expired" | "unauthenticated"> {
  ...
  if (!jwt || isJwtExpired(jwt)) {
    ...
    const newJwt = await authStore.refreshJwt(); // ✅ Triggers isRefreshingToken // ✅ Ajoute des parenthèses pour lever toute ambiguïté
    if (!newJwt) {
...
export async function refreshToken(): Promise<string | null> {
  ...
  try {
    ...
    // 🌐 URL
  const base = "https://script.google.com/macros/s/AKfycbyvFb607O-JB2ZGgY9Ei1IPdoIA_AjsgdcZ_oRlBc6z1w2gK3nMnq-Sp0R6VrUoNO5lIg/exec";
const query = `route=refresh&refreshtoken=${encodeURIComponent(storedRefreshToken)}`;
const fullURL = `${base}?${query}`;
const proxyURL = `https://cors-proxy-sbs.vercel.app/api/proxy?url=${encodeURIComponent(fullURL)}`;

console.log("🌐 URL de refresh construite :", proxyURL);
...
try {
  ...
  console.timeEnd("⏳ Durée du fetch de refresh");
      ...
      if (error instanceof Error && error.message === "Timeout") {
        ...
        try {
          data = await Promise.race([
            fetch(proxyURL, { method: "GET" }).then((response) => {
              if (!response.ok) throw new Error(`HTTP ${response.status}`);
              return response.json();
            }),
