...
async function getRefreshTokenFromIndexedDB(): Promise<string | null> {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open("AuthDB"); // Assure-toi que c'est bien "AuthDB" et pas une autre base

    request.onerror = () => {
      console.error("❌ [DEBUG] Erreur lors de l'ouverture de IndexedDB");
      resolve(null);
    };
    ...
    request.onsuccess = () => {
      ...
      // Vérifier si l'object store "tokens" existe
      if (!db.objectStoreNames.contains("tokens")) {
        console.error("❌ [DEBUG] L'object store 'tokens' n'existe pas !");
        resolve(null);
        return;
      }
