...
export async function checkAndRestoreTokens() {
  ...
  console.log("📌 [DEBUG] JWT trouvé :", jwt);
  console.log("📌 [DEBUG] Refresh Token trouvé :", refreshToken);

  if (!jwt && refreshToken) {
      console.warn("⚠️ JWT expiré ou manquant, tentative de rafraîchissement...");
      jwt = await refreshToken();
      if (jwt) {
          await saveTokens(jwt, refreshToken); // 🔥 On sauvegarde le nouveau JWT
      } else {
          console.error("❌ Impossible de récupérer un nouveau JWT !");
          return false;
      }
  }

  if (jwt) {
      console.log("✅ JWT valide :", jwt);
      localStorage.setItem("jwt", jwt);
      return true;
  } else {
      console.warn("❌ Aucun JWT disponible, l'utilisateur doit se reconnecter.");
      return false;
  }
}
