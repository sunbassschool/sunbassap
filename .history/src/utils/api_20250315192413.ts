...
export async function restoreTokensIfNeeded() {
  ...
  try {
    ...
    if (jwt) {
      console.log("✅ JWT trouvé dans IndexedDB, restauration en cours...");
      localStorage.setItem("jwt", jwt);
      sessionStorage.setItem("jwt", jwt);
      document.cookie = `jwt=${jwt}; Secure; SameSite=Strict; path=/`;
    } else {
      console.warn("⚠️ Aucun JWT trouvé en IndexedDB !");
    }

    if (refreshToken) {
      console.log("✅ Refresh Token trouvé, restauration en cours...");
      localStorage.setItem("refreshToken", refreshToken);
      sessionStorage.setItem("refreshToken", refreshToken);
      document.cookie = `refreshToken=${refreshToken}; Secure; SameSite=Strict; path=/`;
    } else {
      console.warn("⚠️ Aucun Refresh Token trouvé en IndexedDB !");
    }
  } catch (error) {
    console.error("❌ Erreur lors de la restauration des tokens :", error);
  }
