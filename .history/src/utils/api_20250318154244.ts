...
async function handleRefreshToken() {
  ...
  if (storedRefreshToken) {
    try {
      const response = await refreshToken(); // Appel à l'API de rafraîchissement du token
      if (response.status === "success") {
        // On récupère le nouveau JWT et refreshToken
        const newJwt = response.data.jwt;
        const newRefreshToken = response.data.refreshToken;

        // Mettre à jour les tokens dans le localStorage et sessionStorage
        localStorage.setItem("jwt", newJwt);
        sessionStorage.setItem("jwt", newJwt);
        localStorage.setItem("refreshToken", newRefreshToken);
        sessionStorage.setItem("refreshToken", newRefreshToken);

        console.log("✅ Nouveau JWT et Refresh Token récupérés et stockés !");
      } else {
        console.warn("⚠️ Échec du rafraîchissement du token : statut échoué");
        // Redirige vers la page de login si le rafraîchissement échoue
        router.replace("/login");
      }
    } catch (error) {
      console.error("❌ Erreur lors de la tentative de rafraîchissement du token :", error);
      // Redirige vers la page de login si une erreur se produit
      router.replace("/login");
    }
  } else {
