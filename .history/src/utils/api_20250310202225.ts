...
export async function getItemFromIndexedDB(key: string): Promise<any | null> {
  try {
    const db = await openDB("AuthDB", 1);
    const tx = db.transaction("authStore", "readonly");
    const store = tx.objectStore("authStore");
    ...
    console.log(`📌 [DEBUG] Récupération depuis IndexedDB : ${key} =`, result?.value ?? null);

    return result ? result.value : null;
  } catch (err) {
    console.warn(`⚠️ Impossible de récupérer ${key} depuis IndexedDB`, err);
    return null;
  }
}
...
export async function getToken(): Promise<string | null> {
  const jwtFromDB = await getJWTFromIndexedDB();
  const jwtFromStorage = localStorage.getItem("jwt") || sessionStorage.getItem("jwt");
  const jwtFromCookie = getCookie("jwt");

  console.log("📌 [DEBUG] JWT depuis IndexedDB :", jwtFromDB);
  console.log("📌 [DEBUG] JWT depuis localStorage/sessionStorage :", jwtFromStorage);
  console.log("📌 [DEBUG] JWT depuis les cookies :", jwtFromCookie);

  const finalJwt = jwtFromDB || jwtFromStorage || jwtFromCookie;

  if (!finalJwt) {
    console.warn("🔍 [getToken] Aucun JWT trouvé, vérification de firstLoginDone...");
    ...
    if (!firstLoginDB && !firstLoginCookie) {
      console.log("🚀 Première ouverture détectée, on attend que l'utilisateur se connecte !");
      return null; // 🔥 On ne force pas le logout
    }
  }

  console.log("🔍 [getToken] Token final retourné :", finalJwt || "Aucun JWT trouvé");
  return finalJwt || null;
}
...
export async function checkAndRestoreTokens() {
  ...
  if (!jwt && !refreshToken) {
    console.warn("⚠️ Aucun JWT ni RefreshToken trouvé ! Vérification de firstLoginDone...");
    ...
    if (!firstLoginDB && !firstLoginCookie) {
      ...
      // ✅ Marquer que l'utilisateur démarre une session neuve
      await saveItemToIndexedDB("firstLoginDone", "true");
      setCookie("firstLoginDone", "true", 365);
      
      return; // 🔥 On attend que l'utilisateur se connecte normalement
    }

    console.log("❌ JWT expiré et aucun refreshToken, l'utilisateur doit se reconnecter.");
    await logoutUser();
    return;
  }
...
async function getCookie(name: string): string | null {
...
export async function refreshToken() {
  ...
  if (!storedRefreshToken) {
    ...
    // ✅ Vérification si c'est une première ouverture AVANT d'appeler handleRefreshFailure()
    const firstLoginDB = await getItemFromIndexedDB("firstLoginDone");
    const firstLoginCookie = getCookie("firstLoginDone") === "true";
    ...
    await handleRefreshFailure(); // 🔴 Ce code ne s'exécutera PAS lors de la première ouverture
    return null;
  }
  ...
  try {
    ...
    if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
      ...
      // ✅ Dès qu'un utilisateur a un refresh token, on marque qu'il s'est connecté
      await saveItemToIndexedDB("firstLoginDone", "true");
      setCookie("firstLoginDone", "true", 365);
...
export async function updateTokens(newJwt: string | null, newRefreshToken: string | null) {
  ...
  try {
    ...
    // 🔥 Ajout : Marquer la première connexion
    console.log("🚀 Première connexion réussie, enregistrement de firstLoginDone !");
    await saveItemToIndexedDB("firstLoginDone", "true");
    setCookie("firstLoginDone", "true", 365);
  ...
  async (error) => {
    if (error.response && error.response.status === 401) {
      ...
      if (newJwt) {
        ...
        console.warn("🚨 Impossible de rafraîchir le token, vérification de firstLoginDone...");

        const firstLoginDB = await getItemFromIndexedDB("firstLoginDone");
        const firstLoginCookie = getCookie("firstLoginDone");

        if (!firstLoginDB && !firstLoginCookie) {
          console.log("🚀 Première ouverture détectée, on ne force PAS le logout.");
          return Promise.reject(error); // 🔥 On laisse juste l'erreur remonter sans déconnecter
        }

        console.log("❌ Échec du refresh après une connexion, l'utilisateur doit se reconnecter.");
        await handleRefreshFailure();
      }
    }
...
export async function handleRefreshFailure() {
  console.warn("🚨 Échec du refresh, vérification si on doit déconnecter l'utilisateur...");

  const firstLoginDB = await getItemFromIndexedDB("firstLoginDone");
  const firstLoginCookie = getCookie("firstLoginDone");

  if (!firstLoginDB && !firstLoginCookie) {
    console.log("🚀 Première ouverture détectée, on ne supprime pas les tokens.");
    return;
  }

  console.log("🔴 Suppression des tokens car le refresh a échoué après la première connexion.");
  await logoutUser();
}
