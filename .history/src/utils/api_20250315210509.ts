...
export async function restoreTokensIfNeeded() {
  console.log("🔄 Vérification de la validité des tokens avant toute tentative de refresh...");

  const jwt = localStorage.getItem("jwt") || sessionStorage.getItem("jwt");
  const refreshToken = localStorage.getItem("refreshToken") || sessionStorage.getItem("refreshToken");

  if (!jwt && !refreshToken) {
    console.warn("⚠️ Aucun JWT ni Refresh Token disponible. Redirection vers login.");
    return false;
  }
  ...
  // Si le JWT est expiré mais qu'un refreshToken est disponible, on peut tenter un refresh
  if (refreshToken) {
    console.log("🔄 JWT expiré, mais refreshToken présent. On peut tenter un refresh.");
    return true;
  }

  console.warn("❌ Impossible de refresh : Aucun refreshToken disponible.");
  return false;
}
...
export async function checkIndexedDBStatus(): Promise<void> {
  try {
    ...
    const jwt = await db.get("authStore", "jwt"); // 🔄 Uniformisation avec db.get()
