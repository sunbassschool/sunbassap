...
export async function getRefreshTokenFromDB(): Promise<string | null> {
  ...
  // Si aucun token trouvé dans localStorage ou sessionStorage, on cherche directement dans IndexedDB
  if (!storedRefreshToken) {
    try {
      const db = await openDB("AuthDB", 1);
      const tx = db.transaction("authStore", "readonly");
      ...
      if (result?.value) {
        storedRefreshToken = result.value;
        console.log("📦 Refresh token récupéré depuis IndexedDB :", storedRefreshToken);

        // 🔄 Synchronisation des tokens dans localStorage et sessionStorage
