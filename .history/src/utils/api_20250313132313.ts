...
interface TokenObject {
  jwt?: string;
  [key: string]: any;  // Autres propriétés possibles dans tokenObj
}

export async function getToken(): Promise<string | null> {
  // Modification ici : tokenObj peut être un objet, une chaîne ou null
  let tokenObj: TokenObject | string | null = await getJWTFromIndexedDB();  // 🔍 IndexedDB en priorité
  console.log("📌 [DEBUG] JWT brut depuis IndexedDB :", tokenObj);

  let token: string | null = null;

  // ✅ Si tokenObj est un objet avec une propriété `jwt`, on l'extrait
  if (tokenObj && typeof tokenObj === 'object' && 'jwt' in tokenObj) {
    token = tokenObj.jwt;  // Si c'est un objet, on utilise tokenObj.jwt
  } else if (typeof tokenObj === 'string') {
    token = tokenObj;  // Si c'est une chaîne, on l'utilise directement
  }

  // Si aucun token n'est trouvé dans IndexedDB, on vérifie localStorage et sessionStorage
  if (!token) {
  ...
  // Si le token est toujours absent, on essaie de le récupérer depuis les cookies
  if (!token) {
  ...
  return token || null; // Retourne un string ou null
}
...
export function getFromIndexedDB(key: string): Promise<any> {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('myDatabase', 1);  // Ouvre la base de données IndexedDB
    request.onsuccess = function(event) {
      // Vérification que event.target n'est pas null
      const db = event.target ? (event.target as IDBRequest).result : null;
      if (!db) {
        reject('Erreur : IndexedDB non accessible');
        return;
      }
      
      const transaction = db.transaction('storeName', 'readonly');  // Transaction en lecture seule
      ...
      getRequest.onerror = function() {
        reject('Erreur lors de la récupération depuis IndexedDB');
      };
    };
...
export function saveToIndexedDB(key: string, value: any): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = function(event) {
      ...
      const transaction = db.transaction('storeName', 'readwrite');  // Transaction en lecture-écriture
      const store = transaction.objectStore('storeName');
...
// Modification de la fonction pour accepter un argument
...
export async function getValidToken(): Promise<string | null> {
  ...
  // 🔧 Extraction du token si `getToken()` renvoie un objet ou une chaîne de caractères
  let jwt: string | null = null;

  // Si jwtData est un objet avec la propriété 'value', on utilise jwtData.value
  if (typeof jwtData === 'object' && jwtData?.value) {
    jwt = jwtData.value;
  } else if (typeof jwtData === 'string') {
    // Sinon, on considère que jwtData est déjà le token brut
    jwt = jwtData;
  }
...
export async function syncRefreshToken() {
  ...
  try {
    ...
    if (!dbToken && !localToken && !cookieToken) {
      ...
      await logoutUser(); // Forcer la déconnexion seulement si le JWT est aussi expiré
      isSyncing = false;
      return;
    }
