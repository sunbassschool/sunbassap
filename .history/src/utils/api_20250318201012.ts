...
export async function getValidToken(): Promise<string | null> {
  ...
  // Vérification de l'existence du JWT
...
// Générer un JWT expiré à partir d'un payload de base
function createExpiredJwt() {
  const payload = {
      email: "test@example.com",
      role: "user",
      exp: Math.floor(Date.now() / 1000) - 60  // Expire dans 60 secondes
  };

  const secret = 'yourSecretKey';  // La clé secrète pour signer le JWT
  const header = {
      alg: "HS256",
      typ: "JWT"
  };

  // Encodage du JWT en base64url
  const base64Url = (str) => {
  ...
  const signature = base64Url(encodedHeader + "." + encodedPayload);  // Simule la signature (normalement, tu signes avec ta clé secrète)

  return `${encodedHeader}.${encodedPayload}.${signature}`;
}

// Crée un JWT expiré et le stocke dans localStorage
const expiredJwt = createExpiredJwt();
localStorage.setItem("jwt", expiredJwt);
localStorage.setItem("refreshToken", "dummyRefreshToken");  // Token fictif pour test

console.log("JWT expiré simulé stocké dans localStorage :");
