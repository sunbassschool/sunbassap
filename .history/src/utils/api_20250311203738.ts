...
export async function checkAndRestoreTokens() {
  ...
  if (storedRefreshToken) {
      console.log("🔄 [FIX] Réécriture du refresh token dans localStorage, sessionStorage, IndexedDB et Cookies.");
      localStorage.setItem("refreshToken", storedRefreshToken);
      sessionStorage.setItem("refreshToken", storedRefreshToken);
      await saveRefreshTokenToIndexedDB(storedRefreshToken);

      // 🔄 Mise à jour du cookie (expire en 30 jours)
      document.cookie = `refreshToken=${storedRefreshToken}; path=/; max-age=${30 * 24 * 60 * 60}; Secure; HttpOnly`;
  } else {
  ...
  if (!jwt || isJwtExpired(jwt)) {
      ...
      if (newJwt) {
          console.log("✅ Nouveau JWT récupéré et stocké partout !");
          localStorage.setItem("jwt", newJwt);
          sessionStorage.setItem("jwt", newJwt);
          await saveJWTToIndexedDB(newJwt);

          // 🔄 Mise à jour du cookie (expire à l'expiration du JWT)
          const jwtExpirationTime = getJwtExpirationTime(newJwt); // Fonction à définir pour récupérer l'expiration
          document.cookie = `jwt=${newJwt}; path=/; max-age=${jwtExpirationTime}; Secure; HttpOnly`;
      } else {
...
export async function saveRefreshTokenToIndexedDB(refreshToken: string): Promise<void> {
    await verifyIndexedDBSetup();  // Vérifie qu'IndexedDB est bien accessible
    ...
    try {
        ...
        console.log("✅ Refresh token stocké dans IndexedDB :", refreshToken);
    } catch (error) {
        console.error("❌ Erreur lors du stockage du refresh token en IndexedDB :", error);
    }
}
...
async function handleRefreshFailure() {
  console.warn("🚨 [DEBUG] handleRefreshFailure() appelé. Suppression des tokens en cours...");
  localStorage.removeItem("refreshToken");
  sessionStorage.removeItem("refreshToken");
  await getRefreshTokenFromIndexedDB();

  console.warn("🔄 [DEBUG] Redirection vers la page de connexion...");
  window.location.href = "/login"; // Rediriger l'utilisateur proprement
}
