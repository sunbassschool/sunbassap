...
export async function getToken(): Promise<string | null> {
  // 1. Rapide : localStorage/sessionStorage
  ...
  // 3. IndexedDB en fallback final (cas rare)
  if (!token) {
    const fromDb = await getJWTFromIndexedDB();
    token = typeof fromDb === "object" ? fromDb.jwt ?? null : fromDb;
  }
...
export function isJwtExpired(token: any): boolean {
  console.warn("⚠️ FORÇAGE DEBUG : JWT toujours considéré comme expiré !");
  return true;
}
...
export async function refreshToken(): Promise<string | null> {
  ...
  try {
    ...
    console.log("🧾 IndexedDB.refreshToken :", fromDb);

    // ⏱ Vérif expiration refreshToken côté client
    ...
    try {
      ...
      console.log("🔗 URL envoyée au proxy :", url);
console.log("📬 Headers envoyés automatiquement par le navigateur :", {
