...
export let isRefreshing = false; // Ajoute cette ligne en haut de api.ts
...
export async function resetIndexedDB(): Promise<void> {
  ...
  try {
    ...
    if (!dbs.some(db => db.name === "AuthDB")) {
      console.log("✅ AuthDB n'existe pas, aucune suppression nécessaire.");
      return;
    }
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      deleteRequest.onerror = () => {
        console.error("❌ Erreur lors de la suppression de IndexedDB :", deleteRequest.error);
