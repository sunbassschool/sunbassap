...
export async function getRefreshTokenFromDB() {
  ...
  try {
    ...
    console.log("✅ IndexedDB est accessible après mise en veille.");
    ...
    if (refreshTokenEntry?.value) {
      ...
      // 🔄 Vérifier et restaurer le refreshToken dans les autres stockages
const storedRefreshToken = refreshTokenEntry.value;
...
if (!cookies) {
  ...
  console.log("🍪 Refresh token restauré dans les cookies.");
    ...
    console.error("❌ IndexedDB inaccessible après mise en veille :", error);
    return null;
  }
  // 🍪 **Vérifie dans les cookies en dernier recours**
  const cookies = document.cookie.split("; ");
  const refreshTokenCookie = cookies.find(row => row.startsWith("refreshToken="));
  if (refreshTokenCookie) {
    const refreshToken = refreshTokenCookie.split("=")[1];
    console.log("🍪 Refresh token récupéré des cookies :", refreshToken);

    // 🔄 **Le restaurer immédiatement dans IndexedDB et localStorage !**
    await updateRefreshTokenInDB(refreshToken);

    return refreshToken;
  }
  // ⚠️ Supprimé : inutile d'ouvrir à nouveau la base
  // const db = await openDB("AuthDB", 1); ❌
...
export function scheduleJwtRefresh() {
  const refreshInterval = /Mobi|Android/i.test(navigator.userAgent) ? 2 * 60 * 1000 : 10 * 60 * 1000;
  ...
  setTimeout(async () => {
    ...
    if (!newJwt) {
      console.error("❌ Refresh échoué, l'utilisateur sera déconnecté.");
      await logoutUser();
    } else {
