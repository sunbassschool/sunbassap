...
export async function restoreTokensIfNeeded(): Promise<void> {
  ...
  if (!jwt || !refreshToken) {
    console.warn("⚠️ Tokens absents de localStorage/sessionStorage, récupération depuis IndexedDB...");
  ...
  console.log("📥 Restauration immédiate des tokens dans localStorage et sessionStorage...");

  // ✅ Stocker immédiatement dans LocalStorage & SessionStorage
  ...
  // ✅ Vérification après stockage pour éviter toute incohérence
  if (!localStorage.getItem("jwt") || !sessionStorage.getItem("jwt")) {
    console.error("❌ Échec de la restauration des tokens dans localStorage/sessionStorage !");
    return;
  }

  // ✅ Synchronisation globale des tokens
  console.log("🔄 Synchronisation des tokens avec `updateTokens()`...");
  await updateTokens(jwt, refreshToken);

  console.log("✅ Restauration complète des tokens !");
}
