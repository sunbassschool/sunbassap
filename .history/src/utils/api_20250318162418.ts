...
export async function refreshToken(): Promise<string> {
  ...
  isRefreshing = new Promise<string>((resolve, reject) => {
    ...
    // Ajouter le reject pour qu'il soit disponible
    reject("Aucun refresh token trouvé.");
  });
...
export async function handleRefreshToken() {
  ...
  if (storedRefreshToken) {
    try {
      ...
      const responseData = JSON.parse(response); // Parse la réponse si elle est sous forme de string
