...
async function tryFetchWithTimeout(url: string, timeoutMs: number = 15000): Promise<any> {
...
export async function refreshToken(): Promise<string | null> {
  ...
  try {
    ...
    let data;
    try {
      const tryFetchWithTimeout = () => Promise.race([
      ...
      data = await tryFetchWithTimeout();
    } catch (error) {
    ...
    if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
      ...
      // 🧠 Mise à jour IMMEDIATE du token AVANT tout await
      ...
      // 🧼 Ensuite : sync (y compris IndexedDB)
