...
export async function checkAndRestoreTokens() {
  ...
  if (storedRefreshToken) {
      console.log("🔄 [FIX] Réécriture du refresh token dans localStorage, sessionStorage et IndexedDB.");
      localStorage.setItem("refreshToken", storedRefreshToken);
      sessionStorage.setItem("refreshToken", storedRefreshToken);
      await saveRefreshTokenToIndexedDB(storedRefreshToken); // 🔄 Mise à jour dans IndexedDB
  } else {
  ...
  if (!jwt || isJwtExpired(jwt)) {
      ...
      if (newJwt) {
          ...
          await saveJWTToIndexedDB(newJwt); // 🔄 Mise à jour dans IndexedDB
      } else {
