...
export async function resetIndexedDB(): Promise<void> {
  if (indexedDBResetInProgress) {
    console.warn("⚠️ Réinitialisation déjà en cours, annulation...");
    return;
  }

  indexedDBResetInProgress = true; // ✅ Empêcher double suppression
  ...
  await new Promise(resolve => setTimeout(resolve, 500)); // ✅ Pause pour éviter un accès trop rapide
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = function (event) {
      ...
      indexedDBResetInProgress = false; // ✅ Réinitialisation terminée
      resolve();
    };

    request.onerror = function (event) {
      console.error("❌ Erreur lors de la réinitialisation d'IndexedDB :", event);
      indexedDBResetInProgress = false; // ✅ Réinitialisation échouée
      reject(event);
    };
  });
}
...
export async function checkAndRestoreTokens(): Promise<boolean> {
  ...
  const firstTimeUser = !localStorage.getItem("hasLoggedInBefore");
  ...
  if (!storedRefreshToken) {
    ...
    // ✅ Si c'est la première fois, on ne déconnecte pas l'utilisateur
    if (firstTimeUser) {
      console.log("🚀 Première ouverture détectée, aucun token encore stocké.");
      return false; // ✅ Permet d'afficher /login sans forcer un logout
    }
  ...
  // ✅ Marquer que l'utilisateur s'est déjà connecté une fois
