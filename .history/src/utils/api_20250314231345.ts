...
export async function getJWTFromIndexedDB(): Promise<string | null> {
  try {
    ...
    // ✅ Vérifier si `authStore` existe AVANT d’essayer d’accéder aux données
    if (!db.objectStoreNames.contains("authStore")) {
...
export async function checkAndRestoreTokens(): Promise<boolean> {
  ...
  // ✅ Empêcher les actions inutiles si l'utilisateur n'a jamais eu de session
  let hadSessionBefore = !!localStorage.getItem("jwt") || !!sessionStorage.getItem("jwt") || !!await getRefreshTokenFromDB();
  
  if (!hadSessionBefore) {
    console.warn("🚨 Aucun token trouvé, utilisateur jamais connecté. Pas de déconnexion nécessaire.");
    return false; // L'utilisateur est simplement "non connecté", pas besoin de logout
  }

  if (localStorage.getItem("session_expired") === "true") {
    console.warn("🚨 La session a expiré précédemment, arrêt de la récupération des tokens.");
    return false;
  }
  ...
  // 🔄 Si JWT est valide, on continue normalement
  if (jwt && !isJwtExpired(jwt)) {
    console.log("✅ JWT encore valide, pas de déconnexion forcée.");
    return true;
  }

  // 🚨 Aucun refreshToken disponible → L'utilisateur doit se reconnecter
  if (!storedRefreshToken) {
    console.warn("❌ Aucun refreshToken trouvé, et le JWT est expiré. Déconnexion nécessaire.");
    await logoutUser();
    window.dispatchEvent(new Event("auth-checked")); // ✅ Signal pour afficher le login
    return false;
  }

  // ✅ Tenter de rafraîchir le JWT
  try {
...
export async function logoutUser() {
  ...
  try {
    ...
    setTimeout(() => {
      console.log("🔄 Redirection vers la page d'introduction...");
      ...
      router.replace("/intro"); // 🔥 Redirection initiale sans recharger la page

      // ⏳ Petite attente avant de rediriger vers /login
      setTimeout(() => {
        console.log("🔄 Redirection vers la page de connexion...");
      
      }, 1000); // Attente de 1 seconde avant de rediriger vers /login
    }, 2500);
