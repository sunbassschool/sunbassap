...
export async function resetIndexedDB(): Promise<void> {
  ...
  // 🛑 Fermer toutes les connexions ouvertes avant suppression
  indexedDB.databases().then((databases) => {
    databases.forEach((dbInfo) => {
      if (dbInfo.name === "AuthDB") {
        console.warn("🔌 Fermeture des connexions à AuthDB...");
        indexedDB.deleteDatabase("AuthDB");
      }
    });
  });

  // ✅ Attendre un court délai pour s'assurer de la suppression
...
export async function initIndexedDB(): Promise<void> {
  ...
  try {
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      ...
      // 🛑 On attend un instant pour éviter un accès immédiat
      await new Promise(resolve => setTimeout(resolve, 500));
