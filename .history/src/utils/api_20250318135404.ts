...
export async function getRefreshTokenFromDB(): Promise<string | null> {
  ...
  try {
    ...
    // ✅ Vérifier si l'object store "authStore" existe
    if (!db.objectStoreNames.contains("authStore")) {
      console.warn("⚠️ 'authStore' absent, recréation de IndexedDB...");
      await verifyIndexedDBSetup(); // 🔄 Recréer la base si nécessaire
      return null; // Éviter d'accéder à un store inexistant
    }
...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      ...
      // ✅ Vérifier si IndexedDB est toujours utilisé
      try {
        await deleteDB("AuthDB");
        console.log("✅ IndexedDB supprimé avec succès !");
      } catch (deleteError) {
        console.error("❌ Erreur lors de la suppression de IndexedDB :", deleteError);
      }
      ...
      // ✅ **Restauration des tokens après la recréation de la DB**
      setTimeout(async () => {
