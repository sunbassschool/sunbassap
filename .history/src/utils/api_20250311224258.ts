...
async function attemptTokenRefresh(storedRefreshToken: string, attempt = 1): Promise<any> {
  console.log("📌 [DEBUG] Avant correction - Token utilisé :", storedRefreshToken);

  // 🚨 Vérifions si le token ressemble à un JWT (ce qui est un problème)
  if (storedRefreshToken && storedRefreshToken.split(".").length === 3) {
    console.warn("⚠️ [ALERTE] Le token récupéré ressemble à un JWT. On devrait avoir un Refresh Token !");
    
    // 👉 Forcer la récupération d’un vrai refresh token
    const correctedToken = await getRefreshTokenFromIndexedDB();

    if (correctedToken && correctedToken.split(".").length !== 3) {
      console.log("✅ [DEBUG] Correction appliquée - Utilisation du Refresh Token :", correctedToken);
      storedRefreshToken = correctedToken;
    } else {
      console.error("❌ [DEBUG] Impossible de corriger le Refresh Token. Déconnexion...");
      return logoutUser();
    }
  }

  console.log("📡 [DEBUG] Token réellement envoyé à l'API :", storedRefreshToken);

  // 🔄 Effectuer l'appel API avec le bon Refresh Token
  const url = `https://cors-proxy-37yu.onrender.com/https://script.google.com/macros/s/...&refreshtoken=${encodeURIComponent(storedRefreshToken)}`;
  try {
    ...
    console.error("❌ [DEBUG] Erreur lors du rafraîchissement du token :", error.message);
    return logoutUser();
  }
}
