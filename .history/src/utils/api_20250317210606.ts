...
// Fonction pour mettre à jour les tokens dans tous les stockages
export async function updateTokens(jwt: string, refreshToken: string) {
  console.log("🔄 Mise à jour des tokens...");

  // Vérifier si le refresh token a changé
  const oldRefreshToken = await getRefreshTokenFromDB();
  ...
  // Mise à jour unifiée
  await Promise.all([
    saveToIndexedDB("jwt", jwt),
    saveToIndexedDB("refreshToken", refreshToken),
    localStorage.setItem("jwt", jwt),
    localStorage.setItem("refreshToken", refreshToken),
    sessionStorage.setItem("jwt", jwt),
    sessionStorage.setItem("refreshToken", refreshToken),
