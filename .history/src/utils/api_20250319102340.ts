...
async function restoreTokensAfterDBReset() {
  ...
  try {
    ...
    if (jwt) {
      store.put({ id: "jwt", token: jwt });
    }
    if (refreshToken) {
      store.put({ id: "refreshToken", token: refreshToken });
    }

    console.log("✅ Tokens restaurés avec succès dans IndexedDB !");
  } catch (error) {
    ...
    request.onsuccess = async () => {
      ...
      if (!db.objectStoreNames.contains("authStore")) {
        ...
        try {
          ...
          // ✅ On retente la création d'IndexedDB
          const isRecreated = await verifyIndexedDBSetup();

          if (isRecreated) {
            console.log("🔄 IndexedDB recréée, tentative de restauration des tokens...");
...
export async function refreshToken(): Promise<string | null> {
  if (isRefreshing) {
    ...
    return isRefreshing; // Si une tentative de rafraîchissement est déjà en cours, on retourne la promesse existante
  }
  ...
  try {
    ...
    if (!storedRefreshToken) {
      ...
      return null;  // Retourne null ici en cas de problème
    }
    ...
    if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
      ...
      // Synchronisation des tokens dans tous les stockages
    ...
    await handleRefreshFailure();
    return null;  // Retourne null ici en cas d'erreur
  } finally {
