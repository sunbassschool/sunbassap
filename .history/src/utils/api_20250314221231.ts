...
export async function getJWTFromIndexedDB(): Promise<string | null> {
  try {
    ...
    if (result) {
      console.log("✅ JWT récupéré depuis IndexedDB :", result.value);
      return result.value;
    } else {
      console.warn("⚠️ JWT absent d'IndexedDB, tentative de restauration depuis LocalStorage...");
      return localStorage.getItem("jwt") || null;
    }
  } catch (error) {
    console.error("❌ Erreur d'accès à IndexedDB, récupération depuis LocalStorage :", error);
    return localStorage.getItem("jwt") || null;
  }
}
...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    const db = await openDB("AuthDB", 3); // 🔥 Toujours utiliser la version la plus récente

    if (!db.objectStoreNames.contains("authStore")) {
      console.warn("⚠️ 'authStore' est manquant, tentative de recréation...");

      // 🔄 Fermeture propre avant suppression
      db.close();

      // 🕐 Attendre un peu avant suppression (évite les erreurs de blocage)
      await new Promise((resolve) => setTimeout(resolve, 500));

      console.log("🗑️ Suppression de IndexedDB...");
      await deleteDB("AuthDB");
      console.log("✅ IndexedDB supprimée avec succès.");

      // 🕐 Attendre avant recréation (évite un conflit avec l'ancien cache)
      await new Promise((resolve) => setTimeout(resolve, 500));

      console.log("🚀 Réouverture de IndexedDB...");
      await openDB("AuthDB", 3, {
        upgrade(db) {
          console.log("🛠️ Création de 'authStore'...");
