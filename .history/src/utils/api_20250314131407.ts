...
interface UserInfo {
  email: string;
  prenom?: string; // Facultatif si pas toujours nécessaire
}
...
export async function restoreUserInfo(): Promise<UserInfo | null> {
  ...
  if (email) {
    ...
    return { email, prenom: prenom || undefined }; // Retourne un objet valide
  }
  ...
  try {
    ...
    const restoredEmail = emailData?.value || null;
    const restoredPrenom = prenomData?.value || null;

    if (restoredEmail) {
      localStorage.setItem("email", restoredEmail);
      sessionStorage.setItem("email", restoredEmail);
    }

    if (restoredPrenom) {
      localStorage.setItem("prenom", restoredPrenom);
      sessionStorage.setItem("prenom", restoredPrenom);
    }

    if (restoredEmail) {
      console.log("✅ Infos utilisateur restaurées !");
      return { email: restoredEmail, prenom: restoredPrenom || undefined };
    }
  } catch (error) {
    console.error("❌ Erreur lors de la restauration des infos utilisateur :", error);
  }

  return null; // Retourne `null` si aucun email trouvé
}
...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    ...
    // ✅ Si l'object store est absent, on l'ajoute sans supprimer la DB
    if (!db.objectStoreNames.contains("authStore")) {
      console.warn("⚠️ 'authStore' est manquant, création en cours...");
      db.close();

      // ✅ Ajouter l'object store SANS supprimer la base
...
export async function preventIndexedDBCleanup() {
  ...
  try {
    // Vérifie et crée la DB et l'object store "authStore" si besoin
    const ready = await verifyIndexedDBSetup();
    if (!ready) {
      console.warn("⚠️ IndexedDB non prête, protection annulée.");
      return;
    }

    // Ouvre la DB en version 2
    let db = await openDB("AuthDB", 2);
    // Si l'object store "authStore" n'existe pas, forcer une upgrade vers la version 3 pour le créer
    if (!db.objectStoreNames.contains("authStore")) {
      console.warn("⚠️ 'authStore' introuvable lors de la protection, création forcée...");
      db.close();
      db = await openDB("AuthDB", 3, {
        upgrade(db) {
          if (!db.objectStoreNames.contains("authStore")) {
            db.createObjectStore("authStore", { keyPath: "key" });
            console.log("✅ 'authStore' créé lors de la protection !");
          }
        },
      });
    }
...
// ✅ Appeler cette fonction **uniquement après une connexion réussie**
function markUserAsLoggedIn() {
...
export async function syncRefreshToken() {
  ...
  try {
    ...
    if (!dbToken && !localToken && !cookieToken) {
      ...
      if (!userExists) {
        console.log("🚀 Aucun utilisateur n'a jamais été connecté, pas de reset ni de logout.");
        isSyncing = false;
        return;
      }
      ...
      // 🔴 Forcer la déconnexion **uniquement si un utilisateur s'était déjà connecté**
      if (userExists) {
        console.log("🔴 Déconnexion forcée car un utilisateur a déjà été connecté.");
        await logoutUser();
      }
...
export async function scheduleJwtRefresh() {
  if (window.jwtRefreshScheduled) return;

  const jwt = await getToken();
  if (!jwt || isJwtExpired(jwt)) {
    console.warn("🚨 Aucun JWT valide trouvé, tentative immédiate de refresh.");
    await refreshToken();
    return;
  }
  ...
  const now = Date.now();
  const delay = exp - now - 2 * 60 * 1000;

  if (delay < 0) {
    console.warn("⏳ JWT déjà expiré, tentative de refresh...");
    await refreshToken();
  } else {
    console.log(`⏳ Refresh planifié dans ${Math.round(delay / 1000)} secondes`);
    setTimeout(async () => {
      await refreshToken();
      window.jwtRefreshScheduled = false;
      scheduleJwtRefresh();
    }, delay);
  }
}
