...
// Fonction pour récupérer et valider le token JWT, ou tenter de le rafraîchir si nécessaire
export async function getValidToken(): Promise<string | null> {
  // 1. Vérification du JWT dans les différents stockages (localStorage, sessionStorage, IndexedDB, cookies)
  const token = await storageManager.getTokenFromAllStorages("jwt");

  // 2. Si le JWT est trouvé et valide, retourne le token
  if (token && !isJwtExpired(token)) {
    console.log("✅ JWT valide trouvé.");
    return token;
  }

  // 3. Si le JWT est absent ou expiré, tente de rafraîchir le token
  const refreshToken = await getRefreshTokenFromDB();  // Récupère le refresh token depuis la DB
  
  if (refreshToken) {
    console.log("🔄 Tentative de rafraîchissement du JWT...");

    // Si refreshToken est un simple token (string), tu dois appeler la fonction qui rafraîchit le JWT avec ce token.
    const newJwt = await refreshJwtWithToken(refreshToken); // Remplacer par la bonne fonction
    if (newJwt) {
  ...
  // Si aucun JWT valide ou refresh token n'est disponible
  console.warn("❌ Impossible de récupérer un JWT valide.");
  return null;
}


  // 4. Si aucun token valide n'est trouvé, l'utilisateur doit se reconnecter
  console.warn("❌ Aucun token valide trouvé, l'utilisateur doit se reconnecter.");
  return null; // Aucun token valide trouvé
}
...
// Fonction pour vérifier si le JWT est expiré
...
export async function checkAndRestoreTokens() {
  ...
  if (!jwt || isJwtExpired(jwt)) {
    ...
    await restoreTokens(storedRefreshToken);
    const newJwt = await refreshToken();
    if (!newJwt) {
      console.error("❌ Impossible de restaurer le JWT.");
    }
  } else {
...
export async function syncTokens() {
  console.log("🔄 Synchronisation des tokens entre les différents espaces de stockage...");

  // Récupère le JWT et le refresh token depuis tous les espaces de stockage
  const localJwt = localStorage.getItem("jwt");
  const sessionJwt = sessionStorage.getItem("jwt");
  const cookieJwt = document.cookie.split("; ").find(row => row.startsWith("jwt="))?.split("=")[1];
  
  // Vérifie s'il y a des incohérences entre les tokens
  if (localJwt !== sessionJwt || localJwt !== cookieJwt) {
    console.warn("⚠️ Incohérence détectée entre localStorage, sessionStorage et cookies. Mise à jour des tokens...");
    
    // Synchronise les tokens pour garantir qu'ils soient cohérents
    if (localJwt) {
      storageManager.setTokenInAllStorages("jwt", localJwt);
    } else if (sessionJwt) {
      storageManager.setTokenInAllStorages("jwt", sessionJwt);
    } else if (cookieJwt) {
      storageManager.setTokenInAllStorages("jwt", cookieJwt);
    }
  }

  // Vérifie également le refresh token
  ...
  if (refreshToken) {
    if (localRefreshToken !== refreshToken || sessionRefreshToken !== refreshToken || cookieRefreshToken !== refreshToken) {
      console.warn("⚠️ Refresh token incohérent, mise à jour...");
      storageManager.setTokenInAllStorages("refreshToken", refreshToken);
    }
  }
}
  ...
  export async function refreshToken(): Promise<string | null> {
    ...
    // Synchronisation des tokens avant le rafraîchissement
    await syncTokens(); // Assure-toi que les tokens sont bien synchronisés dans tous les espaces de stockage
  ...
  async function attemptTokenRefresh(storedRefreshToken: string): Promise<string | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`❌ Erreur HTTP lors du rafraîchissement du token : ${response.status}`);
      ...
      if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
        ...
        console.error("❌ Réponse API invalide, données manquantes");
