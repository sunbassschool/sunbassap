...
export async function refreshToken(): Promise<string> {
  ...
  isRefreshing = new Promise(async (resolve) => {
    try {
      console.log("🔄 Tentative de rafraîchissement du JWT...");
      
      const refreshToken = await getRefreshTokenFromDB();
      if (!refreshToken) {
        console.warn("❌ Aucun refresh token disponible, annulation.");
        resolve("");
        isRefreshing = null;
        return;
      }
      ...
      if (!response.ok) {
        console.error("❌ Erreur lors du refresh, déconnexion...");
        await handleRefreshFailure();
        resolve("");
      } else {
        const { jwt, newRefreshToken } = await response.json();
        await updateTokens(jwt, newRefreshToken);
        resolve(jwt);
      }
    } catch (error) {
      console.error("❌ Erreur inattendue dans refreshToken:", error);
      resolve("");
    } finally {
      isRefreshing = null; // 🔥 Libération du verrou une fois le refresh terminé
    }
  });
