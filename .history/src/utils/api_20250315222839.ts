...
export function deleteDB(dbName: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    dbRequest.onblocked = () => {
      console.warn(`⚠️ Suppression bloquée ! Fermez les autres onglets de l'application.`);
    };
  });
}
...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    ...
    // 🛑 Fermer et supprimer proprement la base si elle est corrompue
    console.log("🛑 Fermeture et suppression complète d'IndexedDB...");
    ...
    await new Promise((resolve) => setTimeout(resolve, 500)); // Pause pour s'assurer de la suppression
    ...
    console.log("✅ AuthDB supprimé avec succès !");

    console.log("🔄 Réouverture d'IndexedDB pour recréation...");
    const dbRequest = indexedDB.open("AuthDB", 1);

    dbRequest.onupgradeneeded = (event) => {
      console.log("📌 `onupgradeneeded` déclenché ! Création de `authStore`...");
      const upgradeDb = dbRequest.result;
      if (!upgradeDb.objectStoreNames.contains("authStore")) {
        upgradeDb.createObjectStore("authStore", { keyPath: "key" });
    ...
    dbRequest.onsuccess = (event) => {
      const upgradeDb = dbRequest.result;
      console.log("📂 Après recréation, stores disponibles :", upgradeDb.objectStoreNames);
      upgradeDb.close();
    };
...
export async function preventIndexedDBCleanup() {
  ...
  try {
    ...
    // ✅ Transaction sur `authStore` après vérification
    const tx = db.transaction("authStore", "readwrite");
    const store = tx.objectStore("authStore");
    ...
    if (error.name === "NotFoundError") {
      ...
      // 🔄 Attendre un petit délai avant de relancer la recréation
      await new Promise(resolve => setTimeout(resolve, 500)); // ✅ Pause de 500ms
