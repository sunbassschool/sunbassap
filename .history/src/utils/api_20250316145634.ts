...
eexport async function getValidToken(): Promise<string | null> {
  ...
  // ✅ Si le JWT est valide, mais le refresh token est absent, on essaie de le restaurer
  if (jwt && !isTokenExpired(jwt)) {
    console.log("✅ JWT valide trouvé !");

    if (!refreshToken) {
      console.warn("⚠️ Aucun refresh token trouvé, tentative de récupération...");
      ...
      if (refreshToken) {
        ...
        console.warn("❌ Impossible de restaurer le refresh token.");
      }
    }
    ...
    return jwt; // ✅ Utilisation du JWT existant
  }
  ...
  try {
    ...
    const newJwt = await refreshToken(); // ✅ Suppression des parenthèses inutiles
...
export async function restoreTokensToIndexedDB() {
  ...
  try {
    ...
    if (jwtInDB && refreshTokenInDB) {
      ...
      console.warn("⚠️ Tokens absents en IndexedDB, tentative de récupération...");
    ...
    // ✅ Vérifier et restaurer dans sessionStorage et cookies si nécessaire
    if (!sessionStorage.getItem("jwt") && jwtInDB?.value) {
      sessionStorage.setItem("jwt", jwtInDB.value);
      console.log("📦 JWT restauré dans SessionStorage.");
    }

    if (!sessionStorage.getItem("refreshToken") && refreshTokenInDB?.value) {
      sessionStorage.setItem("refreshToken", refreshTokenInDB.value);
      console.log("📦 Refresh Token restauré dans SessionStorage.");
    }

    if (!document.cookie.includes("jwt=") && jwtInDB?.value) {
      document.cookie = `jwt=${jwtInDB.value}; Secure; SameSite=None; path=/`;
      console.log("🍪 JWT restauré dans les cookies.");
    }

    if (!document.cookie.includes("refreshToken=") && refreshTokenInDB?.value) {
      document.cookie = `refreshToken=${refreshTokenInDB.value}; Secure; SameSite=None; path=/`;
      console.log("🍪 Refresh Token restauré dans les cookies.");
    }

    console.log("✅ Tokens synchronisés dans tous les stockages !");
  } catch (error) {
...
export async function refreshToken(): Promise<string> {
  if (isRefreshing) {
    ...
    return isRefreshing ?? Promise.resolve(""); 
  }
  ...
  try {
    ...
    // ✅ Ajout d'un timeout de 5s pour éviter de bloquer
    ...
    const data = await Promise.race([fetchPromise, new Promise((_, reject) => setTimeout(() => reject("Timeout"), 5000))]);
