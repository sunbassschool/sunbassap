...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    ...
    // Ouvrir la DB pour voir si `authStore` est présent
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      ...
      const dbRequest = indexedDB.open("AuthDB", 1);

      dbRequest.onupgradeneeded = (event) => {
        console.log("📌 `onupgradeneeded` déclenché !");
        const newDb = dbRequest.result;
        if (!newDb.objectStoreNames.contains("authStore")) {
          newDb.createObjectStore("authStore", { keyPath: "key" });
      ...
      dbRequest.onsuccess = (event) => {
        ...
        if (!newDb.objectStoreNames.contains("authStore")) {
          ...
          console.log("✅ `authStore` est bien présent après recréation !");
        }
      ...
      dbRequest.onerror = (event) => {
        console.error("❌ Erreur lors de l'ouverture d'IndexedDB :", dbRequest.error);
      };
