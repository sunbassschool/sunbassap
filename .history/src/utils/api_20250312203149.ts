...
export async function logoutUser() {
  ...
  try {
    ...
    // 🔄 ✅ Redirection immédiate AVANT le nettoyage complet
    setTimeout(() => {
      console.log("🔄 Redirection immédiate vers la page de connexion...");
      window.location.href = "/login";
    }, 100); // 🔥 Petit délai pour ne pas bloquer l'interface

    // 🗑️ Lancer le nettoyage en arrière-plan
    setTimeout(async () => {
      try {
    ...
    }, 500); // 🔥 Nettoyage 500ms après la redirection pour ne pas ralentir l'utilisateur
