...
export async function refreshToken(): Promise<string | null> {
  ...
  if (!storedRefreshToken || storedRefreshToken === "undefined") {
    console.warn("🚨 Refresh token absent ou invalide");
  ...
  const url = `https://cors-proxy-37yu.onrender.com/https://script.google.com/macros/s/...${encodeURIComponent(storedRefreshToken)}`;
  ...
  try {
    ...
    if (error instanceof Error && error.message === "Timeout") {
      console.warn("⏳ Timeout détecté. Retry...");
      data = await Promise.race([
        fetch(url, { method: "GET" }).then((response) => {
          if (!response.ok) throw new Error(`HTTP ${response.status}`);
          return response.json();
  ...
  if (data?.jwt && data?.refreshToken) {
    ...
    console.log("🔄 Store mis à jour :", authStore.user);
