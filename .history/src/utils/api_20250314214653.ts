...
export async function getJWTFromIndexedDB(): Promise<string | null> {
  try {
    ...
    const db = await openDB("AuthDB", 3);
...
export async function restoreTokensToIndexedDB() {
  ...
  try {
    ...
    const db = await openIndexedDB("AuthDB", 3);
    const tx = db.transaction("authStore", "readonly");
    const store = tx.objectStore("authStore");
...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  console.log("🔍 Vérification de l'IndexedDB...");

  return new Promise<boolean>((resolve, reject) => {
    const request = indexedDB.open("AuthDB", 3); // Augmenter si nécessaire
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains("authStore")) {
        console.log("🛠️ 'authStore' est manquant, création...");
    ...
    request.onsuccess = () => {
      const db = request.result;
      if (!db.objectStoreNames.contains("authStore")) {
        console.warn("⚠️ 'authStore' toujours manquant après ouverture !");
        resolve(false);
        return;
      }
    ...
    request.onerror = () => {
      console.error("❌ Erreur lors de la vérification IndexedDB :", request.error);
...
export async function restoreTokensIfNeeded(): Promise<void> {
  ...
  if (!jwt || !refreshToken) {
    ...
    try {
      const db: IDBDatabase = await openIndexedDB("AuthDB", 3);
