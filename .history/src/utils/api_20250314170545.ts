...
import type { UserInfo } from "@/types"; // 📌 Assure-toi d'importer UserInfo

async function restoreUserInfo(): Promise<UserInfo | null> {
  try {
    // Exemple de récupération depuis IndexedDB ou localStorage
    const storedUserInfo = localStorage.getItem("userInfo");
    if (storedUserInfo) {
      return JSON.parse(storedUserInfo) as UserInfo;
    }

    console.warn("⚠️ Aucun userInfo trouvé.");
    return null;
  } catch (error) {
