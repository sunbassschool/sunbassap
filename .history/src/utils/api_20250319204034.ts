...
export async function refreshToken(): Promise<string | null> {
  ...
  try {
    // Récupérer le JWT actuel et vérifier s'il est expiré avant de rafraîchir
    let token = localStorage.getItem("jwt") || sessionStorage.getItem("jwt");
    if (token && isJwtExpired(token)) {
      console.log("⚠️ Token expiré, démarrage du rafraîchissement...");
    } else {
      console.log("✅ Token valide, pas de besoin de rafraîchissement.");
      return token; // Si le JWT n'est pas expiré, on le renvoie directement
    }
