...
async function hasUserEverLoggedIn(): Promise<boolean> {
  // Vérifier si un indicateur de connexion existe en localStorage
  const userStatus = localStorage.getItem("userLogged");

  if (userStatus === "true") {
    return true; // ✅ L'utilisateur s'est déjà connecté au moins une fois.
  }

  // Vérification supplémentaire avec IndexedDB (si utilisée pour stocker des sessions)
  try {
    const db = await indexedDB.open("appDB");
    const tx = db.transaction("sessions", "readonly");
    const store = tx.objectStore("sessions");
    const userSession = await store.get("userSession");

    if (userSession) {
      return true;
    }
  } catch (error) {
    console.warn("⚠️ Impossible de vérifier la connexion via IndexedDB :", error);
  }

  return false; // 🛑 L'utilisateur ne s'est jamais connecté.
}
...
// Appeler cette fonction lors de la première connexion réussie :
localStorage.setItem("userLogged", "true");
...
export async function syncRefreshToken() {
  ...
  try {
    ...
    // Vérifier si l'utilisateur s'est déjà connecté au moins une fois
    const userExists = await hasUserEverLoggedIn(); // ✅ Vérification ajoutée
    ...
    if (!dbToken && !localToken && !cookieToken) {
      ...
      if (!userExists) {
        console.log("🚀 Aucun utilisateur n'a jamais été connecté, pas de reset nécessaire.");
        isSyncing = false;
        return;
      }
      ...
      // 🛑 NE PAS FORCER LE LOGOUT SI L'UTILISATEUR N'A JAMAIS ÉTÉ CONNECTÉ
      if (userExists) {
