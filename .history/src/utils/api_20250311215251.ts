...
export async function getRefreshTokenFromIndexedDB(): Promise<string | null> {
  ...
  try {
    ...
    if (!result || !result.value) {
      console.warn("⚠️ [DEBUG] Aucun refresh token valide trouvé dans IndexedDB.");
      return null;
    }

    console.log("📌 [DEBUG] Refresh token extrait de IndexedDB :", result.value);
    return result.value; // ✅ On renvoie uniquement la valeur
  } catch (error) {
...
export async function refreshToken(): Promise<string | null> {
  ...
  if (storedRefreshToken.includes(".")) {
    console.warn("⚠️ [ALERTE] Le refresh token ressemble à un JWT (mauvais stockage ?). Vérification en cours...");
  }
