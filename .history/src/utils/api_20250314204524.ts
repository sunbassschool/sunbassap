...
export async function getToken(): Promise<string | null> {
  ...
  console.log("📌 [DEBUG] JWT depuis IndexedDB :", token);
  ...
  if (!token) {
    token = document.cookie
      ...
      ?.split("=")[1] || null;
    console.log("📌 [DEBUG] JWT depuis les cookies :", token);
  }
...
export async function resetIndexedDB(): Promise<void> {
  ...
  // 🛑 Supprimer la base de données proprement
  ...
  // ✅ Attendre un court instant pour éviter les accès trop rapides
  await new Promise(resolve => setTimeout(resolve, 500));

  // ✅ Réouverture de la base et recréation de `authStore`
  return new Promise((resolve, reject) => {
    const request = indexedDB.open("AuthDB", 1);

    request.onupgradeneeded = function (event) {
      console.log("✅ Création de `authStore`...");
    ...
    request.onerror = function (event) {
      console.error("❌ Erreur lors de la réinitialisation d'IndexedDB :", event);
      reject(event);
    };
  });
}
...
async function initIndexedDB(): Promise<void> {
  ...
  try {
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      ...
      await resetIndexedDB(); // ✅ Utilisation de resetIndexedDB()

      // 🛑 Vérification répétée pour s'assurer que `authStore` est bien recréé
      let checkAttempts = 0;
      while (checkAttempts < 5) {
        const dbCheck = await openIndexedDB("AuthDB", 1);
        if (dbCheck.objectStoreNames.contains("authStore")) {
          console.log("✅ `authStore` détecté, IndexedDB est prête !");
          break;
        }
        checkAttempts++;
        await new Promise(resolve => setTimeout(resolve, 500)); // Attente progressive
      }
    }
...
export async function getJWTFromIndexedDB(): Promise<string | null> {
  try {
    ...
    const jwtObj: any = await db.get("authStore", "jwt");

    console.log("📌 [DEBUG] JWT brut depuis IndexedDB :", typeof jwtObj, jwtObj);

    // ✅ Extraction correcte du JWT
    let jwt: string | null = null;
    if (jwtObj) {
      if (typeof jwtObj === "object") {
        // ⚠️ Vérifie si IndexedDB stocke un objet { key: "jwt", value: "TOKEN" }
        if ("key" in jwtObj && "value" in jwtObj) {
          jwt = String(jwtObj.value); // 🔥 Récupération de `value`
        } else if ("jwt" in jwtObj) {
          jwt = String(jwtObj.jwt);
        }
      } else if (typeof jwtObj === "string") {
        jwt = jwtObj;
      }
    }
...
export async function checkAndRestoreTokens(): Promise<boolean> {
  ...
  // ✅ Charger les tokens
  const [storedRefreshToken, jwt] = await Promise.all([
    getRefreshTokenFromDB(),
    getToken()
  ]);

  console.log("📌 [DEBUG] RefreshToken :", storedRefreshToken);
  ...
  if (!storedRefreshToken) {
    ...
    console.warn("❌ JWT expiré et aucun refreshToken, vérification de firstLoginDone...");

    // 🚀 Vérifier si c'est une première ouverture après suppression des données
    const firstLoginDB = await getItemFromIndexedDB("firstLoginDone");
    const firstLoginCookie = getCookie("firstLoginDone");

    if (!firstLoginDB && !firstLoginCookie) {
      console.log("🚀 Première ouverture après suppression des données, on ne force PAS le logout.");
      await saveItemToIndexedDB("firstLoginDone", "true");
      setCookie("firstLoginDone", "true", 365);
      return false;
    }

    console.warn("🚨 Aucun token valide, l'utilisateur doit être déconnecté.");
    await logoutUser();
    return false;
  }

  console.log("✅ Tokens trouvés, pas de déconnexion.");
  return true;
}
...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      console.warn("⚠️ 'authStore' est manquant, suppression de IndexedDB...");
      db.close();
      await deleteDB("AuthDB"); // Supprime la base corrompue
      console.log("🛠️ IndexedDB supprimée, recréation en cours...");

      // Recréation immédiate
      await openDB("AuthDB", 1, {
        upgrade(db) {
          db.createObjectStore("authStore", { keyPath: "key" });
...
export async function restoreTokensIfNeeded(): Promise<void> {
  console.log("🔄 Tentative de récupération des tokens depuis localStorage...");

  let jwt: string | null = localStorage.getItem("jwt") || sessionStorage.getItem("jwt");
  let refreshToken: string | null = localStorage.getItem("refreshToken") || sessionStorage.getItem("refreshToken");

  if (!jwt || !refreshToken) {
    console.warn("⚠️ Tokens absents de localStorage/sessionStorage, tentative de récupération depuis IndexedDB...");

    try {
      const db: IDBDatabase = await openIndexedDB("AuthDB", 1);
      const tx: IDBTransaction = db.transaction("authStore", "readonly");
      const store: IDBObjectStore = tx.objectStore("authStore");

      jwt = await getItemFromStore(store, "jwt");
      refreshToken = await getItemFromStore(store, "refreshToken");

      console.log("📌 [DEBUG] JWT récupéré depuis IndexedDB :", jwt);
      console.log("📌 [DEBUG] RefreshToken récupéré depuis IndexedDB :", refreshToken);
    } catch (error) {
      console.error("❌ Erreur lors de l'accès à IndexedDB :", error);
    }
  }
  ...
  if (!jwt || !refreshToken) {
    console.warn("⚠️ Impossible de restaurer les tokens : aucun disponible !");
    return;
  }

  console.log("📥 Restauration des tokens dans localStorage et sessionStorage...");
  
  // ✅ Écriture dans localStorage et sessionStorage
  localStorage.setItem("jwt", jwt);
  sessionStorage.setItem("jwt", jwt);
  localStorage.setItem("refreshToken", refreshToken);
  sessionStorage.setItem("refreshToken", refreshToken);

  console.log("✅ Tokens copiés dans localStorage et sessionStorage !");
  
  // ✅ Appel de `updateTokens()` pour synchroniser partout
