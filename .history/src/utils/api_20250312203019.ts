...
export async function logoutUser() {
  ...
  try {
    ...
    // 🗑️ Suppression complète des cookies (compatibilité maximale)
    ...
    // 🗑️ Suppression des données de IndexedDB
    try {
      console.log("🗑️ Nettoyage de IndexedDB...");
      await clearIndexedDBData();
      console.log("✅ IndexedDB nettoyée !");
    } catch (error) {
      console.warn("⚠️ IndexedDB non accessible ou déjà vide.");
    }

    // 🗑️ Suppression complète du localStorage et sessionStorage
    ...
    setTimeout(() => {
      ...
      window.location.href = "/login"; // 🔥 On pourrait ajouter un `window.location.reload();` pour être sûr.
    }, 300);
