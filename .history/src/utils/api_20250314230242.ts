...
export async function logoutUser() {
  ...
  try {
    ...
    // ✅ Marquer que la déconnexion est en cours pour éviter une boucle
    sessionStorage.setItem("isLoggingOut", "true");

    // ✅ Affichage du message de déconnexion stylisé
    showLogoutMessage();

    // 🗑️ **Suppression des tokens**
    ...
    // ✅ **Forcer un refresh complet pour éviter une boucle**
    setTimeout(() => {
      console.log("🔄 Rafraîchissement de la page après déconnexion...");
      window.location.reload(); // 🔥 Recharge l'application pour éviter les conflits de route
    }, 1500);
