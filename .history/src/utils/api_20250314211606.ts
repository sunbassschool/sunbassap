...
export async function resetIndexedDB(): Promise<void> {
  ...
  await new Promise(resolve => setTimeout(resolve, 300)); // ✅ Petite pause pour éviter un accès trop rapide

  console.log("🔄 Ouverture de la nouvelle IndexedDB...");
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = function (event) {
      console.log("✅ IndexedDB recréée avec succès !");
      indexedDBResetInProgress = false;
      resolve();
    };

    request.onerror = function (event) {
      console.error("❌ Erreur lors de la création d'IndexedDB :", event);
      indexedDBResetInProgress = false;
      reject(event);
    };
  });
