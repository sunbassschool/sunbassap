...
export function getItemFromStore(store: IDBObjectStore, key: string): Promise<string | null> {
  return new Promise((resolve, reject) => {
...
function putItemInStore(store: IDBObjectStore, key: string, value: string): Promise<void> {
  return new Promise((resolve, reject) => {
...
export async function restoreTokensIfNeeded(): Promise<void> {
  ...
  // 🔍 Si `localStorage` et `sessionStorage` sont vides, on vérifie IndexedDB
  if (!jwt || !refreshToken) {
    ...
    try {
      ...
      jwt = await getItemFromStore(store, "jwtToken");
      refreshToken = await getItemFromStore(store, "refreshToken");

      if (jwt && refreshToken) {
        console.log("✅ Tokens récupérés depuis IndexedDB !");
      }
    } catch (error) {
      console.error("❌ Erreur lors de l'accès à IndexedDB :", error);
    }
  ...
  // ❌ Si toujours aucun token, on arrête
  if (!jwt || !refreshToken) {
    console.warn("⚠️ Impossible de restaurer les tokens : aucun disponible !");
    return;
  }

  console.log("📥 Restauration des tokens dans tous les stockages...");

  try {
    // ✅ Stockage dans IndexedDB si nécessaire
    const db: IDBDatabase = await openIndexedDB("AuthDB", 1);
    const tx: IDBTransaction = db.transaction("authStore", "readwrite");
    const store: IDBObjectStore = tx.objectStore("authStore");

    await putItemInStore(store, "jwtToken", jwt);
    await putItemInStore(store, "refreshToken", refreshToken);

    console.log("✅ Tokens mis à jour dans IndexedDB.");

    // ✅ Mise à jour dans localStorage, sessionStorage et cookies
    ...
    console.log("✅ Tokens synchronisés partout (IndexedDB, localStorage, sessionStorage, cookies).");

    // ✅ Synchronisation avec `updateTokens()` après la restauration
