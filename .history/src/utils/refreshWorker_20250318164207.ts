self.addEventListener("message", async (event) => {
  if (event.data.action === "refreshToken") {
    ...
    try {
      ...
      // Cast explicite de `error` en `Error`
      const e = error as Error;

      // On vérifie ensuite que l'erreur est bien une instance d'Error et accède à message
      self.postMessage({ status: "error", message: e.message || "Erreur inconnue" });
    }
