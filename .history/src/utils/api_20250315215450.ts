...
export async function preventIndexedDBCleanup() {
  if (indexedDBCleanupRunning) return; // 🚀 Évite plusieurs exécutions simultanées
  indexedDBCleanupRunning = true;

  console.log("🛡️ Protection contre la suppression d'IndexedDB...");

  try {
    // 🔄 Vérifie et recrée IndexedDB si nécessaire
    const db = await openDB("AuthDB", 1, {
      upgrade(db) {
        if (!db.objectStoreNames.contains("authStore")) {
    ...
    // ✅ Vérifie que l'object store existe AVANT de faire une transaction
    if (!db.objectStoreNames.contains("authStore")) {
      console.error("❌ `authStore` n'existe toujours pas après création !");
      return;
    }

    // 🔄 Ajoute une clé "keepAlive" pour éviter la suppression d'IndexedDB
    const tx = db.transaction("authStore", "readwrite");
    const store = tx.objectStore("authStore");
    ...
    // 🔄 Si IndexedDB est corrompue, la supprimer et recréer
    if (error.name === "NotFoundError") {
      console.warn("⚠️ IndexedDB corrompue, suppression et recréation...");
      await deleteDB("AuthDB");
      await preventIndexedDBCleanup(); // 🔄 Relancer après suppression
    }
  }

  // 🔄 Relance la protection toutes les 24h
  setTimeout(() => {
