...
export async function scheduleJwtRefresh() {
  console.log("🔄 Planification dynamique du rafraîchissement du JWT...");

  async function refreshIfNeeded() {
    const token = await getToken();
    if (!token) {
      console.warn("⚠️ Aucun JWT trouvé, arrêt du refresh.");
      return;
    }
    ...
    if (shouldRefreshJwt(token)) {
      ...
      const refreshed = await refreshToken(); // 🔄 Rafraîchir le token
      if (!refreshed) {
        console.error("❌ Échec du refresh, on ne relance pas immédiatement.");
        return;
      }

      console.log("✅ JWT rafraîchi avec succès, attente avant le prochain check.");
      setTimeout(refreshIfNeeded, 30 * 1000); // 🕒 Pause 30s avant le prochain check
      return;
    }

    // 🔄 Calcul dynamique du prochain refresh (vérifier 2 min avant expiration)
    const decoded = JSON.parse(atob(token.split(".")[1]));
    const expirationTime = decoded.exp * 1000;
    const nextCheck = Math.max(2 * 60 * 1000, expirationTime - Date.now() - 2 * 60 * 1000);

    console.log(`⏳ Prochain refresh dans ${nextCheck / 1000} secondes`);
    setTimeout(refreshIfNeeded, nextCheck);
  }

  refreshIfNeeded(); // Lance la vérification immédiate
}
