...
async function syncAllStorages(jwt: string, refreshToken: string) {
  ...
  // 🔹 Vérifie que IndexedDB est prête avant de faire des mises à jour
  await getAuthDB();  // Assure que la DB est prête (surtout après une suppression)
  
  // 🔹 Mise à jour des tokens dans IndexedDB
...
export async function updateRefreshTokenInDB(newRefreshToken: string | null) {
  ...
  try {
    const db = await getAuthDB(); // ✅ Instance centralisée et sécurisée
    const tx = db.transaction("authStore", "readwrite");
    const store = tx.objectStore("authStore");
