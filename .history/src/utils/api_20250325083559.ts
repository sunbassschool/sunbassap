...
export async function refreshToken(): Promise<string | null> {
  ...
  try {
    ...
    // ⏱ Vérif d’expiration du refreshToken côté client
    const storedExp = localStorage.getItem("refreshToken_exp") || sessionStorage.getItem("refreshToken_exp");
    if (storedExp && Date.now() > parseInt(storedExp)) {
      console.warn("⛔ Refresh token expiré (côté client).");
    ...
    if (!storedRefreshToken || storedRefreshToken.trim() === "" || storedRefreshToken === "undefined") {
    ...
    console.log("📌 [DEBUG] Tentative de refresh à :", new Date().toISOString());
    console.log("📌 [DEBUG] Refresh token récupéré brut :", storedRefreshToken);

    const existingJwt = localStorage.getItem("jwt") || sessionStorage.getItem("jwt");
    if (existingJwt) {
      try {
        const decoded = JSON.parse(atob(existingJwt.split('.')[1]));
        console.log("📌 [DEBUG] Ancien JWT avant refresh:", decoded.exp, new Date(decoded.exp * 1000).toISOString());
      } catch (e) {
        console.warn("❓ [DEBUG] Échec du décodage du JWT existant :", e);
      }
    }

    const url = `https://cors-proxy-37yu.onrender.com/https://script.google.com/macros/s/AKfycbzrKmjtH3Z9HXBU6-2b8sC-QXNPSoiN0ueeTNXX5dnHfEjcEG7PUqMGD35GU6rmgeWiBQ/exec?route=refresh&refreshtoken=${encodeURIComponent(
      storedRefreshToken
    )}`;
    ...
    if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
      ...
      // 📌 Mettre à jour la nouvelle date d'expiration du refreshToken côté client (7 jours)
