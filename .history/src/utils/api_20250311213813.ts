...
export async function refreshToken(): Promise<string | null> {
  ...
  console.log("📡 [DEBUG] Refresh Token récupéré pour envoi :", storedRefreshToken);

  // 🛑 Vérification pour éviter d'envoyer un JWT au lieu d'un refreshToken
  if (storedRefreshToken.split(".").length === 3) {
    console.error("❌ [ERROR] Le Refresh Token semble être un JWT. Vérifie la source du token !");
    return null;
  }

  try {
    // ✅ 2️⃣ Appel à l'API pour rafraîchir le JWT avec le **BON** Refresh Token
    const newJwt = await attemptTokenRefresh(storedRefreshToken);

    if (!newJwt) {
      console.error("❌ [DEBUG] Impossible de rafraîchir le JWT, réponse API invalide.");
      await handleRefreshFailure();
      return null;
    }
    ...
    localStorage.setItem("jwt", newJwt);
    sessionStorage.setItem("jwt", newJwt);
    await saveJWTToIndexedDB(newJwt);
