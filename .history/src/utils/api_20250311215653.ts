...
export async function getRefreshTokenFromIndexedDB(): Promise<string | null> {
  console.log("🔄 [DEBUG] Vérification du refresh token dans IndexedDB...");

  try {
    const db = await openDB("AuthDB", 1);
    const tx = db.transaction("authStore", "readonly");
    const store = tx.objectStore("authStore");
    const result = await store.get("refreshToken"); // Vérifie qu'on récupère la bonne clé

    console.log("📌 [DEBUG] Contenu de IndexedDB pour refreshToken :", result);

    if (result && result.value) {
      console.log("📌 [DEBUG] Refresh token extrait de IndexedDB :", result.value);
      return result.value; // ✅ Retourne uniquement la valeur
    } else {
      console.warn("⚠️ [DEBUG] Aucun refresh token trouvé dans IndexedDB.");
      return null;
    }
  } catch (error) {
    console.error("❌ [DEBUG] Erreur lors de la récupération du refresh token depuis IndexedDB :", error);
    return null;
  }
}
