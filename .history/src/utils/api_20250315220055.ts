...
eexport async function verifyIndexedDBSetup(): Promise<boolean> {
  if (!window.indexedDB) {
  ...
  try {
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      ...
      // ⚠️ On utilise la même version (1) mais on s'assure que le store est créé
    ...
    // 🛑 Dernière vérification : si `authStore` n'existe toujours pas, il y a un problème
    if (!db.objectStoreNames.contains("authStore")) {
