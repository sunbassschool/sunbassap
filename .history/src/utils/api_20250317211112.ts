...
// ✅ Déclarer `isRefreshing` UNE SEULE FOIS au bon endroit
...
export async function refreshToken(): Promise<string> {
  ...
  isRefreshing = new Promise((resolve) => {
    if (!refreshWorker) {
      refreshWorker = new Worker(new URL("./refreshWorker.ts", import.meta.url), { type: "module" });
    }

    refreshWorker!.onmessage = async (event) => {
      const { status, jwt, refreshToken } = event.data;
      
      if (status === "success") {
        console.log("✅ [Worker] Refresh réussi !");
        await updateTokens(jwt, refreshToken);
        resolve(jwt);
      } else {
        console.error("❌ [Worker] Échec du refresh :", event.data.message);
        await handleRefreshFailure();
        resolve("");
      }

      isRefreshing = null; // ✅ Libération du verrou après traitement
    };

    getRefreshTokenFromDB().then((refreshToken) => {
      refreshWorker!.postMessage({ action: "refreshToken", refreshToken });
    });
  });
...
export async function updateTokens(newJwt: string | null, newRefreshToken: string | null) {
  ...
  try {
    ...
    try {
      ...
      console.error("❌ Erreur de stockage en localStorage/sessionStorage :", error);
    }

    // ✅ Mise à jour des cookies
    try {
      document.cookie = `jwt=${newJwt}; Secure; HttpOnly; SameSite=Strict; path=/; max-age=3600`;
      document.cookie = `refreshToken=${newRefreshToken}; Secure; HttpOnly; SameSite=Strict; path=/; max-age=604800`;
    ...
    try {
      ...
      console.error("❌ Erreur lors de la mise à jour d'IndexedDB :", error);
    }

    console.log("✅ Tous les tokens ont été mis à jour !");
  } catch (error) {
    console.error("❌ Erreur inattendue lors de la mise à jour des tokens :", error);
  }
}
  ...
  if (jwtExists || refreshTokenExists) {
    ...
    if (typeof logoutUser === "function") {
      if (typeof isLoading !== "undefined") {
        isLoading.value = true; // ✅ Active le chargement avant la déconnexion
      }

      await logoutUser(); // 🔥 Déconnexion propre

      if (typeof isLoading !== "undefined") {
        isLoading.value = false; // ✅ Désactive le chargement après la déconnexion
      }
    } else {
