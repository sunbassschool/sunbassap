...
export async function restoreJwt(): Promise<string | null> {
  ...
  // 1️⃣ Vérifie d'abord localStorage et sessionStorage
  let storedJwt = localStorage.getItem("jwt") || sessionStorage.getItem("jwt");

  // 2️⃣ Si absent, cherche dans les cookies
  if (!storedJwt) {
    storedJwt = document.cookie.split("; ").find(row => row.startsWith("jwt="))?.split("=")[1] || null;
    if (storedJwt) console.log("🍪 JWT restauré depuis les cookies !");
  }

  // 3️⃣ Si toujours rien, tente IndexedDB (si disponible)
  if (!storedJwt && window.indexedDB) {
    try {
      const db = await openIndexedDB();
      storedJwt = await db.get("jwt");
      if (storedJwt) console.log("✅ JWT restauré depuis IndexedDB !");
    } catch (error) {
      console.error("⚠️ Erreur lors de la récupération du JWT depuis IndexedDB :", error);
    }
  }

  // 4️⃣ Retourne le JWT ou null
  return storedJwt || null;
}

// Fonction d'ouverture IndexedDB (si tu l’utilises)
async function openIndexedDB() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open("SunBassDB", 1);
