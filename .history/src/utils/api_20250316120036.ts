...
export async function refreshToken(): Promise<string> {
  if (isRefreshing) {
    ...
    return isRefreshing ?? Promise.resolve(""); // ✅ Assure toujours un retour de type `Promise<string>`

}

  console.log("🔒 Activation du verrou de rafraîchissement...");
  isRefreshing = new Promise<string>((resolve) => { // ✅ Correction de la fermeture
    resolvePromise = (value: string) => resolve(value || ""); // ✅ Correction
  });
  ...
  try {
    ...
    if (!storedRefreshToken) {
      ...
      if (resolvePromise) resolvePromise(""); // ✅ Remplace `null` par `""`
      isRefreshing = null;
      return "";
    }
