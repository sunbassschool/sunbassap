...
export function isJwtExpired(token: string | { key: string; value: string } | null): boolean {
  ...
  try {
    ...
    // Vérification de la présence du champ exp et de son expiration
    if (!decodedPayload.exp) {
      console.warn("⚠️ [isJwtExpired] Le JWT ne contient pas de champ 'exp'.");
      return true; // Si le champ exp est manquant, le token est invalide
    }
    ...
    return true; // Si une erreur survient pendant le décodage, le token est considéré comme invalide
  }
}
...
export async function preventIndexedDBCleanup() {
  ...
  try {
    ...
    await store.put({ id: "keepAlive", value: Date.now() });
