...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    console.time("⏳ Vérification IndexedDB");
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      ...
      console.time("🗑️ Suppression de IndexedDB");
      await deleteDB("AuthDB");
      console.timeEnd("🗑️ Suppression de IndexedDB");

      await new Promise((resolve) => setTimeout(resolve, 500));

      console.time("🚀 Réouverture IndexedDB");
      ...
      console.timeEnd("🚀 Réouverture IndexedDB");

      console.log("✅ 'authStore' recréé avec succès !");
    }

    console.timeEnd("⏳ Vérification IndexedDB");
    return true;
  } catch (error) {
