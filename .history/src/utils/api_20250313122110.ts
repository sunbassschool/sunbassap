...
async function fetchNewRefreshToken(jwt: string | null): Promise<string | null> {
  ...
  try {
    const url = `https://cors-proxy-37yu.onrender.com/https://script.google.com/macros/s/.../exec?route=getRefreshToken&jwt=${encodeURIComponent(jwt)}`;
    console.log("📡 Appel API pour récupérer un refreshToken avec JWT :", jwt);
    ...
    if (typeof data === "object" && data.status === "success" && typeof data.refreshToken === "string") {
      console.log("✅ Refresh token récupéré :", data.refreshToken);
