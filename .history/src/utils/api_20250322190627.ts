...
export async function getValidToken(): Promise<string | null> {
  ...
  // 🔐 Étape 1 : Vérifie le JWT local (sessionStorage ou localStorage)
  let jwt = localStorage.getItem("jwt") || sessionStorage.getItem("jwt");

  if (jwt && typeof jwt === "string" && jwt.length > 20 && !isJwtExpired(jwt)) {
    console.log("✅ JWT valide trouvé dans localStorage/sessionStorage !");
    return jwt;
  }

  // 🔐 Étape 2 : Vérifie depuis IndexedDB
  jwt = await getToken();
  ...
  if (!jwt || typeof jwt !== "string" || jwt.length < 10) {
    console.warn("🚨 JWT invalide ou trop court :", jwt);
  }

  // 🔁 Étape 3 : Tente le rafraîchissement si nécessaire
  if (isRefreshing) {
    console.log("⏳ Refresh déjà en cours, en attente...");
    return await waitForRefresh(); // 🔁 Attend le token déjà en cours de refresh
  }
  ...
  try {
    ...
    if (newJwt && typeof newJwt === "string" && newJwt.length > 20) {
      console.log("✅ Refresh réussi, nouveau JWT obtenu.");
      return newJwt;
    } else {
      console.error("❌ Refresh échoué, JWT invalide :", newJwt);
      return null;
    }
  } catch (error) {
...
async function waitForRefresh(timeout = 3000): Promise<string | null> {
...
export async function refreshToken(): Promise<string | null> {
  if (isRefreshing) {
    ...
    return await isRefreshing; // ✅ Attends la promesse, au lieu de la retourner brute
  }
  ...
  try {
    ...
    if (!storedRefreshToken) {
      ...
      resolvePromise?.(null); // ✅ Ne bloque pas les autres
      return null;
    }
    ...
    if (data.status === "success" && data.data?.jwt) {
      ...
      console.log("✅ Nouveau JWT :", data.data.jwt);
      resolvePromise?.(data.data.jwt); // ✅ Résout la promesse de façon sûre
    ...
    resolvePromise?.(null); // ✅ Résout même en cas d’erreur
    return null;
  } finally {
    isRefreshing = null; // ✅ Libère le verrou
  }
}
