...
export async function refreshToken(): Promise<string | null> {
  console.log("🔄 [DEBUG] Tentative de rafraîchissement...");

  let storedRefreshToken = await getRefreshTokenFromDB();
  console.log("📌 [DEBUG] Refresh token avant refresh API :", storedRefreshToken);
  ...
  try {
      ...
      console.log("✅ [DEBUG] Nouveau JWT reçu :", newJwt);

      return newJwt;
  } catch (error) {
      console.error("❌ [DEBUG] Erreur API lors du refresh :", error);

      console.log("🛑 [DEBUG] Suppression du refresh token...");
