...
export async function getRefreshTokenFromIndexedDB(): Promise<string | null> {
  ...
  try {
    ...
    // 🔥 Récupération du refresh token uniquement !
    ...
    console.log("📌 [DEBUG] Refresh token récupéré depuis IndexedDB :", result.value);
    
    // Vérification si le résultat contient bien une valeur
    return typeof result === "object" && result.value ? result.value : null;
...
export async function refreshToken(): Promise<string | null> {
  ...
  if (!storedRefreshToken) {
    ...
    storedRefreshToken = await getRefreshTokenFromIndexedDB();
    ...
    // ✅ Vérification si c'est bien une chaîne et non un objet
    if (typeof storedRefreshToken === "object" && storedRefreshToken.value) {
      storedRefreshToken = storedRefreshToken.value;
    }
  ...
  // 📡 [DEBUG] Vérification finale avant envoi à l'API
...
async function attemptTokenRefresh(storedRefreshToken: string): Promise<string | null> {
  ...
  console.log("📡 [DEBUG] Requête de rafraîchissement envoyée à :", url);
  ...
  try {
    ...
    if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
      console.log("✅ [DEBUG] Nouveau JWT reçu :", data.data.jwt);
      console.log("✅ [DEBUG] Nouveau Refresh Token reçu :", data.data.refreshToken);

      await updateTokens(data.data.jwt, data.data.refreshToken);
      return data.data.jwt;
    } else {
      console.error("❌ [DEBUG] Réponse API invalide ou incomplète :", data);
      return null;
    }
  } catch (error) {
    console.error("❌ [DEBUG] Erreur lors de la requête de rafraîchissement :", error);
    return null;
  }
}
