...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    ...
    // Ouvrir la DB en lecture seule pour vérifier son état
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      ...
      console.log("🛑 Base de données supprimée. Attente avant recréation...");
      ...
      // ⚠️ On ouvre SANS await pour capturer les événements
      ...
      dbRequest.onsuccess = (event) => {
        ...
        if (!newDb.objectStoreNames.contains("authStore")) {
          ...
          console.log("✅ `authStore` est bien présent !");
        }

        newDb.close(); // Fermer proprement la connexion
