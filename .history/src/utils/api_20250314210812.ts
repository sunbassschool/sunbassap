...
export async function resetIndexedDB(): Promise<void> {
  ...
  await new Promise(resolve => setTimeout(resolve, 500)); // ✅ Pause pour éviter l'accès trop rapide
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = async function (event) {
      ...
      // ✅ Vérifier que 'authStore' existe après recréation
      const db = (event.target as IDBRequest<IDBDatabase>).result;
      if (!db.objectStoreNames.contains("authStore")) {
        console.error("❌ Erreur critique : 'authStore' toujours manquant après recréation !");
        reject(new Error("authStore introuvable après recréation."));
        return;
      }
