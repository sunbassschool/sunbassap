...
expo function getItemFromStore(store: IDBObjectStore, key: string): Promise<string | null> {
  return new Promise((resolve, reject) => {
    const request = store.get(key);
...
export async function restoreTokensIfNeeded(): Promise<void> {
  ...
  if (!jwt || !refreshToken) {
    console.warn("⚠️ Aucun token trouvé dans localStorage, l'utilisateur devra se reconnecter.");
    return;
  }

  console.log("✅ Tokens trouvés, restauration en cours...");

  if (!window.indexedDB) {
    console.error("❌ IndexedDB n'est pas supportée par ce navigateur !");
    return;
  }
