...
export async function removeFromIndexedDB(key: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open("authStore", 1);
    ...
    request.onsuccess = function () {
      ...
      deleteRequest.onsuccess = function () {
        console.log(`🗑️ Clé ${key} supprimée de IndexedDB`);
        resolve();
      };

      deleteRequest.onerror = function () {
        console.error(`❌ Erreur de suppression de ${key} dans IndexedDB`);
...
export async function restoreTokensIfNeeded() {
  ...
  try {
    ...
    if (jwtData?.value && isJwtExpired(jwtData.value)) {
      console.warn("❌ JWT expiré trouvé dans IndexedDB, suppression...");
      await removeFromIndexedDB("jwt");
    }
...
export async function handleRefreshToken() {
  ...
  if (!refreshToken?.value) {
    console.warn("❌ Aucun refreshToken disponible, impossible de rafraîchir !");
    await clearTokens();
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error("❌ Le serveur a rejeté la demande de refresh !");
    }

    const data = await response.json();
    console.log("✅ Nouveau JWT reçu :", data.jwt);

    if (!data.jwt || isJwtExpired(data.jwt)) {
      console.warn("❌ Le nouveau JWT est invalide ou expiré !");
      await clearTokens();
      return;
    }

    // ✅ Stocker correctement le JWT
    localStorage.setItem("jwt", data.jwt);
    sessionStorage.setItem("jwt", data.jwt);
    await saveToIndexedDB("jwt", data.jwt);

    console.log("✅ JWT mis à jour avec succès !");
  } catch (error) {
    console.error("❌ Erreur lors du refresh du JWT :", error);
    await clearTokens();
  }
}
async function clearTokens() {
  console.warn("🚨 Suppression complète des tokens !");
  localStorage.removeItem("jwt");
  sessionStorage.removeItem("jwt");
  await removeFromIndexedDB("jwt");
  await removeFromIndexedDB("refreshToken");
}
