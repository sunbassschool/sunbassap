...
export async function getJWTFromIndexedDB(): Promise<string | null> {
  try {
    ...
    // ✅ Ajout du typage précis
    const jwtObj: { jwt?: string } | string | null = await db.get("authStore", "jwt");

    console.log("📌 [DEBUG] JWT brut depuis IndexedDB :", typeof jwtObj, jwtObj);

    // ✅ Correction : si IndexedDB retourne un objet, on extrait `.jwt`
    const jwt = typeof jwtObj === "object" && jwtObj?.jwt ? jwtObj.jwt : jwtObj;
