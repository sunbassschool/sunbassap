...
import { getJWT, getRefreshToken, saveTokens } from "@/utils/auth.js";
import { refreshJWT } from "@/utils/auth.js";
...
export async function checkAndRestoreTokens() {
    ...
    if (!jwt && refreshToken) {
        ...
        if (jwt) {
            await saveTokens(jwt, refreshToken); // Sauvegarde dans IndexedDB
        } else {
