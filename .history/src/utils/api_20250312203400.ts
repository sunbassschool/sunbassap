...
console.log("🔍 [getToken] Token récupéré :", token); // 🛠️ Ajoute un log pour voir ce qui est récupéré
if (!token) {
...
export async function checkAndRefreshOnWakeUp() {
  ...
  if (!storedRefreshToken) { // ✅ On ne vérifie PAS l'expiration du refresh token !
...
export async function logoutUser() {
  ...
  try {
    ...
    // 🚀 **ÉTAPE 1 : Suppression immédiate des JWT et tokens critiques**
    console.log("🗑️ Suppression immédiate des tokens...");
    sessionStorage.removeItem("jwt");
    sessionStorage.removeItem("refreshToken");
    localStorage.removeItem("jwt");
    localStorage.removeItem("refreshToken");

    // 🔥 Optionnel : Annuler toute récupération automatique de session en bloquant temporairement IndexedDB
    localStorage.setItem("session_expired", "true");

    // ✅ **Redirection rapide (mais avec un léger délai pour éviter les conflits)**
    setTimeout(() => {
      console.log("🔄 Redirection vers la page de connexion...");
      window.location.href = "/intro";
    }, 300); // 🔥 300ms d'attente pour éviter les conflits avec le refreshToken

    // 🧹 **ÉTAPE 2 : Nettoyage en arrière-plan (moins urgent)**
    setTimeout(async () => {
      try {
        ...
        console.log("🗑️ Nettoyage de localStorage et sessionStorage...");
        localStorage.clear();
        sessionStorage.clear();
        console.log("✅ Données utilisateur nettoyées !");
        
        // 🔔 Déclencher un événement pour informer les autres composants (ex: Vuex, Pinia, événements personnalisés)
        window.dispatchEvent(new Event("logout"));

      } catch (error) {
        console.error("❌ Erreur lors du nettoyage :", error);
      }
    }, 500); // 🔥 Le reste du nettoyage se fait légèrement après la redirection
