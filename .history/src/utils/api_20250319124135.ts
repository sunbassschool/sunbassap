...
export async function refreshToken(): Promise<string | null> {
  ...
  try {
    ...
    if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
      ...
      // 🗑️ **Supprimer l'ancien JWT avant d'ajouter le nouveau**
...
export async function updateTokens(newJwt: string | null, newRefreshToken: string | null) {
  ...
  try {
    // ✅ Stockage rapide (LocalStorage / SessionStorage)
    localStorage.setItem("jwt", newJwt);
    sessionStorage.setItem("jwt", newJwt);
    localStorage.setItem("refreshToken", newRefreshToken);
    sessionStorage.setItem("refreshToken", newRefreshToken);
    ...
    // ✅ Stockage dans IndexedDB (asynchrone, donc on attend qu'il soit bien enregistré)
    await updateJWTInIndexedDB(newJwt);
    await updateRefreshTokenInDB(newRefreshToken);

    console.log("✅ Tokens mis à jour dans IndexedDB !");

    // ✅ Vérification après stockage (évite les problèmes de timing)
    const storedJWT = await getTokenFromIndexedDB();
    console.log("🔍 JWT récupéré après stockage :", storedJWT);

    if (storedJWT !== newJwt) {
      console.error("⚠️ Problème : le JWT stocké ne correspond pas !");
    }
...
export async function updateRefreshTokenInDB(newRefreshToken: string | null) {
  ...
  try {
    ...
    await store.put({ id: "refreshToken", value: newRefreshToken });
...
export async function updateJWTInIndexedDB(newJwt: string | null) {
  ...
  try {
    ...
    await store.put({ id: "jwt", value: newJwt });
