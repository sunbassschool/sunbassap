...
async function restoreTokensIfNeeded() {
  ...
  const dbRequest = indexedDB.open("AuthDB");
  dbRequest.onsuccess = function () {
    const db = dbRequest.result;
    const tx = db.transaction("authStore", "readwrite");
    const store = tx.objectStore("authStore");

    store.put({ key: "jwtToken", value: jwt });
  ...
  dbRequest.onerror = function () {
    console.error("❌ Erreur lors de l'ouverture de IndexedDB pour restauration des tokens !");
  };
}
