...
export function getFromIndexedDB(key: string): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = function (event) {
      ...
      const getRequest = store.db.get(key);
...
export async function getJWTFromIndexedDB(): Promise<string | null> {
  try {
    ...
    // ✅ Vérification que `store` est bien un IDBObjectStore
    console.log("📌 Vérification de store :", store);

    if (typeof store.get !== "function") {
      console.error("❌ Erreur : store.get n'est pas une fonction. Vérifiez IndexedDB !");
      return null;
    }
...
export async function checkIndexedDBStatus(): Promise<void> {
  try {
    ...
    const jwt = await store.db.get("jwt");
    const refreshToken = await store.db.get("refreshToken");
...
try {
  ...
  const tx = db.transaction("authStore", "readonly");
  const store = tx.objectStore("authStore");
  const expirationEntry = await store.db.get("refreshTokenExpiration");
