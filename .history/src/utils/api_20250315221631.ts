...
export async function getValidToken(): Promise<string | null> {
  ...
  if (!jwtExists && !refreshTokenExists) {
    ...
    return null; // 🔄 Pas de tentative de refresh, pas de logout inutile
  }
  ...
  // 🔍 Vérification de l'existence d'un refresh token avant de tenter le refresh
  ...
  if (!refreshToken) {
    ...
    // 🗑️ Suppression du JWT expiré (évite les bugs)
    localStorage.removeItem("jwt");
    sessionStorage.removeItem("jwt");

    // 🛑 On ne logout **que si l'utilisateur était connecté avant**
    if (jwtExists) {
      console.warn("🚨 L'utilisateur était connecté, on force la déconnexion.");
      await logoutUser(); // 🔥 Déconnexion forcée **uniquement si nécessaire**
    }
  ...
  try {
    ...
    if (newJwt) {
      ...
      await logoutUser(); // 🔥 Forcer la déconnexion en cas d'échec
      return null;
    }
  } catch (error) {
    console.error("❌ Erreur lors du rafraîchissement du JWT :", error);
    await logoutUser(); // 🔥 Déconnexion forcée en cas d'erreur critique
    return null;
  }
}
...
export async function restoreTokensToIndexedDB() {
  ...
  try {
    ...
    const jwtInDB = await getItemFromStore(store.name, "jwt");

    const refreshTokenInDB = await getItemFromStore(store.name, "refreshToken");
...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    if (!(globalThis as any).__indexedDBTimerStarted) {
      console.time("⏳ Temps total de création IndexedDB");
      (globalThis as any).__indexedDBTimerStarted = true;
    }

    console.log("🔍 Vérification de l'existence d'IndexedDB...");

    // Vérifier si `AuthDB` existe déjà
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      ...
      db.close();
      await new Promise((resolve) => setTimeout(resolve, 500)); // ⏳ Pause courte

      console.time("⏳ Temps de suppression IndexedDB");
      await deleteDB("AuthDB", {
        blocked() {
          console.warn("⚠️ La suppression d'IndexedDB est bloquée par un autre processus !");
        },
      });
      console.timeEnd("⏳ Temps de suppression IndexedDB");

      await new Promise((resolve) => setTimeout(resolve, 500)); // Pause courte avant recréation

      console.log("🔍 Vérification si la suppression a bien été effectuée...");
      ...
      if (dbExists) {
        console.error("❌ AuthDB existe encore après suppression !");
        return false;
      }

      console.timeEnd("⏳ Temps total de création IndexedDB");
      delete (globalThis as any).__indexedDBTimerStarted;
      console.time("⏳ Temps pour ouvrir IndexedDB");

      console.log("🔄 Tentative de recréation d'IndexedDB...");
      
      // 📌 Solution : Augmenter temporairement la version pour forcer `upgrade`
      console.log("📌 Augmentatzzion temporaire de la version pour forcer `upgrade`...");
      await openDB("AuthDB", 2, {
        upgrade(db) {
          console.log("✅ `onupgradeneeded` déclenché (version 2) !");
        },
      }).then(db => db.close());

      console.log("🔄 Réouverture en version 1 avec `authStore`...");

      // 📌 Maintenant, on redescend la version à 1 pour recréer `authStore`
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      console.error("❌ `authStore` n'existe toujours pas après ouverture !");
      return false;
    }
...
export async function restoreTokensIfNeeded(): Promise<boolean> {
  ...
  if (!(await verifyIndexedDBSetup())) {
    ...
    return false; // ❌ IndexedDB non dispo, donc retour false
  }

  try {
    // ✅ Vérifie si des tokens existent AVANT de les restaurer
    const jwt = await getItemFromStore("authStore", "jwt");
    const refreshToken = await getItemFromStore("authStore", "refreshToken");

    if (!jwt && !refreshToken) {
      console.warn("⚠️ Aucun token trouvé, inutile de continuer.");
      return false; // ❌ Pas de tokens, donc retour false
    }

    if (jwt) {
      console.log("✅ JWT trouvé, mise à jour...");
    ...
    if (refreshToken) {
      console.log("✅ Refresh Token trouvé, mise à jour...");
      localStorage.setItem("refreshToken", refreshToken);
      sessionStorage.setItem("refreshToken", refreshToken);
      document.cookie = `refreshToken=${refreshToken}; Secure; SameSite=Strict; path=/`;
    }

    return true; // ✅ Retourne true si au moins un token a été trouvé et mis à jour
  } catch (error) {
    console.error("❌ Erreur lors de la restauration des tokens :", error);
    return false; // ❌ Retour false en cas d'erreur
  }
}
...
export async function preventIndexedDBCleanup() {
  if (indexedDBCleanupRunning) {
    console.log("🔄 IndexedDB déjà en protection, arrêt de la fonction.");
    return;
  }
  ...
  try {
    console.time("⏳ Temps d'ouverture IndexedDB");

    // 🔄 Vérification et ouverture d'IndexedDB
    const db = await openDB("AuthDB", 1, {
      upgrade(db) {
        console.log("📌 Mise à niveau d'IndexedDB...");
        if (!db.objectStoreNames.contains("authStore")) {
          console.warn("⚠️ `authStore` manquant, recréation...");
          db.createObjectStore("authStore", { keyPath: "key" });
          console.log("✅ `authStore` créé avec succès !");
        }
      },
    });

    console.timeEnd("⏳ Temps d'ouverture IndexedDB");

    // 🔍 Vérification de la présence du store
    if (!db.objectStoreNames.contains("authStore")) {
      console.error("❌ `authStore` n'existe toujours pas après ouverture !");
      return;
    }

    console.log("📌 Début de la transaction sur `authStore`...");
    const tx = db.transaction("authStore", "readwrite");
    const store = tx.objectStore("authStore");

    await store.put({ key: "keepAlive", value: Date.now() });
    console.log("✅ IndexedDB maintenu en vie avec `keepAlive`.");

  } catch (error) {
    console.error("❌ Erreur lors de la protection d'IndexedDB :", error);
    ...
    if (error.name === "NotFoundError") {
      ...
      console.log("⏳ Attente avant recréation...");
      await new Promise(resolve => setTimeout(resolve, 500));

      console.log("🔄 Nouvelle tentative de création d'IndexedDB...");
      return await preventIndexedDBCleanup();
    }
  }

  console.log("⏳ Attente de 24h avant la prochaine exécution...");
  setTimeout(() => {
    indexedDBCleanupRunning = false;
    console.log("🔄 Relance de `preventIndexedDBCleanup()` après 24h.");
