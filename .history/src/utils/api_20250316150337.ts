...
export async function getValidToken(): Promise<string | null> {
  ...
  // 🔄 Vérification des tokens stockés
  ...
  // ✅ Si le JWT est valide, on le garde
  if (jwt && !isTokenExpired(jwt)) {
    ...
    if (!storedRefreshToken) {
      ...
      // 🔥 Tentative de récupération d'un nouveau refresh token à partir du JWT (à adapter selon ton API)
      const newRefreshToken = await regenerateRefreshToken(jwt);
      if (newRefreshToken) {
        console.log("✅ Refresh token recréé :", newRefreshToken);
        await updateRefreshTokenInDB(newRefreshToken);
      } else {
        console.warn("❌ Impossible de recréer un refresh token.");
      }
    }
  ...
  // 🔍 Vérification du refresh token avant de tenter un refresh
  const storedRefreshToken = await getRefreshTokenFromDB();
