...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      ...
      // ✅ Attendre avant suppression pour éviter un conflit
      await new Promise((resolve) => setTimeout(resolve, 500));

      await deleteDB("AuthDB");

      // ✅ Attendre après suppression pour être sûr que la base est bien libérée
      await new Promise((resolve) => setTimeout(resolve, 1000));

      console.log("🚀 Réouverture de IndexedDB...");
      const newDB = await openDB("AuthDB", 1, {
        upgrade(db) {
