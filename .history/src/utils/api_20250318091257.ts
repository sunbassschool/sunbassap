...
export async function refreshToken(): Promise<string> {
  ...
  try {
    ...
    // ✅ API avec Proxy CORS
    const url = `https://cors-proxy-37yu.onrender.com/https://script.google.com/macros/s/AKfycbz-K2F1WavlIe5pxTaXwnirV1pw9pjiq6Q776zui50scu7qNaJZBVCUFfh3frkw7GVhWg/exec?route=refresh&refreshtoken=${encodeURIComponent(storedRefreshToken)}`;
    ...
    const data = await Promise.race([
      fetchPromise,
      new Promise((_, reject) => setTimeout(() => reject("Timeout"), 10000)) // Timeout de 10 secondes
    ]);
