...
export async function restoreTokensToIndexedDB() {
  ...
  // 🔍 Vérification des tokens dans localStorage et sessionStorage
  ...
  // ✅ Si aucun token n'est trouvé dans ces stockages, on vérifie les cookies
  if (!storedJwt) {
    const jwtCookie = document.cookie.split("; ").find(row => row.startsWith("jwt="))?.split("=")[1];
    if (jwtCookie) {
      storedJwt = jwtCookie;
  ...
  if (!storedRefreshToken) {
    ...
    if (refreshTokenCookie) {
      storedRefreshToken = refreshTokenCookie;
      console.log("🍪 Refresh Token trouvé dans les cookies !");
    }
  }

  // ❌ Si aucun token valide n'est trouvé, on arrête
  if (!storedJwt || !storedRefreshToken) {
    console.warn("⚠️ Impossible de restaurer IndexedDB : données manquantes.");
    return;
  }

  console.log("📥 Restauration du JWT et refreshToken dans tous les stockages...");
  ...
  console.log("✅ IndexedDB, localStorage et sessionStorage restaurés !");
}
