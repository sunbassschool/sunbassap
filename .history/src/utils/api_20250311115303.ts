...
async function checkOtherStorageForRefreshToken(): Promise<string | null> {
  ...
  if (refreshTokenCookie) {
    const cookieRefreshToken = refreshTokenCookie.split("=")[1];  // Renommé en cookieRefreshToken
    console.log("🍪 Refresh token trouvé dans les cookies :", cookieRefreshToken);
...
export function scheduleJwtRefresh() {
  ...
  setTimeout(async () => {
    ...
    // Renommage de la fonction `refreshToken` locale pour éviter le conflit
    const getNewRefreshToken = async (): Promise<string> => {
      return "someToken";  // Exemple, ici tu devrais renvoyer le vrai token
    };
...
export async function refreshToken(): Promise<string> {
  ...
  if (!storedRefreshToken) {
    ...
    await handleRefreshFailure();
    return "";  // Retourne une chaîne vide ici aussi
  }
  ...
  try {
    ...
    if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
      ...
      return data.data.jwt;  // Retourne toujours une chaîne ici
    } else {
  ...
  if (document.visibilityState === "visible") {
    ...
    if (!jwt || isJwtExpired(jwt)) {
      ...
      if (!refreshToken || typeof refreshToken !== "function") {
        console.error("❌ La fonction de rafraîchissement du token n'est pas disponible.");
        await handleRefreshFailure();
        return;
      }
      ...
      if (!newJwt || typeof newJwt !== "string") {
        console.warn("❌ Le JWT renvoyé n'est pas valide. L'utilisateur devra se reconnecter.");
        await handleRefreshFailure();
        return;
      }else {
...
export async function forceSyncRefreshToken() {
  ...
  if (validToken !== sessionToken) {
    sessionStorage.setItem("refreshToken", validToken);
