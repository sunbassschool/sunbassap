...
export async function getRefreshTokenFromDB(): Promise<string | null> {
  ...
  console.log("🔍 [DEBUG] Vérification du refresh token dans les cookies...");
  const cookieToken = document.cookie
      .split("; ")
      .find(row => row.startsWith("refreshToken="))
      ?.split("=")[1];

  console.log("📌 [DEBUG] Refresh token depuis Cookies:", cookieToken);

  if (cookieToken) {
      console.log("🔄 [FIX] Réécriture du refresh token depuis les cookies.");
      localStorage.setItem("refreshToken", cookieToken);
      sessionStorage.setItem("refreshToken", cookieToken);
  }
...
export async function getRefreshTokenFromDB(): Promise<string | null> {
  ...
  const refreshToken = await getRefreshTokenFromIndexedDB(); // IndexedDB
  console.log("📌 [DEBUG] Refresh token depuis IndexedDB:", refreshToken);
