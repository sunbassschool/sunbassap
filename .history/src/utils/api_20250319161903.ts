  ...
  try {
    ...
    if (!token) {
      ...
      if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
        ...
        // **Stocker la route précédente avant le refresh**
        const previousRoute = sessionStorage.getItem("redirectAfterRefresh");
    
        // **Si une route a été mémorisée, on redirige vers cette route**
        if (previousRoute) {
            sessionStorage.removeItem("redirectAfterRefresh");
            router.push(previousRoute); // Redirige vers la page d'origine
        } else {
            router.push("/home"); // Si aucune route mémorisée, on peut rediriger vers la page d'accueil ou une autre page
        }
