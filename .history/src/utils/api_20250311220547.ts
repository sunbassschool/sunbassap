...
async function getRefreshTokenFromIndexedDB(): Promise<string | null> {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => {
      console.error("❌ [DEBUG] Erreur ouverture IndexedDB");
      resolve(null);
    };
    ...
    request.onsuccess = () => {
      ...
      getRequest.onerror = () => {
        console.error("❌ [DEBUG] Impossible de récupérer le refreshToken depuis IndexedDB");
        resolve(null);
      };
    };
  });
...
async function attemptTokenRefresh(storedRefreshToken: string): Promise<any> {
  let encodedToken = encodeURIComponent(storedRefreshToken);
  console.log(`📡 [DEBUG] Requête envoyée avec Refresh Token : ${storedRefreshToken} (Encodé: ${encodedToken})`);
  ...
  try {
    ...
    console.log("🔄 [DEBUG] Tentative de récupération du refreshToken depuis IndexedDB...");
    const indexedDbRefreshToken = await getRefreshTokenFromIndexedDB();

    if (indexedDbRefreshToken) {
      console.log("✅ [DEBUG] Refresh Token trouvé dans IndexedDB, nouvelle tentative...");
      return attemptTokenRefresh(indexedDbRefreshToken); // On réessaie avec ce token
    } else {
      console.error("❌ [DEBUG] Aucun refreshToken valide trouvé !");
...
async function attemptTokenRefresh(storedRefreshToken: string): Promise<any> {
  ...
  const url = `https://cors-proxy-37yu.onrender.com/https://script.google.com/macros/s/AKfycbxc69Alhr0V0-DqrHblDJonvPKFy8PbjhEA6nA4sC4aFCTJOCKO9t0Q-uaApC_vcXgyHA/exec?route=refresh&refreshtoken=${encodedToken}`;
  console.log("📡 [DEBUG] URL API :", url);
