...
export async function refreshToken(): Promise<string | null> {
  ...
  console.log("📡 [DEBUG] Refresh Token envoyé à l'API :", storedRefreshToken);

  try {
    // ✅ 2️⃣ Appel à l'API pour rafraîchir le JWT avec le **BON** Refresh Token
    const response = await fetch("https://ton-api.com/auth/refresh", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ refreshToken: storedRefreshToken }), // 🔥 Assure-toi que c'est bien `refreshToken`
    });
    ...
    if (!response.ok) {
      ...
      console.error("📡 [DEBUG] Réponse API complète :", data);
      return null;
    }

    if (!data.accessToken) {
      console.error("❌ [DEBUG] Impossible de rafraîchir le JWT, réponse API invalide :", data);
      await handleRefreshFailure();
      return null;
    }
    ...
    await saveJWTToIndexedDB(data.accessToken);
