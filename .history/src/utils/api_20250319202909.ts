...
export async function getValidToken(): Promise<string | null> {
  ...
  // 1️⃣ 🔥 Vérifier d'abord dans localStorage/sessionStorage
  let jwt = localStorage.getItem("jwt") || sessionStorage.getItem("jwt");
  ...
  // 2️⃣ 🔄 Si pas trouvé, essayer IndexedDB
...
export function isJwtExpired(token: string | { key: string; value: string } | null): boolean {
  ...
  console.log("📌 [isJwtExpired] Token reçu :", token);

  if (typeof token !== "string" || !token.includes(".") || token.split(".").length !== 3) {
    console.warn("⚠️ [isJwtExpired] Token malformé :", token);
    return true;
  }
  ...
  try {
    ...
    console.log("📌 [isJwtExpired] Payload décodé :", decodedPayload);

    if (!decodedPayload.exp) {
      console.warn("⚠️ [isJwtExpired] Le JWT ne contient pas de champ 'exp'.");
      return true;
    }

    const expDate = decodedPayload.exp * 1000; // Conversion en millisecondes
    const now = Date.now();

    console.log(`⏳ Expiration : ${new Date(expDate).toLocaleString()} | Maintenant : ${new Date(now).toLocaleString()}`);

    if (expDate < now) {
      console.warn("⚠️ [isJwtExpired] Token expiré.");
      return true;
    }
...
export async function restoreTokensIfNeeded() {
  ...
  try {
    const jwtData = await getFromIndexedDB("jwt");
    const refreshTokenData = await getFromIndexedDB("refreshToken");

    console.log("📌 JWT récupéré depuis IndexedDB :", jwtData);
    console.log("📌 Refresh Token récupéré :", refreshTokenData);

    if (jwtData?.value && !isJwtExpired(jwtData.value)) {
      console.log("✅ JWT restauré et encore valide !");
      localStorage.setItem("jwt", jwtData.value);
      return;
    }

    if (refreshTokenData?.value) {
      console.log("🔄 JWT expiré, mais refreshToken disponible. Tentative de refresh...");
      await handleRefreshToken();
    } else {
      console.warn("❌ Aucun refreshToken disponible, suppression des tokens expirés.");
    ...
    const db = await openDB("AuthDB", 1, {
      upgrade(db) {
        if (!db.objectStoreNames.contains("authStore")) {
          ...
          db.createObjectStore("authStore", { keyPath: "id" }); // ✅ Harmonisation avec "id"
        }
      },
    });
...
export async function refreshToken(): Promise<string | null> {
  if (isRefreshing) {
    ...
    return isRefreshing; // Si une tentative est déjà en cours, retourne la promesse existante
  }

  console.log("🔒 Activation du verrou de rafraîchissement...");
  isRefreshing = new Promise<string | null>((resolve, reject) => {
    resolvePromise = (value: string) => resolve(value || "");
  });

  try {
    // Si le JWT n'est pas présent dans le localStorage/sessionStorage, on tente un rafraîchissement
    let token = localStorage.getItem("jwt") || sessionStorage.getItem("jwt");
    if (!token) {
      console.log("⚠️ Aucun JWT trouvé. Tentative de rafraîchissement...");

      // Essayer de récupérer le refresh token
      ...
      const data = await Promise.race([ 
        fetchPromise, 
        new Promise((_, reject) => setTimeout(() => reject("Timeout"), 15000)) // Timeout de 15 secondes
      ]);
      ...
      if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
        ...
        // Supprimer l'ancien JWT avant d'ajouter le nouveau
        await removeItemFromStore("authStore", "jwt");

        // Synchronisation des nouveaux tokens
        ...
        window.location.reload(); // Recharger l'interface Vue
      ...
      return token; // Le JWT est valide, on le retourne
    }
  } catch (error) {
...
function removeItemFromStore(store: string, key: string) {
  const fullKey = `${store}:${key}`; // Vous pourriez gérer le store et la clé de cette manière
...
export async function handleRefreshToken() {
  // Si une tentative de rafraîchissement est déjà en cours, on attend qu'elle se termine
  if (isRefreshing) {
    console.log("🔄 Rafraîchissement déjà en cours, on attend.");
    return isRefreshing; // On retourne la promesse pour que les appels suivants attendent
  }

  isRefreshing = new Promise<string | null>(async (resolve, reject) => {
    const storedRefreshToken = localStorage.getItem("refreshToken") || sessionStorage.getItem("refreshToken");

    // Si le refresh token est présent, tenter un rafraîchissement
    if (storedRefreshToken) {
      try {
        const response = await refreshToken(); // Appel à l'API pour rafraîchir les tokens

        // Vérification et traitement de la réponse
        let responseData;
        try {
          if (!response) {
            throw new Error("Réponse vide ou mal formatée");
          }
          responseData = JSON.parse(response); // Décodage de la réponse
        } catch (error) {
          reject("Réponse serveur invalide ou mal formatée");
          return;
        }
        ...
        if (responseData.status === "success") {
          ...
          // Forcer un re-rendu dans l'interface
          authStore.triggerRefresh(); // Déclenche un re-rendu

          resolve(newJwt); // Résolution avec le nouveau JWT
        } else {
          reject("Échec du rafraîchissement du token");
        }
      } catch (error) {
      ...
      reject("Aucun refresh token disponible");
    }
  });

  // Gérer l'échec du rafraîchissement (échec de la promesse)
  isRefreshing.catch(() => {
    // Redirige l'utilisateur si le rafraîchissement échoue
    console.warn("⚠️ Le rafraîchissement des tokens a échoué.");
    router.replace("/login");
  }).finally(() => {
    // Libérer la promesse quand le processus de rafraîchissement est terminé
    isRefreshing = null;
  });

  return isRefreshing; // Retourne la promesse du rafraîchissement
}
...
export async function handleRefreshFailure() {
  ...
  if (jwtExists || refreshTokenExists) {
    // ✅ L'utilisateur était connecté → on affiche un message + déconnexion
  ...
  // ✅ Si l'utilisateur était connecté, on redirige vers la page de connexion
  // Si l'utilisateur n'était pas connecté, on n'a pas besoin de redirection ici
  if (jwtExists || refreshTokenExists) {
    router.replace("/login");
  }
