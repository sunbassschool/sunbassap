...
export async function verifyIndexedDBSetup(): Promise<boolean> {
  ...
  try {
    ...
    // Ouvrir la base de données pour voir si `authStore` existe
    let db = await openDB("AuthDB", 1);
    console.log("📂 Stores existants :", db.objectStoreNames);

    if (!db.objectStoreNames.contains("authStore")) {
      console.warn("⚠️ `authStore` est manquant ! Tentative de réparation...");

      db.close(); // Fermer proprement avant de tenter un fix

      // 🚀 Nouvelle approche : ouvrir avec `onupgradeneeded` pour recréer `authStore`
      const dbRequest = indexedDB.open("AuthDB", 1);

      dbRequest.onupgradeneeded = (event) => {
        console.log("📌 `onupgradeneeded` déclenché ! Tentative de création de `authStore`...");
      ...
      dbRequest.onsuccess = (event) => {
        const upgradeDb = dbRequest.result;
        console.log("📂 Après correction, stores disponibles :", upgradeDb.objectStoreNames);
        upgradeDb.close();
      };
