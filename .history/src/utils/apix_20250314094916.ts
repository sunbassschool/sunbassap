...
let indexedDBCleanupRunning = false;
...
export async function preventIndexedDBCleanup(): Promise<void> {
...
export async function checkIndexedDBStatus(): Promise<void> {
  try {
    ...
    console.log("🔍 IndexedDB contient le JWT :", jwt ? jwt.value : "❌ Perdu !");
  } catch (error) {
    console.warn("⚠️ IndexedDB introuvable après réveil :", error);
  }
}
