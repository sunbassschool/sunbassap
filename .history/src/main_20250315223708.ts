...
async function initApp() {
  ...
  // ✅ Masquer l'application tant que l'authentification n'est pas prête
  ...
  app.mount("#app"); // Monter l'application immédiatement (même si invisible)

  checkAndFixLogo();

  // ✅ Vérifier que IndexedDB est prête
  const isDBReady = await verifyIndexedDBSetup();
  if (!isDBReady) {
    console.warn("⚠️ IndexedDB non prête, arrêt de l'initialisation !");
    return;
  }

  console.log("🔄 Vérification et restauration des tokens...");
  const authStatus = await checkAndRestoreTokens();

  if (authStatus === "valid") {
    console.log("✅ L'utilisateur est connecté !");
    isAuthReady = true;
  } else if (authStatus === "expired") {
    console.warn("⚠️ Session expirée, redirection vers la page de connexion...");
    await logoutUser();
    router.replace("/login");
  } else {
    console.warn("⚠️ L'utilisateur n'est pas connecté, redirection vers login.");
    router.replace("/login");
  }

  // ✅ Afficher l'application seulement après la vérification
  setTimeout(() => {
    if (loadingScreen) {
