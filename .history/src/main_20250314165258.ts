...
async function initIndexedDB() {
  ...
  try {
    ...
    // ✅ Ne plus restaurer les tokens ici (c'est fait dans `initAuth()`)
    await initAuth();  
  } catch (error) {
...
async function initAuth() {
  ...
  try {
    ...
    // ✅ 🔥 Vérification et restauration des tokens AVANT la vérification utilisateur
    await restoreTokensIfNeeded();

    // ✅ 🔥 Mise à jour forcée des tokens après restauration
    const jwt = localStorage.getItem("jwt");
    const refreshToken = localStorage.getItem("refreshToken");
    if (jwt && refreshToken) {
      console.log("🔄 Mise à jour forcée des tokens après restauration...");
