...
async function initApp() {
  ...
  const hasValidTokens = await restoreTokensIfNeeded();

  if (!hasValidTokens) {
    console.warn("⚠️ Aucun token valide, redirection vers /login.");
    router.replace("/login");
    return;
  }
  ...
  // ✅ Afficher l'application une fois l'authentification prête
  setTimeout(() => {
    if (loadingScreen) {
