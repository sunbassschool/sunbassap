...
,
  ...
  // ✅ Décalage initial pour permettre au premier clic d'être entendu
  this.nextNoteTime = this.audioContext.currentTime + 0.1;

  // ✅ Lance l’audio scheduling dès maintenant (y compris le premier clic)
  this.scheduleBeat();

  // ✅ Active les flags de lecture
  this.metronome.isPlaying = true;
  this.isPlaying = true;

  // ✅ Démarre la boucle de planification continue
...
,
    scheduleBeat() {
      ...
      while (this.nextNoteTime < now + 0.1) {
        this.playClick(this.nextNoteTime, 1, 1); // ✅ Forcer beat/sub = 1

        this.nextNoteTime += subDuration;
      }
    },
    playClick(time) {
  // 🧠 Capture l'état AVANT mise à jour du store
  console.log(`[CLICK] Beat: ${this.metronome.currentBeat}, Sub: ${this.metronome.currentSubdivision}`);

  const beat = this.metronome.currentBeat;
  const sub = this.metronome.currentSubdivision;
  const isStrongBeat = beat === 1 && sub === 1 && !this.metronome.disableStrongBeat;

  let buffer, volume;

  if (sub === 1) {
    buffer = isStrongBeat ? this.soundBuffers.strong : this.soundBuffers.weak;
    volume = isStrongBeat ? this.volumeStrong : this.volumeWeak;
  } else {
