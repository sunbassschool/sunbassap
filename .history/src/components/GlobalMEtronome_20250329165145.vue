...
// Appliquer swing sur les subdivisions (ex: 2e, 4e, etc.)
if (this.subdivision > 1 && this.currentSubdivision % 2 === 0) {
  swingOffset = (this.swingAmount * subDuration) / 2;
}

this.nextNoteTime += subDuration + swingOffset;
  }
}
...
playClick(time) {
  ...
  if (isFirstSub) {
    // 🥁 On est sur le début d'un temps
    if (isFirstBeat) {
