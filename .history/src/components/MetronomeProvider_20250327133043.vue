...
function startMetronome() {
  ...
  initAudioContext(); // Initialiser l'AudioContext
  audioContext.resume(); // Démarrer le contexte audio
  currentBeat = 1;
  currentSubdivision = 0;
  nextNoteTime = audioContext.currentTime;
  isPlaying.value = true;
  scheduleNextBeat(); // Démarrer les beats
}

/* ⏹️ Arrêt */
function stopMetronome() {
  isPlaying.value = false;
  clearInterval(beatIntervalId); // Remplacer cancelAnimationFrame par clearInterval
  showMiniMetronome.value = true; // Afficher le widget même après l'arrêt
}
...
document.addEventListener('visibilitychange', () => {
  if (!document.hidden && isPlaying.value) {
    audioContext.resume(); // Reprendre l'audio quand l'utilisateur revient dans l'application
    scheduleNextBeat(); // Relancer les battements du métronome
  }
});
...
function scheduleNextBeat() {
  ...
  beatIntervalId = setInterval(() => scheduleNextBeat(), 1000 / (tempo.value / 60)); // Gérer les battements avec setInterval
}
