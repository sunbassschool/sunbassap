...
export default {
  ...
  computed: {
    ...
    tempo: {
      ...
      set(val) {
        this.metronome.tempo = val; // Met à jour le tempo dans le store
      }
    },
    ...
    // Synchronise le volume fort avec le store Pinia
    volumeStrong: {
      get: () => this.metronome.volumeStrong,  // Lire le volume fort depuis le store
      set: (val) => this.metronome.setVolumeStrong(val)  // Mettre à jour le volume fort dans le store
    },

    // Synchronisation avec le store pour la subdivision
    subdivision: {
      get: () => this.metronome.subdivision, // Lire la subdivision depuis le store
      set: (val) => this.metronome.setSubdivision(val)  // Mettre à jour la subdivision dans le store
    }
  },
  ...
  methods: {
    ...
    startMetronome() {
      const metronome = useMetronomeStore(); // 👈 ici
    ...
    stopMetronome() {
      const metronome = useMetronomeStore(); // 👈 AJOUT
    ...
    playClick(time) {
      ...
      let volume = this.volumeSub; // Applique volumeSub ici
