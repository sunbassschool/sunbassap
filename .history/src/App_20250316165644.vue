...
const isAuthenticated = ref(false); // ✅ Indique si l'utilisateur est connecté
...
onMounted(async () => {
  ...
  // ✅ Timeout de 7s pour éviter les blocages
  ...
  try {
    ...
    if (!jwt) {
      ...
      isAuthenticated.value = false; // ✅ On force Vue à détecter l'état
...
// ✅ Surveille la route et met à jour l'authentification
watch(() => router.currentRoute.value, (newRoute) => {
  console.log("🔄 Changement de route :", newRoute.path);
  isAuthenticated.value = newRoute.path !== "/login";
});
</script>
