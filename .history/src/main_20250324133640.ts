...
function finalizeApp() {
  ...
  setTimeout(() => {
    if (loadingScreen) {
      loadingScreen.style.transition = "opacity 0.2s ease-out";  // Réduit le temps de transition
      loadingScreen.style.opacity = "0";  // Rendre invisible rapidement
      setTimeout(() => {
        loadingScreen.style.display = "none"; // Cache totalement l'écran après la transition
      }, 200); // Délai plus court (pour que le "fade-out" soit rapide)
    }
