...
export const useAuthStore = defineStore("auth", {
  ...
  actions: {
    ...
    async loadUser() {
      ...
      const userInfo = getUserInfoFromJWT(this.jwt as string);

      if (!userInfo) {
        console.warn("⚠️ JWT invalide, tentative de refresh");
        await this.refreshJwt();
      
        // 💡 Si toujours pas de JWT valide après refresh → logout ici
        if (!this.jwt) {
          console.error("🚨 Aucun JWT valide même après refresh. Déconnexion forcée.");
          this.logout();
        }
      } else {
        this.user = userInfo;
    ...
    async refreshJwt(): Promise<string | null> {
      ...
      try {
        ...
        if (newJwt) {
          ...
          console.warn("❌ Aucun JWT après refresh, mais on attend confirmation avant logout.");
          // ⚠️ Ne logout pas ici ! Laisse le flow décider après
        }
      } catch (err) {
        console.error("❌ Erreur lors du refresh JWT :", err);
        // ❗ Laisse le choix de logout à un autre niveau
      } finally {
