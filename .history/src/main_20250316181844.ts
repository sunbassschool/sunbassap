...
async function initApp() {
  ...
  // ✅ Masquer l'application et afficher un écran de chargement
  ...
  if (jwt && refreshToken) {
    ...
    await router.replace("/login"); 
    await nextTick(); // ✅ Attendre que Vue applique la redirection
    return; // 🚀 Stopper immédiatement l'exécution ici !
  }
  ...
  setTimeout(() => {
    ...
    if (appContainer) {
      ...
      appContainer.style.opacity = isAuthReady ? "1" : "0"; // 🔥 Ne pas afficher l'UI si non authentifié
    }
  }, 500);
}
