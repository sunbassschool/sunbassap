...
router.beforeEach(async (to, from, next) => {
  ...
  try {
    // 🛠️ Assurer que IndexedDB est prête
    ...
    // 🕵️‍♂️ Si le JWT est manquant mais un refresh est en cours, on attend
    if (!jwt && isRefreshing) {
      console.warn("🔄 Refresh Token en cours, on attend...");
    ...
    // 🚀 Si après le refresh, le JWT est là, on continue
    if (jwt) {
