...
router.beforeEach(async (to, from, next) => {
  ...
  // ✅ Bloquer la navigation vers /login tant que l'authentification n'est pas terminée
  if (!authCheckComplete && to.path === "/login") {
    console.warn("⏳ Authentification en cours, attente avant d'afficher /login...");
    return;
  }
  ...
  // ✅ Si JWT valide, empêcher l'accès à /login et rediriger immédiatement
  if (jwt && !isJwtExpired(jwt)) {
    console.log("✅ JWT valide, accès autorisé.");
    authCheckComplete = true;
    authCheckInProgress = false;
    return to.path === "/login" ? next("/dashboard") : next();
  }
  ...
  try {
    ...
    if (newJwt) {
      ...
      // ✅ Redirection immédiate pour éviter que l’utilisateur voie /login
      return next(to.path === "/login" ? "/dashboard" : to.fullPath);
    }
  } catch (error) {
