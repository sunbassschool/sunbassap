...
router.beforeEach(async (to, from, next) => {
  ...
  // ✅ Empêcher la boucle infinie de redirection vers /login
  if (to.path === "/login" && from.path === "/login") {
    console.warn("⚠️ Déjà sur /login, pas de nouvelle redirection.");
  ...
  if (jwt && !isJwtExpired(jwt)) {
    ...
    authCheckInProgress = false;
    return next(to.path === "/login" || to.path === "/" ? "/dashboard" : to.fullPath);
  }
  ...
  if (!storedRefreshToken) {
    ...
    // ✅ Empêcher de re-router si on est déjà sur la page de login
    if (to.path !== "/login") {
      return next("/login");
    }
    return next();
