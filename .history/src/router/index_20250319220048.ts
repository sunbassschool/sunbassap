...
router.beforeEach(async (to, from, next) => {
  ...
  try {
    ...
    if (!isDBReady) {
      ...
      return next("/login"); // 🚨 Ajout de la redirection
    }
    ...
    // 🚨 Ajout de la redirection si l'utilisateur tente d'accéder à une page protégée
    console.warn("🚨 Aucun JWT valide, redirection vers login !");
    authCheckInProgress = false;
    return next("/login"); // 🚨 C'était manquant ici

  } catch (error) {
    console.error("❌ Erreur d'authentification :", extractErrorMessage(error));
    authCheckInProgress = false;

    // 🚨 Ajout de la redirection si une erreur empêche l'accès
    return next("/login");
  }
});
