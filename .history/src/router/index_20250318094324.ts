...
async function initApp() {
  ...
  app.use(createPinia());
  app.use(router);
  app.mount("#app"); // ✅ Monte l'app immédiatement

  console.log("🔄 Vérification des tokens en background...");
  
  try {
    const isDBReady = await verifyIndexedDBSetup();
    if (!isDBReady) {
      throw new Error("IndexedDB non prête");
    }
...
initApp(); // ✅ Démarrer immédiatement
