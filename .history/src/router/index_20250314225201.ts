...
let authCheckComplete = false; // ✅ Bloque la redirection vers /login tant que le refresh est en cours
...
router.beforeEach(async (to, from, next) => {
  ...
  // ✅ Vérification que IndexedDB est prête AVANT d’accéder aux tokens
  ...
  // ✅ Si JWT valide, empêcher l'accès à /login
  if (jwt && !isJwtExpired(jwt) && to.path === "/login") {
    console.warn("🚫 Accès refusé : utilisateur déjà connecté !");
    authCheckInProgress = false;
    return next("/dashboard"); // 🔄 Rediriger vers la page principale de l’utilisateur
  }

  // ✅ Si JWT valide, accès autorisé
  ...
  try {
    ...
    if (newJwt) {
      ...
      // ✅ Empêcher un utilisateur connecté d’accéder à /login
      if (to.path === "/login") {
